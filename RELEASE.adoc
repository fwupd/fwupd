...
Create release and https://github.com/fwupd/fwupd/tags[upload tarball].
...
Update https://github.com/flathub/org.freedesktop.fwupd[flatpak] package
for new release:

=== Forking main

When forking `main` into a stable `2_1_X`, be sure to disable the
following CI jobs:

* `publish-docs` (`.circleci/config.yml`)
* `deploy-store` (`snap.yaml`)

Also update `SECURITY.md`, removing the oldest branch and add the new
branch at the top. To make sure it’s done right, you can the reference
https://github.com/fwupd/fwupd/commit/433e809318c68c9ab6d4ae50ee9c4312503185d8[this
commit].
