...
struct _FuQcFirehoseFirmware {
	FuFirmware parent_instance;
	guint32 image_id;
	GArray *allowed_pending_image_ids;
};

G_DEFINE_TYPE(FuQcFirehoseFirmware, fu_qc_firehose_firmware, FU_TYPE_ZIP_FIRMWARE)

void
fu_qc_firehose_firmware_set_image_id(FuQcFirehoseFirmware *self, guint32 image_id)
{
	g_return_if_fail(FU_IS_QC_FIREHOSE_FIRMWARE(self));
...
{
	g_return_val_if_fail(FU_IS_QC_FIREHOSE_FIRMWARE(self), FALSE);
...
static void
fu_qc_firehose_firmware_export(FuFirmware *firmware, FuFirmwareExportFlags flags, XbBuilderNode *bn)
{
	FuQcFirehoseFirmware *self = FU_QC_FIREHOSE_FIRMWARE(firmware);
...
static void
fu_qc_firehose_firmware_init(FuQcFirehoseFirmware *self)
{
	self->allowed_pending_image_ids = g_array_new(FALSE, FALSE, sizeof(guint32));
}

static void
fu_qc_firehose_firmware_finalize(GObject *object)
{
	FuQcFirehoseFirmware *self = FU_QC_FIREHOSE_FIRMWARE(object);
	g_array_unref(self->allowed_pending_image_ids);
	G_OBJECT_CLASS(fu_qc_firehose_firmware_parent_class)->finalize(object);
}

static void
fu_qc_firehose_firmware_class_init(FuQcFirehoseFirmwareClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuFirmwareClass *firmware_class = FU_FIRMWARE_CLASS(klass);
	object_class->finalize = fu_qc_firehose_firmware_finalize;
	firmware_class->export = fu_qc_firehose_firmware_export;
}

FuQcFirehoseFirmware *
fu_qc_firehose_firmware_new(void)
{
	return g_object_new(FU_TYPE_QC_FIREHOSE_FIRMWARE, NULL);
}
