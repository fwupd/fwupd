...
fu_qc_firehose_device_erase(FuQcFirehoseDevice *self, XbNode *xn, GError **error)
{
	...
	if (!fu_qc_firehose_device_write_xml(self, bn, error)) {
		g_prefix_error(error, "failed to erase program command: ");
		return FALSE;
	}
			      ...
			      GError **error)
{
	...
	if (!fu_qc_firehose_device_write_xml(self, bn, error)) {
		g_prefix_error(error, "failed to write program command: ");
		return FALSE;
	}
	if (!fu_qc_firehose_device_read_xml(self, 5 * FU_QC_FIREHOSE_DEVICE_TIMEOUT_MS, error)) {
		g_prefix_error(error, "failed to set up write program: ");
		return FALSE;
	}
...
fu_qc_firehose_device_apply_patch(FuQcFirehoseDevice *self, XbNode *xn, GError **error)
{
	...
	if (!fu_qc_firehose_device_write_xml(self, bn, error)) {
		g_prefix_error(error, "failed to patch program command: ");
		return FALSE;
	}
...
static gboolean
fu_qc_firehose_device_write_firmware_targets(FuQcFirehoseDevice *self,
					     FuFirmware *firmware,
					     GPtrArray *xns,
					     FuProgress *progress,
					     GError **error)
{
	...
	for (guint i = 0; i < xns->len; i++) {
		...
		if (g_strcmp0(xb_node_get_element(xn), "erase") == 0) {
			if (!fu_qc_firehose_device_erase(self, xn, error))
				return FALSE;
		} else if (g_strcmp0(xb_node_get_element(xn), "program") == 0) {
			const gchar *filename = xb_node_get_attr(xn, "filename");
		...
		} else if (g_strcmp0(xb_node_get_element(xn), "patch") == 0) {
...
fu_qc_firehose_device_find_bootable(FuQcFirehoseDevice *self, GPtrArray *xns)
{
	for (guint i = 0; i < xns->len; i++) {
		...
		if (filename == NULL)
			continue;
		if (g_strcmp0(xb_node_get_element(xn), "program") != 0)
			continue;
					     ...
					     GError **error)
{
	...
	/* each action */
	xns = xb_silo_query(silo, "data/*", 0, error);
	if (xns == NULL) {
		g_prefix_error(error, "failed to get targets: ");
		return FALSE;
	}
	if (!fu_qc_firehose_device_write_firmware_targets(self,
							  firmware,
							  xns,
							  fu_progress_get_child(progress),
							  error)) {
		g_prefix_error(error, "failed to write targets: ");
		return FALSE;
	}
	fu_progress_step_done(progress);
	...
	if (self->supported_functions & FU_QC_FIREHOSE_FUNCTIONS_SETBOOTABLESTORAGEDRIVE) {
		guint64 bootable = fu_qc_firehose_device_find_bootable(self, xns);
