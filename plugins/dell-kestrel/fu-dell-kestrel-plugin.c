...
fu_dell_kestrel_plugin_device_registered(FuPlugin *plugin, FuDevice *device)
{
	...
	if (fu_usb_device_get_vid(FU_USB_DEVICE(device)) == MST_VMM89_USB_VID &&
	    fu_usb_device_get_pid(FU_USB_DEVICE(device)) == MST_VMM89_USB_PID)
...
fu_dell_kestrel_plugin_get_rtshub_from_devices(GPtrArray *devices, guint16 pid)
{
	for (guint i = 0; i < devices->len; i++) {
		...
		if (!FU_IS_USB_DEVICE(dev))
			continue;

		dev_pid = fu_usb_device_get_pid(FU_USB_DEVICE(dev));
		if (dev_pid != pid)
			continue;

		if (!FU_IS_DELL_KESTREL_RTSHUB(dev))
			return dev;
	}
	return NULL;
}
...
fu_dell_kestrel_plugin_backend_device_removed(FuPlugin *plugin, FuDevice *device, GError **error)
{
	...
	if (FU_IS_USB_DEVICE(device)) {
		...
		if (fu_plugin_cache_lookup(plugin, key))
			fu_plugin_cache_remove(plugin, key);
	}

	for (gsize i = 0; i < G_N_ELEMENTS(cache_keys); i++) {
		if (fu_plugin_cache_lookup(plugin, cache_keys[i]))
