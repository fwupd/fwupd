...
{
	g_autoptr(GByteArray) req = fu_struct_dell_kestrel_ec_databytes_new();
	g_autoptr(GError) error_local = NULL;
	g_autofree gchar *msg = NULL;
	guint16 bitmask = 0x0;

	fu_struct_dell_kestrel_ec_databytes_set_cmd(req, FU_DELL_KESTREL_EC_CMD_SET_MODIFY_LOCK);
	fu_struct_dell_kestrel_ec_databytes_set_data_sz(req, 2);
	...
	if (!fu_struct_dell_kestrel_ec_databytes_set_data(req,
							  (guint8 *)&bitmask,
							  sizeof(bitmask),
							  error))
		return FALSE;
...
fu_dell_kestrel_ec_run_passive_update(FuDellKestrelEc *self, GError **error)
{
	...
	fu_struct_dell_kestrel_ec_databytes_set_cmd(req, FU_DELL_KESTREL_EC_CMD_SET_PASSIVE);
	fu_struct_dell_kestrel_ec_databytes_set_data_sz(req, 1);
	if (!fu_struct_dell_kestrel_ec_databytes_set_data(req,
							  (guint8 *)&bitmap,
							  sizeof(bitmap),
							  error))
...
fu_dell_kestrel_ec_commit_package(FuDellKestrelEc *self, GInputStream *stream, GError **error)
{
	...
	/* get the data bytes */
	bytes = fu_input_stream_read_byte_array(stream,
						0,
						FU_STRUCT_DELL_KESTREL_PACKAGE_FW_VERSIONS_SIZE,
						NULL,
						error);

	fu_struct_dell_kestrel_ec_databytes_set_cmd(req, FU_DELL_KESTREL_EC_CMD_SET_DOCK_PKG);
	fu_struct_dell_kestrel_ec_databytes_set_data_sz(req, streamsz);
	if (!fu_struct_dell_kestrel_ec_databytes_set_data(req, bytes->data, bytes->len, error))
		return FALSE;
