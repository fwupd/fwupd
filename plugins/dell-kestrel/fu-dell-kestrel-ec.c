...
fu_dell_kestrel_ec_commit_package(FuDellKestrelEc *self, GBytes *blob_fw, GError **error)
{
	...
	if (bufsz != FU_STRUCT_DELL_KESTREL_PACKAGE_FW_VERSIONS_SIZE) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INTERNAL,
	...
	fu_struct_dell_kestrel_ec_databytes_set_data_sz(st_req, bufsz);
	if (!fu_struct_dell_kestrel_ec_databytes_set_data(st_req, buf, bufsz, error))
		return FALSE;
