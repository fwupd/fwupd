...
fu_dell_kestrel_rtshub_to_string(FuDevice *device, guint idt, GString *str)
{
	...
	fu_string_append_kb(str, idt, "DockType", self->dock_type);
}
				      ...
				      GError **error)
{
	...
	chunks = fu_chunk_array_new_from_bytes(fw, 0, DELL_KESTREL_RTSHUB_TRANSFER_BLOCK_SIZE);
	if (chunks == NULL)
		return FALSE;
...
fu_dell_kestrel_rtshub_probe(FuDevice *device, GError **error)
{
	...
	/* not interesting */
	if (fu_usb_device_get_vid(FU_USB_DEVICE(device)) != DELL_VID) {
		g_set_error(error,
			    FWUPD_ERROR,
	...
	/* build logical id */
	logical_id = g_strdup_printf("RTSHUB_%04X", fu_usb_device_get_pid(FU_USB_DEVICE(device)));
