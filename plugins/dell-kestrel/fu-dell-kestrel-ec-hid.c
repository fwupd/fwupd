...
{
	g_autoptr(GByteArray) fwbuf = fu_struct_dell_kestrel_ec_hid_fw_update_pkg_new();
	gsize chk_datasz = fu_chunk_get_data_sz(chk);

	/* header */
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_cmd(fwbuf,
							    FU_DELL_KESTREL_EC_HID_CMD_FWUPDATE);
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_ext(fwbuf,
							    FU_DELL_KESTREL_EC_HID_EXT_FWUPDATE);
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_chunk_sz(
	    fwbuf,
	    7 + chk_datasz); // 7 = sizeof(command)

	/* command */
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_sub_cmd(
	    fwbuf,
	    FU_DELL_KESTREL_EC_HID_SUBCMD_FWUPDATE);
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_dev_type(fwbuf, dev_type);
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_dev_identifier(fwbuf, dev_identifier);
	fu_struct_dell_kestrel_ec_hid_fw_update_pkg_set_fw_sz(fwbuf, fw_size);
...
{
	g_autoptr(GByteArray) buf = fu_struct_dell_kestrel_ec_hid_cmd_buffer_new();
	g_return_val_if_fail(cmd_buf->len <= FU_DELL_KESTREL_HIDI2C_MAX_WRITE, FALSE);

	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_cmd(buf,
							 FU_DELL_KESTREL_EC_USB_HID_CMD_WRITE_DATA);
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_ext(
	    buf,
	    FU_DELL_KESTREL_EC_USB_HID_CMD_EXT_I2C_WRITE);
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_dwregaddr(buf, 0x00);
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_bufferlen(buf, GUINT16_TO_LE(cmd_buf->len));
	if (!fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_databytes(buf,
								    cmd_buf->data,
								    cmd_buf->len,
								    error))
		return FALSE;
				...
				GError **error)
{
	...
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_ext(
	    buf,
	    FU_DELL_KESTREL_EC_USB_HID_CMD_EXT_I2C_READ);
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_dwregaddr(buf, GUINT32_TO_LE(cmd));
	fu_struct_dell_kestrel_ec_hid_cmd_buffer_set_bufferlen(buf, GUINT16_TO_LE(res->len + 1));
