					 ...
					 GError **error)
{
	...
	g_autoptr(FuStructSmbiosDd) st_oem = NULL;

	st_oem = fu_struct_smbios_dd_parse_bytes(blob, 0x0, error);
	if (st_oem == NULL)
		return FALSE;
	if (fu_struct_smbios_dd_get_subtype(st_oem) != FU_STRUCT_SMBIOS_DD_SUBTYPE_ME)
		return TRUE;

	st_ver = fu_struct_me_version_parse_bytes(blob, 21, error);
	if (st_ver == NULL)
		return FALSE;
