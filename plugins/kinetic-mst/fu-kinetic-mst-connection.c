...
{
	return fu_kinetic_mst_connection_aux_node_read (self, offset, buf,
							 length, error);
}
...
fu_kinetic_mst_connection_bus_write (FuKineticMstConnection *self,
				       ...
				       guint32 length, GError **error)
{
	return fu_kinetic_mst_connection_aux_node_write (self, offset, buf,
							  length, error);
}

FuKineticMstConnection *
fu_kinetic_mst_connection_new (gint fd, guint8 layer, guint rad)
{
...
gboolean
fu_kinetic_mst_connection_rc_set_command (FuKineticMstConnection *self,
					    guint32 rc_cmd,
					    guint32 length,
					    guint32 offset,
					    const guint8 *buf,
					    GError **error)
{
	...
	do {
		...
		if (cur_length) {
			/* write data */
			if (!fu_kinetic_mst_connection_write (self,
							        REG_RC_DATA,
							        buf, cur_length,
							        error)) {
...
gboolean
fu_kinetic_mst_connection_rc_get_command (FuKineticMstConnection *self,
					    guint32 rc_cmd,
					    guint32 length,
					    guint32 offset,
					    guint8 *buf,
...
gboolean
fu_kinetic_mst_connection_rc_special_get_command (FuKineticMstConnection *self,
						    guint32 rc_cmd,
						    guint32 cmd_length,
						    guint32 cmd_offset,
						    guint8 *cmd_data,
...
gboolean
fu_kinetic_mst_connection_enable_rc (FuKineticMstConnection *self, GError **error)
{
	const gchar *sc = "PRIUS";

	for (gint i = 0; i <= self->layer; i++) {
		g_autoptr(FuKineticMstConnection) connection_tmp = NULL;
		connection_tmp = fu_kinetic_mst_connection_new (self->fd, i, self->rad);
		if (!fu_kinetic_mst_connection_rc_set_command (connection_tmp,
								 UPDC_ENABLE_RC,
								 5, 0, (guint8*)sc,
								 error)) {
...
gboolean
fu_kinetic_mst_connection_disable_rc (FuKineticMstConnection *self, GError **error)
{
