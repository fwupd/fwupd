...
#define FU_TEST_STARLABS_MANUFACTURER "Star Labs"
...
static FuPlugin *
fu_test_bios_plugin_new(const gchar *manufacturer, const gchar *bios_version)
{
...
fu_bios_plugin_starlabs_coreboot_legacy_func(void)
{
	...
	g_autoptr(FuPlugin) plugin =
	    fu_test_bios_plugin_new(FU_TEST_STARLABS_MANUFACTURER, "CBET4000 25.01");
...
fu_bios_plugin_starlabs_coreboot_supported_func(void)
{
	gboolean ret;
	g_autoptr(FuPlugin) plugin =
	    fu_test_bios_plugin_new(FU_TEST_STARLABS_MANUFACTURER, "CBET4000 26.02");
	g_autoptr(FuProgress) progress = fu_progress_new(G_STRLOC);
	g_autoptr(GError) error = NULL;
...
static void
fu_bios_plugin_coreboot_other_manufacturer_func(void)
{
	gboolean ret;
	g_autoptr(FuPlugin) plugin = fu_test_bios_plugin_new("Not Star Labs", "CBET4000 25.01");
	g_autoptr(FuProgress) progress = fu_progress_new(G_STRLOC);
	g_autoptr(GError) error = NULL;
...
main(int argc, char **argv)
{
	...
	g_test_add_func("/fwupd/plugin/bios/coreboot-other-manufacturer",
			fu_bios_plugin_coreboot_other_manufacturer_func);
