...
#define FU_BIOS_PLUGIN_STARLABS_MANUFACTURER	     "Star Labs"
...
fu_bios_plugin_is_starlabs_coreboot_legacy(FuPlugin *plugin, gchar **version_out)
{
	...
	if (g_strcmp0(fu_context_get_hwid_value(ctx, FU_HWIDS_KEY_BIOS_VENDOR), "coreboot") != 0)
		return FALSE;
	if (g_strcmp0(fu_context_get_hwid_value(ctx, FU_HWIDS_KEY_MANUFACTURER),
		      FU_BIOS_PLUGIN_STARLABS_MANUFACTURER) != 0)
		return FALSE;
