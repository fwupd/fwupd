...
This plugin reads the sysfs attributes corresponding to eMMC devices. It
uses the kernel MMC API for flashing devices.
...
One deprecated instance ID is also added; new firmware should not use
this.
...
it is only activated when the device is rebooted.
...
The vendor ID is set from the EMMC vendor, for example set to
`EMMC:{$manfid}`
