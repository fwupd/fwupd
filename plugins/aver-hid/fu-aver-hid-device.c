...
static gboolean
fu_aver_hid_device_check_dual_isp(FuAverHidDevice *device)
{
	FuAverHidDevice *self = FU_AVER_HID_DEVICE(device);
	const guint16 aver_dual_isp_pid_group[] = { 0x0AB1 };
	for (guint i = 0; i < sizeof(aver_dual_isp_pid_group) / sizeof(aver_dual_isp_pid_group[0]); ++i) {
		if (aver_dual_isp_pid_group[i] == fu_usb_device_get_pid(FU_USB_DEVICE(self)))
			return TRUE;
	}
	return FALSE;
}
				   ...
				   GError **error)
{
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		/* copy in payload */
		if (fu_aver_hid_device_check_dual_isp(self) == TRUE) {
