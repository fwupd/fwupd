				 ...
				 GError **error)
{
	...
	if (!fu_hughski_colorhug_device_msg(self,
					    FU_HUGHSKI_COLORHUG_CMD_ERASE_FLASH,
					    buf,
					    sizeof(buf), /* in */
					    NULL,
					    0, /* out */
...
fu_hughski_colorhug_device_get_version(FuHughskiColorhugDevice *self, GError **error)
{
	guint8 buf[6] = {0};
	if (!fu_hughski_colorhug_device_msg(self,
					    FU_HUGHSKI_COLORHUG_CMD_GET_FIRMWARE_VERSION,
					    NULL,
					    0, /* in */
					    buf,
					    sizeof(buf), /* out */
					...
					GError **error)
{
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		buf[3] = 0xFF ^ fu_xor8(fu_chunk_get_data(chk), fu_chunk_get_data_sz(chk));
		if (!fu_memcpy_safe(buf,
				    sizeof(buf),
				    0x4, /* dst */
			...
			return FALSE;
		if (!fu_hughski_colorhug_device_msg(self,
						    FU_HUGHSKI_COLORHUG_CMD_WRITE_FLASH,
						    buf,
						    sizeof(buf), /* in */
						    NULL,
						    0, /* out */
