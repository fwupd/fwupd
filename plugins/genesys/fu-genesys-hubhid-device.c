...
typedef union {
	struct {
		guint8 recipient : 2; /* FuUsbDeviceRecipient */
		guint8 reserved : 3;
		guint8 type : 2; /* FuUsbDeviceRequestType */
		guint8 dir : 1;	 /* FuUsbDeviceDirection */
	};
	guint8 bm;
				     ...
				     GError **error)
{
	...
	setup->req_type.dir = (direction == FU_USB_DEVICE_DIRECTION_DEVICE_TO_HOST)
				  ? 1
...
fu_genesys_hubhid_device_probe(FuDevice *device, GError **error)
{
	if (fu_usb_device_get_device_class(FU_USB_DEVICE(device)) !=
	    FU_USB_DEVICE_CLASS_INTERFACE_DESC) {
