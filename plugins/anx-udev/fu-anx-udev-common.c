...
guint16 g_cust_version = 0;
guint16 g_fw_version = 0;
...
void set_custom_version (guint16 ver)
{
    g_cust_version = ver;
}
void set_fw_version (guint16 ver)
{
    g_fw_version = ver;
}
guint16 get_custom_version (void)
{
    return g_cust_version;
}
guint16 get_fw_version (void)
{
    return g_fw_version;
}
                        ...
                        AnxImgHeader* out_header, guint8 *out_binary)
{
    ...
    while (end_of_file == FALSE) {
        if (new_line) {
            ...
            new_line = FALSE;
            fw_index++;
            line_len = (guint8)hex_str_to_dec(
						(const gchar*)&hex_hdr[fw_index], 2);
            fw_index += 2;
            start_addr = (guint32)hex_str_to_dec(
                        (const gchar*)&hex_hdr[fw_index], 4);
            fw_index += 4;
            rec_type = (guint8)hex_str_to_dec(
						(const gchar*)&hex_hdr[fw_index], 2);
            fw_index +=2;
            switch (rec_type) {
                case 0:
                    ...
                    switch (start_addr) {
						...
						case FLASH_RXFW_ADDR:
							...
							if ((fw_max_addr > fw_start_addr) &&
								...
								fw_max_addr - fw_start_addr + last_len;
							}
							init_addr = TRUE;
							base_index = img_header->secure_tx_payload_len +
						...
						(fw_start_addr == FLASH_OCM_ADDR))
					{
						...
						img_header->fw_ver |= (guint8)hex_str_to_dec(
												(const gchar*)&hex_hdr[fw_index-8], 2);
					}
					break;
				case 1:
					end_of_file = TRUE;
				...
				case 2:
					addr_mode = 2;
					most_high_addr = (guint16)hex_str_to_dec(
													(const gchar*)&hex_hdr[fw_index], 4);
					checksum = line_len+rec_type +
									(guint8)(start_addr & 0xff) +
									(guint8)((start_addr >> 8)&0xff) + 
	...
	if ((img_header->secure_tx_start_addr != 0) &&
			(img_header->secure_tx_payload_len == 0)) {
			img_header->secure_tx_payload_len = fw_max_addr-fw_start_addr +
												last_len;
	}
	if ((img_header->secure_rx_start_addr != 0) &&
			(img_header->secure_rx_payload_len == 0)) {
			img_header->secure_rx_payload_len = fw_max_addr-fw_start_addr +
												last_len;
	}
	if ((img_header->custom_start_addr != 0) &&
			(img_header->custom_payload_len == 0)) {
			img_header->custom_payload_len = fw_max_addr-fw_start_addr +
											last_len;
	}
