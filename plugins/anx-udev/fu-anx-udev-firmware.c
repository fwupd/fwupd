...
struct _FuAnxUdevFirmware {
	FuFirmwareClass  parent_instance;
};

G_DEFINE_TYPE (FuAnxUdevFirmware, fu_anx_udev_firmware, FU_TYPE_FIRMWARE)


static gboolean
fu_anx_udev_firmware_parse (FuFirmware *firmware,
                           GBytes *fw,
                           guint64 addr_start,
                           guint64 addr_end,
                           ...
                           GError **error)
{
	...
	g_debug ("fu_anx_udev_firmware_parse, start:%d, end:%d, flag =%d",
	...
	g_debug ("fw_size : %ld", (glong)fw_size);
	...
	if (!parse_fw_hex_file (hex_hdr, fw_size, img_header, bin_buf))
		return FALSE;
	/* parse version */
	main_ocm_ver = img_header->fw_ver;
	if (main_ocm_ver == 0)
			main_ocm_ver = get_fw_version ();
	custom_fw_version = img_header->custom_ver;
	if (custom_fw_version == 0)
			custom_fw_version = get_custom_version ();
...
static void
fu_anx_udev_firmware_init (FuAnxUdevFirmware *self)
{
...
static void
fu_anx_udev_firmware_class_init (FuAnxUdevFirmwareClass *klass)
{
    FuFirmwareClass *klass_firmware = FU_FIRMWARE_CLASS (klass);
    klass_firmware->parse = fu_anx_udev_firmware_parse;
}

FuFirmware *
fu_anx_udev_firmware_new (void)
{
    return FU_FIRMWARE (g_object_new (FU_TYPE_ANX_UDEV_FIRMWARE, NULL));
}