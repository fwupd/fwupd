...
fu_steelseries_fizz_hid_command_cb(FuDevice *device, gpointer user_data, GError **error)
{
	...
	gboolean ret;
	guint8 rdata[FU_STEELSERIES_BUFFER_REPORT_SIZE] = {0};
	...
	if (!fu_udev_device_pwrite(FU_UDEV_DEVICE(device), 0, buf->data, buf->len, error)) {
	...
	st = fu_struct_steelseries_fizz_hid_response_parse(rdata, sizeof(rdata), 0x0, error);
	if (st == NULL)
		return FALSE;
	...
	if (!fu_memcpy_safe(buf->data, buf->len, 0, rdata, sizeof(rdata), 0, buf->len, error)) {
...
static gboolean
fu_steelseries_fizz_hid_command(FuSteelseriesFizzHid *self, GByteArray *buf, GError **error)
{
...
fu_steelseries_fizz_hid_ensure_version(FuSteelseriesFizzHid *self, GError **error)
{
	...
	fu_dump_raw(G_LOG_DOMAIN, "Version", st->data, st->len);
	if (!fu_steelseries_fizz_hid_command(self, st, error))
		return FALSE;
	fu_dump_raw(G_LOG_DOMAIN, "Version", st->data, st->len);

	/* success */
	version = fu_memstrsafe(st->data, st->len, 0x1, st->len - 1, error);
	if (version == NULL) {
