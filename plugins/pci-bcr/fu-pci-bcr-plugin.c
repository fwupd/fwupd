...
static gboolean
fu_pci_bcr_plugin_is_intel_spi(FuPciBcrPlugin *self, FuDevice *device)
{
	guint64 class_u64 = 0;
	...
	if (attr_vendor == NULL)
		return FALSE;
	if (!fu_strtoull(attr_vendor,
			 &vendor_u64,
			 0,
			 G_MAXUINT16,
			 FU_INTEGER_BASE_AUTO,
			 &error_local)) {
		g_debug("failed to parse PCI vendor %s: %s", attr_vendor, error_local->message);
		return FALSE;
	}
	...
	if (!fu_strtoull(attr_class,
			 ...
			 FU_INTEGER_BASE_AUTO,
			 &error_local)) {
		g_debug("failed to parse PCI class %s: %s", attr_class, error_local->message);
		return FALSE;
	}

	return (class_u64 >> 8) == FU_PCI_BCR_SPI_CLASS;
}
				       ...
				       GError **error)
{
	...
	if (!FU_IS_PCI_DEVICE(device))
		return TRUE;
	if (!fu_pci_bcr_plugin_is_intel_spi(self, device))
		return TRUE;
