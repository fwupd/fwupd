...
<<<<<<< HEAD
	for (guint i = 0; i < attrs->len; i++) {
		...
		/* look for MHI (Modem Host Interface) ports */
		if (!g_str_has_prefix(name, "mhi"))
			continue;

		g_autofree gchar *trigger_path = g_strdup_printf("%s/trigger_edl", name);
		g_debug("attempting to trigger EDL via %s", trigger_path);

		if (fu_udev_device_write_sysfs(FU_UDEV_DEVICE(device),
					       trigger_path, "1", 3000, error)) {
			return TRUE;
		}

		g_prefix_error(error,
			       "failed to trigger EDL for device %s: ",
