...
static gboolean
fu_mm_device_from_json(FuDevice *device, FwupdJsonObject *json_object, GError **error)
{
	FuMmDevice *self = FU_MM_DEVICE(device);
	const gchar *tmp;
	...
	/* optional properties */
	tmp = fwupd_json_object_get_string(json_object, "Version", NULL);
	if (tmp != NULL)
		fu_device_set_version(device, tmp);
	tmp = fwupd_json_object_get_string(json_object, "PhysicalId", NULL);
	if (tmp != NULL)
		fu_device_set_physical_id(device, tmp);
	tmp = fwupd_json_object_get_string(json_object, "BranchAt", NULL);
	if (tmp != NULL)
		fu_mm_device_set_branch_at(self, tmp);

	/* specified by ModemManager, unusually */
	json_array_ids = fwupd_json_object_get_array(json_object, "DeviceIds", NULL);
	...
	/* ports */
	json_object_ports = fwupd_json_object_get_object(json_object, "Ports", NULL);
...
static void
fu_mm_device_add_json(FuDevice *device, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuMmDevice *self = FU_MM_DEVICE(device);
	FuMmDevicePrivate *priv = GET_PRIVATE(self);
	...
	FU_DEVICE_CLASS(fu_mm_device_parent_class)->add_json(device, json_object, flags);

	/* optional properties */
	fwupd_codec_json_append(json_object, "GType", G_OBJECT_TYPE_NAME(self));
	if (fu_device_get_version(device) != NULL)
		fwupd_codec_json_append(json_object, "Version", fu_device_get_version(device));
	if (fu_device_get_physical_id(device) != NULL)
		fwupd_codec_json_append(json_object,
					"PhysicalId",
					fu_device_get_physical_id(device));
	if (priv->branch_at != NULL)
		fwupd_codec_json_append(json_object, "BranchAt", priv->branch_at);
	...
	for (guint i = 0; i < vendor_ids->len; i++) {
		...
		if (g_str_has_prefix(vendor_id, "USB:0x")) {
			...
			fwupd_json_array_add_string(json_array, id);
		}
	...
	fwupd_json_object_add_array(json_object, "DeviceIds", json_array);
	...
	fwupd_json_object_add_object(json_object, "Ports", json_object_ports);
}
