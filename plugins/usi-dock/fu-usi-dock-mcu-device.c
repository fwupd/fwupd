...
fu_usi_dock_mcu_device_reset_usb2(FuUsiDockMcuDevice *self, GError **error)
{
	...
	if (proxy == NULL) {
		g_set_error(error, FWUPD_ERROR, FWUPD_ERROR_NOT_SUPPORTED, "no USB2 proxy");
		return FALSE;
	}
	...
	if (locker == NULL) {
		return FALSE;
		g_prefix_error(error, "failed to open USB2 proxy: ");
	}
	if (!fu_usb_device_reset(FU_USB_DEVICE(proxy), error)) {
		g_prefix_error(error, "failed to reset USB2 proxy: ");
		return FALSE;
	}
