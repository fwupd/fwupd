...
This plugin uses the MCU to write all the dock firmware components. The
MCU version is provided by the DMC bcdDevice.
...
Workaround a provisioning problem by setting the chip type when the new
