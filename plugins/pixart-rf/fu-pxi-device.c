                              ...
                              GError **error)
{
	...
	g_autoptr(GArray) fw_check_sum_table = g_array_new (FALSE, TRUE, sizeof (guint16));
	const guint8 *fw_ptr;
	const gchar *fw_ver_ptr;
	gsize fw_sz = 0;
	guint16 checksum = 0;
	guint16 fw_bin_checksum = 0;
	guint32 idx = 0;
	...
	for (idx = 0 ; idx < fw_sz / MAX_OBJECT_SIZE  ; idx++) {
		...
		g_debug ("idx %u checksum %x checksum in table %x", idx, checksum, g_array_index (fw_check_sum_table, guint16, idx));

	}
	/* add whole bin checksum */
	...
	if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, 2, error))
		return FALSE;
	...
	if (!fu_memcpy_safe (buf, PXI_OTA_BUF_SZ, 2, (guint8*)&fw_ota_init_new, sizeof(struct cmd_fw_ota_init_new), 0, sizeof(struct cmd_fw_ota_init_new), error))
		return FALSE;
	
	if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, sizeof(struct cmd_fw_ota_init_new) + 2, error))
		return FALSE;
	...
	if (!fu_memcpy_safe ((guint8 *)&evt, sizeof(struct hci_evt), 0, buf,PXI_OTA_BUF_SZ, 0, sizeof(struct hci_evt), error))
		return FALSE;
	...
	do {
		...
		if (!fu_memcpy_safe (buf, PXI_OTA_BUF_SZ, 2, (guint8*)&fw_object_create, sizeof(struct cmd_fw_object_create), 0, sizeof(struct cmd_fw_object_create), error))
			return FALSE;		
		
		if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, sizeof(struct cmd_fw_object_create) + 2, error))
			return FALSE;
		...
		while (object_size > 0) {
			...
			if(!fu_memcpy_safe (buf, PXI_OTA_BUF_SZ, 1, (guint8*)&fw_ptr[offset],write_sz, 0, write_sz, error))
				return FALSE;

			if(!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, write_sz + 1, error))
				return FALSE;
			...
			if (prn >= evt.evt_param.ret_param.fw_ota_init_new.prn_threshold) {
				...
				if (!fu_memcpy_safe ((guint8 *)&fw_notify_result, sizeof(struct ret_fw_notify),0, buf, PXI_OTA_BUF_SZ,1, sizeof(struct ret_fw_notify), error))
					return FALSE;
			...
			if (((object_size == 0) && (offset != g_bytes_get_size (fw))) || ((offset == g_bytes_get_size (fw)))) {

				g_debug("object write done");
				g_clear_error (error);
				memset(buf, 0, PXI_OTA_BUF_SZ);
				...
				g_debug ("checksum %x ,table checksum %x", fw_notify_result.checksum,g_array_index (fw_check_sum_table, guint16, idx));

				if (fw_notify_result.checksum != g_array_index (fw_check_sum_table, guint16, idx)) {
	...
	if (!fu_memcpy_safe (buf, PXI_OTA_BUF_SZ, 2, (guint8*)&fw_upgrade,sizeof(struct cmd_fw_upgrade), 0, sizeof(struct cmd_fw_upgrade), error))
		return FALSE;
		
	if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, sizeof(struct cmd_fw_upgrade) + 2, error))
		return FALSE;
	fu_common_dump_raw (G_LOG_DOMAIN, "buf", (guint8 *)buf, PXI_OTA_BUF_SZ);
	/* read fw updgrad command result */
	g_debug ("read fw updgrad result");
	memset (buf, 0, PXI_OTA_BUF_SZ);
	buf[0] = PXI_HID_DEV_OTA_INPUT_REPORT_ID;
	
	fu_udev_device_pread_full (FU_UDEV_DEVICE (device), 1, buf, PXI_OTA_BUF_SZ - 1, error);
	fu_common_dump_raw (G_LOG_DOMAIN, "buf", (guint8 *)buf, PXI_OTA_BUF_SZ);
	...
	if (!fu_memcpy_safe (buf, PXI_OTA_BUF_SZ, 2, (guint8*)&fw_ota_disconnect, sizeof(struct cmd_fw_ota_disconnect), 0, sizeof(struct cmd_fw_ota_disconnect), error))
		return FALSE;	
	
	if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, buf, sizeof(struct cmd_fw_ota_disconnect) + 2, error))
		return FALSE;
...
fu_pxi_device_setup (FuDevice *device, GError **error)
{
	...
	g_autofree gchar *current_checksum = NULL;	
	struct hci_evt evt;
	...
	if (!fu_udev_device_pwrite_full (FU_UDEV_DEVICE (device), 0, (guint8 *)buf, 2, error))
		return FALSE;
	...
	if (!fu_memcpy_safe((guint8 *)&evt, sizeof(struct hci_evt), 0, buf, 64, 0, sizeof(struct hci_evt), error))
		return FALSE;
	...
	current_checksum = g_strdup_printf ("0x%4x",evt.evt_param.ret_param.fw_info_get.checksum);
	
	
	g_debug ("version %s",(gchar*)evt.evt_param.ret_param.fw_info_get.version);
	g_debug ("current_version %s",current_version);
	g_debug ("current_checksum %s",current_checksum);
	
	fu_device_set_version (device, current_version);
	fu_device_add_checksum (device, current_checksum);
