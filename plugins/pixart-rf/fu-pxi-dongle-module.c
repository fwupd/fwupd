...
G_DEFINE_TYPE (FuPxiDongleModule, fu_pxi_dongle_module, FU_TYPE_DEVICE)

static gboolean
fu_pxi_dongle_module_set_feature (FuDevice *self, const guint8 *buf, guint bufsz, GError **error)
{
#ifdef HAVE_HIDRAW_H
...
static gboolean
fu_pxi_dongle_module_get_feature (FuDevice *self, guint8 *buf, guint bufsz, GError **error)
{
#ifdef HAVE_HIDRAW_H
...
static void
fu_pxi_dongle_module_to_string (FuDevice *device, guint idt, GString *str)
{
	FuPxiDongleModule *self = FU_PXI_DONGLE_MODULE (device);
...
fu_pxi_dongle_module_get_parent (FuDevice *self, GError **error)
{
...
static gboolean
fu_pxi_dongle_module_open (FuDevice *device, GError **error)
{
...
static gboolean
fu_pxi_dongle_module_close (FuDevice *device, GError **error)
{
	FuDevice *parent = fu_device_get_parent (device);
	if (parent == NULL) {
		g_set_error_literal (error,
...
static gboolean
fu_pxi_dongle_module_get_cmd_response (FuPxiDongleModule *device, guint8 *res, guint sz, GError **error) 
{
	
	FuPxiWirelessDevice *parent = fu_pxi_dongle_module_get_parent (FU_DEVICE (device) ,error);
	...
	{	
		...
		if (!fu_pxi_dongle_module_get_feature (FU_DEVICE (parent), res, sz, error))
			return FALSE;
...
static gboolean
fu_pxi_dongle_module_check_crc (FuDevice *device, guint16 checksum, GError **error)
{	
	g_autoptr(GByteArray) wireless_module_cmd = g_byte_array_new ();	
	g_autoptr(GByteArray) ota_cmd = g_byte_array_new ();
	FuPxiWirelessDevice *parent = fu_pxi_dongle_module_get_parent (device ,error);
	FuPxiDongleModule *self = FU_PXI_DONGLE_MODULE (device);
	...
	if (!fu_pxi_dongle_module_set_feature (FU_DEVICE (parent), wireless_module_cmd->data, wireless_module_cmd->len, error))
		return FALSE;

	if (!fu_pxi_dongle_module_get_cmd_response (self, res, sizeof(res), error))
		return FALSE;
...
static gboolean
fu_pxi_dongle_module_fw_object_create (FuDevice *device, FuChunk *chk, GError **error)
{
	g_autoptr(GByteArray) wireless_module_cmd = g_byte_array_new ();	
	g_autoptr(GByteArray) ota_cmd = g_byte_array_new ();
	FuPxiDongleModule *self = FU_PXI_DONGLE_MODULE (device);
...
static gboolean
fu_pxi_dongle_module_write_payload (FuDevice *device, FuChunk *chk, GError **error)
{
	g_autoptr(GByteArray) wireless_module_cmd = g_byte_array_new ();	
	g_autoptr(GByteArray) ota_cmd = g_byte_array_new ();
	FuPxiWirelessDevice *parent = fu_pxi_dongle_module_get_parent (device ,error);
...
static gboolean
fu_pxi_dongle_module_write_chunk (FuDevice *device, FuChunk *chk, GError **error)
{
	guint32 prn = 0;
	guint16 checksum;
	...
	if (!fu_pxi_dongle_module_fw_object_create (device, chk, error))
		return FALSE;
	...
	for (guint i = 0; i < chunks->len; i++) {
		...
		if (!fu_pxi_dongle_module_write_payload (device, chk2, error))
			return FALSE;
		prn++;
		...
		if (prn >= self->prn_threshold || i == chunks->len - 1) {
			if (!fu_pxi_dongle_module_check_crc (device, self->checksum, error))
				return FALSE;
			prn = 0;
		}
	}
...
static gboolean
fu_pxi_dongle_module_fw_ota_init_new (FuDevice *device, gsize bufsz, GError **error)
{
...
static gboolean
fu_pxi_dongle_module_fw_ota_ini_new_check (FuDevice *device, GError **error)
{
	...
	/* get pixart wireless module ota init new check command */
	if (!fu_pxi_common_composite_module_cmd (FU_PXI_DEVICE_CMD_FW_OTA_INIT_NEW_CHECK,
							...
							ota_cmd, error))
		return FALSE;
	if (!fu_pxi_dongle_module_set_feature (FU_DEVICE (parent), wireless_module_cmd->data, 
	...
	if(result != OTA_RSP_OK) {
		g_set_error (error,
			     ...
			     FWUPD_ERROR_READ,
			     "cmd rsp check fail: %s [0x%02x]",
			     fu_pxi_common_wireless_module_cmd_result_to_string (result),
			     result);
		return FALSE;
	}
...
static gboolean
fu_pxi_dongle_module_fw_upgrade (FuDevice *device, FuFirmware *firmware, GError **error)
{
	const gchar *version;
	const guint8 *buf;
	gsize bufsz = 0;
...
static gboolean
fu_pxi_dongle_module_reset (FuDevice *device, GError **error)
{
	g_autoptr(GByteArray) wireless_module_cmd = g_byte_array_new ();	
	g_autoptr(GByteArray) ota_cmd = g_byte_array_new ();	
	FuPxiWirelessDevice *parent = fu_pxi_dongle_module_get_parent (device ,error);
...
static FuFirmware *
fu_pxi_dongle_module_prepare_firmware (FuDevice *device,
				GBytes *fw,
...
static gboolean
fu_pxi_dongle_module_write_firmware (FuDevice *device,
				   FuFirmware *firmware,
				   FwupdInstallFlags flags,
				   GError **error)
{
	...
	if (!fu_pxi_dongle_module_fw_ota_init_new (device, g_bytes_get_size (fw), error))
		return FALSE;
	
	if (!fu_pxi_dongle_module_fw_ota_ini_new_check (device ,error))
		return FALSE;
	...
	for (guint i = self->offset; i < chunks->len; i++) {
		...
		if (!fu_pxi_dongle_module_write_chunk (device, chk, error))
			return FALSE;
		fu_device_set_progress_full (device, (gsize) i, (gsize) chunks->len);
	}
	/* fw upgrade command */
	if (!fu_pxi_dongle_module_fw_upgrade (device, firmware, error))
		return FALSE;
	/* delay for wireless module device read command */
	g_usleep (5 * 1000);
	/* send device reset command */
	return fu_pxi_dongle_module_reset (device, error);
}

static void
fu_pxi_dongle_module_init (FuPxiDongleModule *self)
{
...
static void
fu_pxi_dongle_module_class_init (FuPxiDongleModuleClass *klass)
{
	FuDeviceClass *klass_device = FU_DEVICE_CLASS (klass);
	klass_device->open = fu_pxi_dongle_module_open;
	klass_device->close = fu_pxi_dongle_module_close;
	klass_device->write_firmware = fu_pxi_dongle_module_write_firmware;
	klass_device->prepare_firmware = fu_pxi_dongle_module_prepare_firmware;
	klass_device->to_string = fu_pxi_dongle_module_to_string;
}

FuPxiDongleModule *
fu_pxi_dongle_module_new (struct ota_fw_dev_model *model)
{
	FuPxiDongleModule *self = NULL;
	self = g_object_new (FU_TYPE_PXI_DONGLE_MODULE, NULL);
