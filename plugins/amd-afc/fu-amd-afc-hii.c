			   ...
			   GError **error)
{
	...
	setting->kind = op[0] == FU_AMD_AFC_IFR_OPCODE_ONE_OF ? AFC_SETTING_ENUMERATION
							      : AFC_SETTING_INTEGER;
...
static void
fu_amd_afc_hii_scope_clear_bitfield(GArray *scopes, gboolean has_scope, gboolean *bitfield)
{
	*bitfield = FALSE;
...
static gboolean
fu_amd_afc_hii_parse_form_set(FuAmdAfcState *self, const guint8 *op, gsize opsz, GError **error)
{
	g_autoptr(FuStructAmdAfcIfrFormSet) st_form_set =
	    fu_struct_amd_afc_ifr_form_set_parse(op, opsz, 0, error);
	const gchar *name;

	if (st_form_set == NULL)
		return FALSE;
	name =
	    fu_amd_afc_hii_strings_get_string(self,
...
static gboolean
fu_amd_afc_hii_parse_ref(FuAmdAfcState *self,
			 guint form_idx,
			 const guint8 *op,
			 gsize opsz,
			 GError **error)
{
	g_autoptr(FuStructAmdAfcIfrRef) st_ref =
	    fu_struct_amd_afc_ifr_ref_parse(op, opsz, 0, error);
	FuAmdAfcForm *form = g_ptr_array_index(self->forms, form_idx);
	guint16 ref;
...
static gboolean
fu_amd_afc_hii_parse_guid(const guint8 *op, gsize opsz, gboolean *bitfield, GError **error)
{
	g_autoptr(FuStructAmdAfcIfrGuid) st_guid =
	    fu_struct_amd_afc_ifr_guid_parse(op, opsz, 0, error);
...
static gboolean
fu_amd_afc_hii_parse_question(FuAmdAfcState *self,
			      guint form_idx,
			      const guint8 *op,
			      gsize opsz,
			      gboolean has_scope,
			      ...
			      GError **error)
{
	...
	g_autoptr(FuStructAmdAfcIfrQuestion) st_question =
	    fu_struct_amd_afc_ifr_question_parse(op, opsz, 0, error);
	gsize width;
	guint16 prompt;
	...
	if (width <= sizeof(guint64) &&
	    opsz >= FU_STRUCT_AMD_AFC_IFR_QUESTION_SIZE + (IFR_NUMERIC_VALUE_COUNT * width)) {
		if (prompt == 0) {
			if (has_scope)
				*setting_idx = -1;
		} else {
	...
	if (question_bitfield)
		fu_amd_afc_hii_scope_clear_bitfield(scopes, has_scope, bitfield);
	return TRUE;
}

static gboolean
fu_amd_afc_hii_parse_option(FuAmdAfcState *self,
			    guint setting_idx,
			    const guint8 *op,
			    gsize opsz,
			    GError **error)
{
	g_autoptr(FuStructAmdAfcIfrOption) st_option =
	    fu_struct_amd_afc_ifr_option_parse(op, opsz, 0, error);
	...
	if (setting->kind != AFC_SETTING_ENUMERATION || width == 0 ||
	    opsz < FU_STRUCT_AMD_AFC_IFR_OPTION_SIZE + width)
		return TRUE;
...
static gboolean
fu_amd_afc_hii_parse_default(FuAmdAfcState *self,
			     guint setting_idx,
			     const guint8 *op,
			     gsize opsz,
			     GError **error)
{
	g_autoptr(FuStructAmdAfcIfrDefault) st_default =
	    fu_struct_amd_afc_ifr_default_parse(op, opsz, 0, error);
	FuAmdAfcSetting *setting = g_ptr_array_index(self->settings, setting_idx);
	guint8 value_type;
	gsize width;
	...
	if (fu_struct_amd_afc_ifr_default_get_id(st_default) == 0 && width > 0 &&
	    opsz >= FU_STRUCT_AMD_AFC_IFR_DEFAULT_SIZE + width) {
		if (!fu_amd_afc_hii_read_value(op,
					       opsz,
			   ...
			   GError **error)
{
	...
	st_hdr = fu_struct_amd_afc_hii_package_header_parse(buf, bufsz, offset, error);
	if (st_hdr == NULL)
		return FALSE;
	...
	if (package_type != HII_PACKAGE_FORMS ||
	    !fu_amd_afc_hii_bounds(bufsz, offset, package_length, NULL)) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
	...
	bufsz = offset + package_length; /* this package cannot be read past */
	...
	while (offset < bufsz) {
		...
		case FU_AMD_AFC_IFR_OPCODE_FORM_SET:
			if (length >= FU_STRUCT_AMD_AFC_IFR_FORM_SET_SIZE &&
			    !fu_amd_afc_hii_parse_form_set(self, op, length, error))
				return FALSE;
			break;
		...
		case FU_AMD_AFC_IFR_OPCODE_REF:
			if (form_idx >= 0 && length >= FU_STRUCT_AMD_AFC_IFR_REF_SIZE &&
			    !fu_amd_afc_hii_parse_ref(self, form_idx, op, length, error))
				return FALSE;
			break;
		case FU_AMD_AFC_IFR_OPCODE_GUID:
			if (length >= FU_STRUCT_AMD_AFC_IFR_GUID_SIZE &&
			    !fu_amd_afc_hii_parse_guid(op, length, &bitfield, error))
				return FALSE;
			break;
		...
		case FU_AMD_AFC_IFR_OPCODE_NUMERIC:
			if (form_idx >= 0 &&
			    length >=
				FU_STRUCT_AMD_AFC_IFR_QUESTION_SIZE + IFR_NUMERIC_VALUE_COUNT &&
			    !fu_amd_afc_hii_parse_question(self,
							   form_idx,
							   op,
							   length,
							   has_scope,
		...
		case FU_AMD_AFC_IFR_OPCODE_ONE_OF_OPTION:
			if (setting_idx >= 0 && length > FU_STRUCT_AMD_AFC_IFR_OPTION_SIZE &&
			    !fu_amd_afc_hii_parse_option(self, setting_idx, op, length, error))
				return FALSE;
			break;
		case FU_AMD_AFC_IFR_OPCODE_DEFAULT:
			if (setting_idx >= 0 && length > FU_STRUCT_AMD_AFC_IFR_DEFAULT_SIZE &&
			    !fu_amd_afc_hii_parse_default(self, setting_idx, op, length, error))
				return FALSE;
			break;
		default:
			break;
