...
fu_amd_afc_invalid_ucs2_func(void)
{
	...
	fu_test_array_u32(strings, 0, (0x04U << 24) | strings->len);
	fu_test_array_u32(strings, 4, 50);
	ret = fu_amd_afc_hii_strings_parse_package(state, strings->data, strings->len, 0, &error);
	g_assert_false(ret);
...
fu_amd_afc_short_strings_func(void)
{
	...
	g_assert_false(
	    fu_amd_afc_hii_strings_parse_package(state, strings, sizeof(strings), 0, &error));
...
fu_amd_afc_invalid_extended_string_func(void)
{
	...
	fu_test_array_u32(strings, 4, 50);
	g_assert_false(
	    fu_amd_afc_hii_strings_parse_package(state, strings->data, strings->len, 0, &error));
	g_assert_error(error, FWUPD_ERROR, FWUPD_ERROR_INVALID_DATA);
}
...
fu_amd_afc_copy_variable(FuContext *ctx_src, FuContext *ctx_dst, GError **error)
{
	...
	g_autoptr(GBytes) variable =
	    fu_efivars_get_data_bytes(fu_context_get_efivars(ctx_src),
