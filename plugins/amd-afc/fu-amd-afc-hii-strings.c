				   ...
				   GError **error)
{
	for (guint i = 0; i < count; i++) {
		...
		if (!fu_amd_afc_hii_strings_store(self,
						  *id,
						  g_string_free_and_steal(g_steal_pointer(&value)),
						  error))
			return FALSE;
		(*id)++;
		*offset += value_len + 1;
				     ...
				     GError **error)
{
	...
	if (bufsz - offset <= HII_STRING_LANGUAGE_OFFSET) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
	...
	if (header_size <= HII_STRING_LANGUAGE_OFFSET || header_size >= bufsz - offset) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
	...
	language = fu_memread_string_safe(buf,
					  offset + header_size,
					  offset + HII_STRING_LANGUAGE_OFFSET,
					  error);
	if (language == NULL)
		return FALSE;
	g_free(self->language);
	self->language = g_string_free_and_steal(g_steal_pointer(&language));
