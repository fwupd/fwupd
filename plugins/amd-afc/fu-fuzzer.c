...
fu_fuzzer_forms(FuAmdAfcState *state, const guint8 *data, gsize size)
{
	...
	if (!fu_amd_afc_hii_parse_forms(state, package->data, package->len, 0, &error))
		return;
...
fu_fuzzer_strings(FuAmdAfcState *state, const guint8 *data, gsize size)
{
	...
	(void)fu_amd_afc_hii_strings_parse_package(state, package->data, package->len, 0, NULL);
}
...
LLVMFuzzerTestOneInput(const guint8 *data, gsize size)
{
	...
	case 2:
		...
		break;
	case 3:
		(void)fu_amd_afc_hii_parse_varstores(state, data + 1, size - 1, 0, NULL);
		break;
	case 4:
