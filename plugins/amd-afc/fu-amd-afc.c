...
G_DEFINE_TYPE_EXTENDED(FuAmdAfcState,
		       fu_amd_afc_state,
		       G_TYPE_OBJECT,
		       0,
		       G_IMPLEMENT_INTERFACE(FU_TYPE_FUZZER, fu_amd_afc_state_fuzzer_iface_init))
...
fu_amd_afc_parse_blob(FuAmdAfcState *self, const guint8 *buf, gsize bufsz, GError **error)
{
	...
	if (!fu_amd_afc_hii_bounds(length,
...
fu_amd_afc_state_parse_table(FuAmdAfcState *self, GBytes *bytes, GError **error)
{
	...
	if (!fu_firmware_parse_bytes(FU_FIRMWARE(table),
				     ...
				     error))
		return FALSE;
	payload = fu_amd_afc_acpi_table_get_payload(table);
...
{
	FuAmdAfcState *self = g_object_new(FU_TYPE_AMD_AFC_STATE, NULL);
...
fu_amd_afc_state_init(FuAmdAfcState *self)
{
	...
	if (ctx == NULL) {
		self->ctx = g_object_new(
		    FU_TYPE_CONTEXT,
		    "flags",
		    (guint64)(FU_CONTEXT_FLAG_NO_QUIRKS | FU_CONTEXT_FLAG_DUMMY_EFIVARS),
		    NULL);
		self->efivars = g_object_ref(fu_context_get_efivars(self->ctx));
	}
}
...
fu_amd_afc_state_add_bios_settings(FuAmdAfcState *self, GError **error)
{
	for (guint i = 0; i < self->settings->len; i++) {
		...
		g_autofree gchar *path = fu_amd_afc_setting_name(setting);
		g_autofree gchar *name_stripped = NULL;
		g_autofree gchar *parent = NULL;
		g_autofree gchar *id = NULL;
		g_autoptr(FuAmdAfcBiosSetting) attr = fu_amd_afc_bios_setting_new(self, i);
		const gchar *name = path;
		const gchar *separator = strrchr(path, '|');
		if (separator != NULL) {
			name = separator + 1;
		...
		if (!fwupd_bios_setting_setup(FWUPD_BIOS_SETTING(attr), error))
			return FALSE;
		if (!fu_context_add_bios_setting(self->ctx, FU_BIOS_SETTING(attr), error))
			return FALSE;
	}
	return TRUE;
}
