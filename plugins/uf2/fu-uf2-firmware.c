...
fu_uf2_firmware_parse_chunk(FuUf2Firmware *self, FuChunk *chk, GByteArray *tmp, GError **error)
{
	...
	/* check magic */
	if (!fu_struct_unpack_from("<LLLLLLLL",
				   error,
				   buf,
				   bufsz,
				   NULL, /* offset */
				   ...
				   &family_id))
		return FALSE;
	if (magic0 != FU_UF2_FIRMWARE_MAGIC_START0) {
		g_set_error(error,
			    G_IO_ERROR,
	...
	if (magic1 != FU_UF2_FIRMWARE_MAGIC_START1) {
		g_set_error(error,
			    G_IO_ERROR,
			    G_IO_ERROR_INVALID_DATA,
	...
	if (flags & FU_UF2_FIRMWARE_BLOCK_FLAG_HAS_EXTENSION_TAG) {
		...
		while (offset < bufsz) {
			...
			if (!fu_struct_unpack_from("<BT", error, buf, bufsz, &offset, &sz, &tag))
				return FALSE;
			if (sz < 4) {
				g_set_error_literal(error,
						    G_IO_ERROR,
	...
	if (!fu_memread_uint32_safe(buf, bufsz, 0x1FC, &magic0, G_LITTLE_ENDIAN, error))
		return FALSE;
	if (magic0 != FU_UF2_FIRMWARE_MAGIC_END) {
		g_set_error(error,
			    G_IO_ERROR,
			    G_IO_ERROR_INVALID_DATA,
