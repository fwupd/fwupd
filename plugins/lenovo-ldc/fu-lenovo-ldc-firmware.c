			     ...
			     GError **error)
{
	...
	st = fu_struct_lenovo_ldc_usage_parse_stream(stream_usage, offset, error);
	if (st == NULL)
		return FALSE;
	self->pid = fu_struct_lenovo_ldc_usage_get_pid(st);
	fu_firmware_set_version_raw(firmware, fu_struct_lenovo_ldc_usage_get_composite_version(st));
	...
	for (guint i = 0; i < usage_items; i++) {
		...
		guint32 target_size;
		guint32 max_size;
		FuLenovoLdcFlashId component_id;
		g_autoptr(FuStructLenovoLdcUsageItem) st_item = NULL;
		g_autoptr(GInputStream) stream_partial = NULL;
		g_autoptr(FuFirmware) img = fu_firmware_new();

		st_item = fu_struct_lenovo_ldc_usage_item_parse_stream(stream_usage, offset, error);
		if (st_item == NULL)
			return FALSE;
		component_id = fu_struct_lenovo_ldc_usage_item_get_component_id(st_item);
		physical_addr = fu_struct_lenovo_ldc_usage_item_get_physical_address(st_item);
		target_size = fu_struct_lenovo_ldc_usage_item_get_target_size(st_item);
		max_size = fu_struct_lenovo_ldc_usage_item_get_max_size(st_item);
		fu_firmware_set_idx(img, component_id);
		if (component_id <= FU_LENOVO_LDC_FLASH_ID_DBG)
			fu_firmware_set_id(img, fu_lenovo_ldc_flash_id_to_string(component_id));
		fu_firmware_set_version_raw(
		    img,
		    fu_struct_lenovo_ldc_usage_item_get_target_version(st_item));
		...
		if (target_size > 0) {
			...
			guint32 crc_provided =
			    fu_struct_lenovo_ldc_usage_item_get_target_crc32(st_item);
...
{
	FuLenovoLdcFirmware *self = FU_LENOVO_LDC_FIRMWARE(firmware);
	g_autoptr(FuStructLenovoLdcUsage) st = fu_struct_lenovo_ldc_usage_new();

	/* header first */
	fu_struct_lenovo_ldc_usage_set_pid(st, self->pid);
...
static gchar *
fu_lenovo_ldc_firmware_convert_version(FuFirmware *firmware, guint64 version_raw)
{
...
static void
fu_lenovo_ldc_firmware_finalize(GObject *object)
{
	//	FuLenovoLdcFirmware *self = FU_LENOVO_LDC_FIRMWARE(object);
	G_OBJECT_CLASS(fu_lenovo_ldc_firmware_parent_class)->finalize(object);
}
...
fu_lenovo_ldc_firmware_class_init(FuLenovoLdcFirmwareClass *klass)
{
	...
	object_class->finalize = fu_lenovo_ldc_firmware_finalize;
	...
	firmware_class->convert_version = fu_lenovo_ldc_firmware_convert_version;
}
