...
static gboolean
fu_jabra_gnp_child_device_probe(FuDevice *device, GError **error)
{
	return TRUE;
}
...
fu_jabra_gnp_child_device_tx_cb(FuDevice *device, gpointer user_data, GError **error)
{
	...
	parent = FU_JABRA_GNP_DEVICE(fu_device_get_parent(device));
...
fu_jabra_gnp_child_device_read_version(FuJabraGnpChildDevice *self, GError **error)
{
	 ...
	 * remove it */
	if (strlen(version) > 0 && version[strlen(version) - 1] == '.')
					 ...
					 GError **error)
{
	...
	/* write squif */
	if (!fu_jabra_gnp_child_device_write_dfu_from_squif(self, error))
		return FALSE;
...
fu_jabra_gnp_child_device_class_init(FuJabraGnpChildDeviceClass *klass)
{
	...
	device_class->probe = fu_jabra_gnp_child_device_probe;
