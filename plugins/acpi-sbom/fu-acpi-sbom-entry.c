			 ...
			 GError **error)
{
	...
	if (payloadsz > fu_firmware_get_size_max(firmware)) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
			    "payload size 0x%x exceeds max 0x%zx",
			    payloadsz,
	...
	if (self->compression == FU_USWID_PAYLOAD_COMPRESSION_ZLIB) {
		...
		if (istream2 == NULL)
			return FALSE;
		blob_uncompressed =
		    fu_input_stream_read_bytes(istream2, 0, payloadsz_max + 1, NULL, error);
	...
	/* payload */
	if (g_bytes_get_size(blob_uncompressed) > payloadsz_max) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
			    "uncompressed payload exceeds max 0x%zx",
			    payloadsz_max);
		return FALSE;
	}
