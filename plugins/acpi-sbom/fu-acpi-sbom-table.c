	...
	while (offset < streamsz) {
		...
		if (fu_firmware_get_size(entry) == 0) {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "SBOM entry had zero size @0x%zx",
				    offset);
			return FALSE;
		}
...
fu_acpi_sbom_table_class_init(FuAcpiSbomTableClass *klass)
{
	...
	fu_firmware_add_image_gtype(firmware_class, FU_TYPE_ACPI_SBOM_ENTRY);
	fu_firmware_set_images_max(firmware_class, 5000);
	fu_firmware_set_size_max(firmware_class, 5 * FU_MB);
}
