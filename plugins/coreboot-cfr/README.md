...
This plugin reads and writes EFI variables using the coreboot CFR GUID.
For `APM_CNT` settings it also opens `/dev/port`, writes the firmware-provided runtime apply ID to
the APM status port, and writes the apply command to the APM control port to trigger the SMI handler.
