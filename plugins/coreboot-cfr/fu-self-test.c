...
fu_coreboot_cfr_test_build_metadata(void)
{
	...
	const FuCorebootCfrEnumTestValue power_profile[] = {
	    {0, "Power Saver"},
	    {1, "Balanced"},
	    {2, "Performance"},
	};
	...
	fu_byte_array_append_uint32(buf, 0x0, G_LITTLE_ENDIAN);
	fu_byte_array_append_uint32(buf, 0x4, G_LITTLE_ENDIAN);
	...
	fu_coreboot_cfr_test_append_option(buf,
					   ...
					   "power_profile",
					   "Power Profile",
					   "Choose maximum battery life or performance",
...
fu_coreboot_cfr_coldplug_func(void)
{
	...
	g_autofree gchar *pending_reboot_dir = NULL;
	g_autofree gchar *pending_reboot_path = NULL;
	...
	pending_reboot_dir = g_dir_make_tmp("fwupd-coreboot-cfr-XXXXXX", &error);
	g_assert_no_error(error);
	g_assert_nonnull(pending_reboot_dir);
	pending_reboot_path = g_build_filename(pending_reboot_dir, "pending_reboot", NULL);
	g_setenv("FWUPD_COREBOOT_CFR_PENDING_REBOOT_PATH", pending_reboot_path, TRUE);
	...
	g_assert_no_error(error);
	g_assert_true(fu_coreboot_cfr_test_set_uint32(efivars, "power_profile", 0x1, &error));
	g_assert_no_error(error);
	...
	setting = fu_bios_settings_get_attr(bios_settings, "org.coreboot.cfr.power_profile");
	g_assert_nonnull(setting);
	g_assert_cmpstr(fwupd_bios_setting_get_current_value(setting), ==, "Balanced");

	setting = fu_bios_settings_get_attr(bios_settings, "org.coreboot.cfr.pending_reboot");
	g_assert_nonnull(setting);
	g_assert_cmpstr(fwupd_bios_setting_get_name(setting),
			==,
			FWUPD_BIOS_SETTING_PENDING_REBOOT);
	g_assert_cmpstr(fwupd_bios_setting_get_current_value(setting), ==, "0");

	/* nocheck:blocked
	 * test owns this process-local override */
	g_unsetenv("FWUPD_COREBOOT_CFR_PENDING_REBOOT_PATH");
	g_assert_cmpint(g_unlink(pending_reboot_path), ==, 0);
	g_assert_cmpint(g_rmdir(pending_reboot_dir), ==, 0);
}
...
fu_coreboot_cfr_write_value_func(void)
{
	...
	g_autofree gchar *pending_reboot_dir = NULL;
	g_autofree gchar *pending_reboot_path = NULL;
	g_autofree gchar *pending_reboot_value = NULL;
	...
	g_assert_nonnull(pending_reboot_dir);
	pending_reboot_path = g_build_filename(pending_reboot_dir, "pending_reboot", NULL);
	g_assert_true(g_file_set_contents(pending_reboot_path, "0", -1, &error));
	g_assert_no_error(error);
	...
	g_assert_true(
	    g_file_get_contents(pending_reboot_path, &pending_reboot_value, NULL, &error));
	g_assert_no_error(error);
	g_assert_cmpstr(pending_reboot_value, ==, "1");
	g_assert_cmpint(g_unlink(pending_reboot_path), ==, 0);
	g_assert_cmpint(g_rmdir(pending_reboot_dir), ==, 0);
}
