				 ...
				 GError **error)
{
	...
	if (!fu_size_checked_inc(&offset, FU_STRUCT_FPC_FF2_HDR_SIZE, error)) {
		g_prefix_error_literal(error, "FPC FF2 header offset overflow: ");
		return FALSE;
	}
	...
	for (guint i = 0; i < blocks_num; i++) {
		...
		if (!fu_size_checked_inc(&offset, st_blkhdr->buf->len, error)) {
			g_prefix_error(error, "FPC FF2 block %u header offset overflow: ", i);
			return FALSE;
		}
		...
		if (st_blksec->buf->len > G_MAXUINT16 - payload_len) {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_FILE,
				    "FPC FF2 block %u payload length overflow",
				    i);
			return FALSE;
		}
