				      ...
				      GError **error)
{
	...
	g_debug("device preparing firmware...");
...
fu_kinetic_dp_device_rescan(FuDevice *device, GError **error)
{
	...
	/* TODO set max firmware size base upon family if we need it */
	switch (self->family) {
	...
	fu_device_add_instance_str(FU_DEVICE(self), "VID", "Kinetic");
	fu_device_add_instance_str(FU_DEVICE(self),
				   "FAM",
				   fu_kinetic_dp_family_to_string(self->family));
	fu_device_add_instance_str(FU_DEVICE(self), "CID", chip_id);
	if (!fu_device_build_instance_id(FU_DEVICE(self), error, "MST", "VEN", "FAM", NULL))
		return FALSE;
	if (!fu_device_build_instance_id(FU_DEVICE(self), error, "MST", "VEN", "FAM", "CID", NULL))
		return FALSE;
