...
#define DPCD_ADDR_CUSTOMER_FW_MIN_REV 0x00514
#define DPCD_SIZE_CUSTOMER_FW_MIN_REV 1
#define DPCD_ADDR_CUSTOMER_PROJ_ID    0x00515
#define DPCD_SIZE_CUSTOMER_PROJ_ID    1
#define DPCD_ADDR_PRODUCT_TYPE	      0x00516
#define DPCD_SIZE_PRODUCT_TYPE	      1
...
/* init value for Kinetic's proprietary CRC-16 calculation*/
#define CRC_INIT_KT_PROP_CRC16 0x1021
/* polynomial for Kinetic's proprietary CRC-16 calculation*/
#define CRC_POLY_KT_PROP_CRC16 0x1021
...
fu_kinetic_dp_secure_aux_isp_read_param_reg(FuKineticDpConnection *self,
					    ...
					    GError **error)
{
	if (!fu_kinetic_dp_connection_read(self, DPCD_ADDR_PARAM_REG, dpcd_val, 1, error)) {
	...
	*status = KT_DPCD_CMD_STS_NONE;
	...
	while (max_time_ms != 0) {
		...
		if (dpcd_val != (cmd_id | DPCD_KT_CONFIRMATION_BIT)) {
			...
			/* dpcd_val == cmd_id */
			/* confirmation bit is cleared by sink, means sent command is
			 * processed */
			return TRUE;
		}
						       ...
						       GError **error)
{
	...
	res = fu_kinetic_dp_connection_write(self, DPCD_ADDR_ISP_REPLY_DATA_REG, buf, len, error);
	if (!res) {
		/* clear reply data length to 0 if failed to write reply data */
		g_prefix_error(error, "failed to write DPCD_KT_REPLY_DATA_REG: ");
		len = 0;
	}
	// FIXME: the GError handling here is funky
					  ...
					  GError **error)
{
	...
	// FIXME: this needs to use FuChunk
					     ...
					     GError **error)
{
	...
	if (!fu_kinetic_dp_secure_aux_isp_wait_dpcd_cmd_cleared(connection,
								...
								&status,
								error)) {
		if (status == KT_DPCD_STS_INVALID_IMAGE)
	...
	if (!fu_memread_uint16_safe(reply_data,
				    sizeof(reply_data),
				    0,
				    &priv->flash_id,
				    G_BIG_ENDIAN,
					  ...
					  GError **error)
{
	...
	g_debug("secure isp driver is running and erasing flash...");
	return TRUE;
}
						   ...
						   GError **error)
{
	...
	if (!fu_memwrite_uint32_safe(pl_size_data,
				     sizeof(pl_size_data),
				     0,
	...
	if (!fu_memwrite_uint16_safe(pl_size_data,
				     sizeof(pl_size_data),
				     8,
					       ...
					       GError **error)
{
	...
	while (wait_count-- > 0) {
		...
		if (status != (cmd_id | DPCD_KT_CONFIRMATION_BIT)) {
			...
			if (status == cmd_id) {
				g_debug("programming firmware done");
				return TRUE;
			}
			g_set_error_literal(error,
					    FWUPD_ERROR,
...
fu_kinetic_dp_secure_aux_isp_disable_aux_forward(FuKineticDpConnection *connection, GError **error)
{
	...
	cmd_id = KT_DPCD_CMD_STS_NONE;
	// FIXME: GError handling is wrong
					     ...
					     GError **error)
{
	...
	/* DPCD 0x509 hw_ver */
	dev_info->chip_rev = dpcd_buf[0];
	/* DPCD 0x50A ~ 0x50C fw_ver */
	dev_info->fw_info.std_fw_ver =
	    (guint32)dpcd_buf[1] << 16 | (guint32)dpcd_buf[2] << 8 | dpcd_buf[3];
	/* DPCD 0x515  customer_proj_id */
	dev_info->fw_info.customer_project_id = dpcd_buf[12];
	/* DPCD (0x50F | 0x514) customer_fw_ver*/
	dev_info->fw_info.customer_fw_ver = (guint16)dpcd_buf[6] << 8 | (guint16)dpcd_buf[11];
	/* DPCD 0x516 */
	dev_info->chip_type = dpcd_buf[13];
				   ...
				   GError **error)
{
	...
	g_debug("start secure AUX-ISP for [%s]...",
