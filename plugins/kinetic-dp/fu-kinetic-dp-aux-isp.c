...
static const KtDpChipBrIdStrTable kt_dp_branch_dev_info_table[] = {
    ...
    {KT_CHIP_MUSTANG_5200, KT_FW_STATE_RUN_APP, {'K', 'T', '5', '2', '0', '0'}, 6},
    /* Puma MC29x0 */
    {KT_CHIP_PUMA_2900, KT_FW_STATE_RUN_IROM, {'P', 'U', 'M', 'A'}, 4},
    {KT_CHIP_PUMA_2900, KT_FW_STATE_RUN_APP, {'M', 'C', '2', '9', '0'}, 5},
    {KT_CHIP_PUMA_2900, KT_FW_STATE_RUN_APP, {'M', 'C', '2', '9', '1', '0'}, 6}};
							...
							GError **error)
{
	...
	/* there might not always be null-terminated character '\0' in
	 *   DPCD branch ID string  (when length is 6) */
					 ...
					 GError **error)
{
	...
	if (root_dev_chip_id == KT_CHIP_JAGUAR_5000 || root_dev_chip_id == KT_CHIP_MUSTANG_5200) {
		...
		/* Wait 10ms for host to process AUX forwarding command */
		g_usleep(10 * 1000);
		return TRUE;
	}
					     ...
					     GError **error)
{
	...
	/* basic chip info (chip ID, F/W state) is obtained from DPCD branch device ID string */
