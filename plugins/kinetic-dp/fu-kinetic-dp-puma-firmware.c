					 ...
					 GError **error)
{
	...
	if (FU_STRUCT_KINETIC_DP_PUMA_HEADER_SIZE > G_MAXUINT32 - code_size) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
				    "Puma firmware code size overflow");
		return FALSE;
	}
	...
	for (guint i = 0; i < FU_STRUCT_KINETIC_DP_PUMA_HEADER_DEFAULT_OBJECT_COUNT; i++) {
		...
		if (FU_STRUCT_KINETIC_DP_PUMA_HEADER_INFO_SIZE > G_MAXUINT32 - obj_length) {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
				    "Puma firmware object %u length overflow",
				    i);
			return FALSE;
		}
		...
		if (obj_total > G_MAXUINT32 - code_size) {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_DATA,
				    "Puma firmware code size overflow at object %u",
				    i);
			return FALSE;
		}
		code_size += obj_total;
		...
		if (!fu_size_checked_inc(&offset, st_obj->buf->len, error)) {
			g_prefix_error(error, "Puma firmware object %u offset overflow: ", i);
			return FALSE;
		}
	}
