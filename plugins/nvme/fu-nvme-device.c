...
G_DEFINE_TYPE (FuNvmeDevice, fu_nvme_device, FU_TYPE_DEVICE)
...
static guint
fu_nvme_device_pci_slot_depth (GUdevDevice *device)
{
	g_autoptr(GUdevDevice) device_tmp = NULL;
	device_tmp = g_udev_device_get_parent_with_subsystem (device, "pci", NULL);
	if (device_tmp == NULL)
		return 0;
...
fu_nvme_device_open (FuDevice *device, GError **error)
{
	...
	/* open device */
	self->fd = g_open (fu_device_get_platform_id (FU_DEVICE (self)), O_RDONLY);
	if (self->fd < 0) {
		g_set_error (error,
			     G_IO_ERROR,
			     G_IO_ERROR_FAILED,
	...
	self->pci_depth = fu_nvme_device_pci_slot_depth (self->udev_device);
	if (self->pci_depth <= 2)
	...
	/* vendor is optional */
	vendor = g_udev_device_get_property (self->udev_device, "ID_VENDOR_FROM_DATABASE");
	if (vendor != NULL)
...
{
	FuNvmeDevice *self;
	self = g_object_new (FU_TYPE_NVME_DEVICE, NULL);
