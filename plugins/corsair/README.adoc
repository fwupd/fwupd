...
All devices handled by one object (FuCorsairDevice). Receivers with
wireless-only devices will be shown as two entities: parent device as a
receiver and wireless device as a child. Difference in behavior is
handled by private flags.

== Wired mice update behavior

Mice and/or it’s wireless adapter must be connected to host via USB
...
The receiver should be connected to host and the mouse should be turned
on and not sleeping.
...
Additionally, an extra instance ID is used for the wireless subdevice,
e.g.
