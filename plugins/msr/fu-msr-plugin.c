...
fu_msr_plugin_add_security_attr_intel_gds(FuPlugin *plugin, FuSecurityAttrs *attrs)
{
	...
	if (fu_cpu_device_needs_mitigation(FU_CPU_DEVICE(device), FU_CPU_MITIGATION_FLAG_GDS))
		return;
...
fu_msr_plugin_add_security_attr_amd_hwcr(FuPlugin *plugin, FuSecurityAttrs *attrs)
{
	...
	if (device != NULL && fu_cpu_device_needs_mitigation(FU_CPU_DEVICE(device),
							     FU_CPU_MITIGATION_FLAG_SINKCLOSE)) {
		guint32 min = fu_cpu_device_get_sinkclose_microcode_ver(FU_CPU_DEVICE(device));
