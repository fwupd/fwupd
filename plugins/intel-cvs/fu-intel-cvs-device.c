...
fu_intel_cvs_device_vid_notify_cb(FuDevice *device, GParamSpec *pspec, gpointer user_data)
{
	...
	/* the CVS vendor ID is allocated by Intel for each CV chip vendor and is nothing to do
	 * with the USB or PCI VID, even if they seem to match... */
	fu_device_build_vendor_id_u16(device, "CVS", fu_device_get_vid(device));
}
