					       ...
					       GError **error)
{
	if (!fu_size_checked_inc(&info->offset, 4, error)) {
		g_prefix_error_literal(error, "WTA file header magic offset overflow: ");
		return FALSE;
	}
	...
	if (header->header_size < 8) {
		g_set_error_literal(error,
				    ...
				    "WTA header size too small");
		return FALSE;
	}
	if (!fu_size_checked_inc(&info->offset, header->header_size - 8, error)) {
		g_prefix_error_literal(error, "WTA file header size offset overflow: ");
		return FALSE;
	}
	...
	if (!fu_size_checked_inc(&info->offset, 16, error)) {
		g_prefix_error_literal(error, "WTA file header padding offset overflow: ");
		return FALSE;
	}
	return TRUE;
}
						 ...
						 GError **error)
{
	...
	if (!fu_size_checked_inc(&info->offset, header->file_name_length + 8, error)) {
		g_prefix_error_literal(error, "WTA record filename offset overflow: ");
		return FALSE;
	}
