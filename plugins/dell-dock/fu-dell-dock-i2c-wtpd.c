...
struct _FuDellDockWtpd {
	FuDevice parent_instance;
};

G_DEFINE_TYPE(FuDellDockWtpd, fu_dell_dock_wtpd, FU_TYPE_DEVICE)

static gchar *
fu_dell_dock_wtpd_ver_string(guint32 wtpd_version)
{
	/* guint32 BCD */
	return g_strdup_printf("%02x.%02x.%02x.%02x",
			       (wtpd_version >> 0) & 0xff,
			       (wtpd_version >> 8) & 0xff,
			       (wtpd_version >> 16) & 0xff,
			       (wtpd_version >> 24) & 0xff);
}

static gboolean
fu_dell_dock_wtpd_setup(FuDevice *device, GError **error)
{
	...
	guint8 devtype = EC_V2_DOCK_DEVICE_TYPE_WTPD;
	...
	/* IDs */
	logical_id = g_strdup_printf("WTPD\\DOCKTYPE_%02x&DOCKSKU_%02x", dock_type, dock_sku);
	...
	/* version */
	wtpd_version = fu_dell_dock_ec_v2_get_wtpd_version(proxy);
	dynamic_version = fu_dell_dock_wtpd_ver_string(wtpd_version);
...
static gboolean
fu_dell_dock_wtpd_write(FuDevice *device,
			FuFirmware *firmware,
			FuProgress *progress,
			...
			GError **error)
{
	...
	g_info("writing weltrend pd firmware version %s", dynamic_version);

	/* construct writing buffer */
	fw_whdr = fu_dell_dock_hid_v2_fwup_pkg_new(fw, EC_V2_DOCK_DEVICE_TYPE_WTPD, 0);
...
static gboolean
fu_dell_dock_wtpd_open(FuDevice *device, GError **error)
{
	return fu_device_open(fu_device_get_proxy(device), error);
}

static gboolean
fu_dell_dock_wtpd_close(FuDevice *device, GError **error)
{
	return fu_device_close(fu_device_get_proxy(device), error);
}

static void
fu_dell_dock_wtpd_finalize(GObject *object)
{
	G_OBJECT_CLASS(fu_dell_dock_wtpd_parent_class)->finalize(object);
}

static void
fu_dell_dock_wtpd_set_progress(FuDevice *self, FuProgress *progress)
{
...
static void
fu_dell_dock_wtpd_init(FuDellDockWtpd *self)
{
...
static void
fu_dell_dock_wtpd_class_init(FuDellDockWtpdClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuDeviceClass *device_class = FU_DEVICE_CLASS(klass);
	object_class->finalize = fu_dell_dock_wtpd_finalize;
	device_class->write_firmware = fu_dell_dock_wtpd_write;
	device_class->open = fu_dell_dock_wtpd_open;
	device_class->setup = fu_dell_dock_wtpd_setup;
	device_class->close = fu_dell_dock_wtpd_close;
	device_class->set_progress = fu_dell_dock_wtpd_set_progress;
}

FuDellDockWtpd *
fu_dell_dock_wtpd_new(FuDevice *proxy)
{
	FuContext *ctx = fu_device_get_context(proxy);
	FuDellDockWtpd *self = NULL;
	self = g_object_new(FU_TYPE_DELL_DOCK_WTPD, "context", ctx, NULL);
