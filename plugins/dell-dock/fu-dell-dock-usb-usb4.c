...
/* write a uint32_t to a little endian ordered uint8_t array */
static void
fu_dell_dock_usb4_write_u32to_buf (guint32 val, guint8 *buf)
{
	buf[0] = (val >> 0) & 0xff;
	buf[1] = (val >> 8) & 0xff;
	buf[2] = (val >> 16) & 0xff;
			        ...
			        guint32 nvm_addr, GError **error)
{
	...
	while (length > 0) {
		...
		fu_dell_dock_usb4_write_u32to_buf (NVM_OFFSET_TO_METADATA(nvm_addr),
						   metadata);
				 ...
				 GError **error)
{
	...
	while (length > 0)  {
		...
		if (!fu_dell_dock_usb4_mbox_data_write (device, buf, 64,
							error)) {
			g_prefix_error (error,
					"hub mbox data write error:");
			return FALSE;
		}
		...
		buf += 64;
		length -= 64;
		fu_device_set_progress_full (device, bytes_done += 64,
					     bytes_total);
	}
			    ...
			    FwupdInstallFlags flags, GError **error)
{
	...
	guchar    nvm_buf[NVM_READ_LENGTH] = {0};
	...
	nvm_vendor_id  = g_strdup_printf ("%02x%02x", nvm_buf[NVM_VID_OFFSET_MAJOR],
				                      nvm_buf[NVM_VID_OFFSET_MINOR]);
	nvm_product_id = g_strdup_printf ("%02x%02x", nvm_buf[NVM_PID_OFFSET_MAJOR],
	...
	if (!fu_dell_dock_usb4_hub_nvm_write(device, fw_buf, fw_blob_size, 0,
					     error))
		return FALSE;
...
fu_dell_dock_usb4_setup (FuDevice *device, GError **error)
{
	...
	g_autofree gchar *nvm_vendor_id  = NULL;
	g_autofree gchar *nvm_product_id = NULL;
	guchar buf[NVM_READ_LENGTH] = {0};

	if (!fu_dell_dock_usb4_hub_nvm_read (device, buf, NVM_READ_LENGTH, 0, error)) {
		g_prefix_error (error, "NVM READ error:");
		return FALSE;
	}
	nvm_version    = g_strdup_printf ("%02x.%02x", buf[NVM_VER_OFFSET_MAJOR],
				                       buf[NVM_VER_OFFSET_MINOR]),
	nvm_vendor_id  = g_strdup_printf ("%02x%02x" , buf[NVM_VID_OFFSET_MAJOR],
				                       buf[NVM_VID_OFFSET_MINOR]);
	nvm_product_id = g_strdup_printf ("%02x%02x" , buf[NVM_PID_OFFSET_MAJOR],
