...
static guint16
fu_dell_dock_mst_get_crc(guint32 length, const guint8 *payload_data)
{
	/* nocheck:magic */
	...
	for (guint32 byte = 0; byte < length; ++byte) {
		val = (guint8)(payload_data[byte] ^ (crc >> 8));
			       ...
			       GError **error)
{
	...
	if (attribs->checksum_cmd == FU_DELL_DOCK_MST_CMD_CRC16_CHECKSUM)
		payload_sum = fu_dell_dock_mst_get_crc(attribs->length + attribs->start, data);
	else {
