...
static MSTType
fu_dell_dock_mst_check_type(FuDevice *device, GError **error)
{
        ...
        for (guint i = 0; i < instance_ids->len; i++){
                ...
                if (g_strcmp0(tmp, (gchar *)(DELL_DOCK_VMM6210_INSTANCE_ID)) == 0)
                        return Cayenne_mst;
                else if (g_strcmp0(tmp, (gchar *)(DELL_DOCK_VM5331_INSTANCE_ID)) == 0)
                        return Panamera_mst;
        }
        return Unknow;
}
...
static guint16
fu_dell_dock_mst_get_crc(guint16 crc,
                                guint8 type,
                                guint32 length,
			       ...
			       GError **error)
{
	...
	if (attribs->checksum_cmd == MST_CMD_CRC16_CHECKSUM)
		payload_sum = fu_dell_dock_mst_get_crc (0, 16, (attribs->length + attribs->start), data);
	else{
...
static gboolean
fu_dell_dock_mst_erase_panamera_bank(FuDevice *proxy, MSTBank bank, GError **error)
{
	const MSTBankAttributes *attribs = NULL;
	guint32 sector;
...
static gboolean
fu_dell_dock_mst_erase_cayenne (FuDevice *proxy, GError **error)
{
...
fu_dell_dock_mst_stop_esm(FuDevice *proxy, GError **error)
{
	...
	if (!fu_dell_dock_mst_read_register(proxy, PANAMERA_MST_RC_DATA_ADDR, length, &quad_bytes, error))
		return FALSE;
	...
	if (!fu_dell_dock_mst_read_register(proxy, PANAMERA_MST_RC_DATA_ADDR, length, &hdcp_bytes, error))
		return FALSE;
			    ...
			    GError **error)
{
	...
	for (guint i = 0; i < retries; i++) {
		...
		if (!fu_dell_dock_mst_erase_panamera_bank(fu_device_get_proxy(device), bank, error))
			return FALSE;
                                ...
                                GError **error)
{
        ...
        for (guint i = 0; i < retries; i++) {
                if (!fu_dell_dock_mst_erase_cayenne (fu_device_get_proxy (device),
                                                     error))
                        return FALSE;
                fu_progress_step_done(progress_local);
                if (!fu_dell_dock_write_flash_bank (device,
                ...
                g_debug ("MST: Bank successfully flashed");
                break;
        }
        /* failed after all our retries */
        if (!checksum) {
        ...
        if (!fu_dell_dock_mst_rc_command (fu_device_get_proxy (device),
                                          MST_CMD_ACTIVATE_FW,
                                          g_bytes_get_size (fw), 0x0,
                                          NULL,
                                          error)) {
                g_prefix_error (error, "Failed to activate FW: ");
                return FALSE;
        }
        g_debug("MST active successfully");
			  ...
			  GError **error)
{
	...
	type = fu_dell_dock_mst_check_type(device, error);
...
fu_dell_dock_mst_probe(FuDevice *device, GError **error)
{
	...
	switch (type){
	case Cayenne_mst:               
		mst_rc_trigger_addr = CAYENNE_MST_RC_TRIGGER_ADDR;
		mst_rc_command_addr = CAYENNE_MST_RC_COMMAND_ADDR;
		mst_rc_data_addr = CAYENNE_MST_RC_DATA_ADDR;
		mst_core_mcu_bootloader_addr = CAYENNE_MST_CORE_MCU_BOOTLOADER_STS;
                break;
	case Panamera_mst:
		mst_rc_trigger_addr = PANAMERA_MST_RC_TRIGGER_ADDR; 
		mst_rc_command_addr = PANAMERA_MST_RC_COMMAND_ADDR;
		mst_rc_data_addr = PANAMERA_MST_RC_DATA_ADDR;
		mst_core_mcu_bootloader_addr = PANAMERA_MST_CORE_MCU_BOOTLOADER_STS;
                break;
	case Unknow:
	default:
		return FALSE;
