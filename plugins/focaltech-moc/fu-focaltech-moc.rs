...
//   - MAGIC: FuFocaltechMocMagic (Sho / Stx / Eot)
...
#[repr(u8)]
enum FuFocaltechMocCmd {
    GetFwVersion = 0x30,  // request firmware version string
    SetBootMode  = 0x32,  // switch device mode; data[0] = FuFocaltechMocBootMode
...
#[derive(New)]
#[repr(C, packed)]
struct FuStructFocaltechMocCmdReq {
...
#[derive(Parse)]
#[repr(C, packed)]
struct FuStructFocaltechMocCmdRsp {
    head: u8,
    ln: u16be,
    cmd: FuFocaltechMocCmd,  // always FU_FOCALTECH_MOC_CMD_ACK (0x04) for success
}
...
#[derive(New)]
#[repr(C, packed)]
struct FuStructFocaltechMocDlHdr {
    head: u8,                  // MSG_MAGIC = 0x02
    ln: u16be,                 // big-endian length field
    cmd: FuFocaltechMocCmd,    // FU_FOCALTECH_MOC_CMD_FW_DOWNLOAD (0x33)
