...
fu_realtek_billboard_device_setup(FuDevice *device, GError **error)
{
	...
	/* query the dual-bank geometry now so an update is never offered when
	 * the user bank cannot be written, e.g. the device only responds on a
	 * USB-C port that exposes the Billboard interface */
	if (!fu_realtek_billboard_device_query_dual_bank(self, &error_local)) {
		g_debug("ignoring %s", error_local->message);
		fu_device_remove_flag(device, FWUPD_DEVICE_FLAG_UPDATABLE);
		return TRUE;
	}

	/* the daemon checks the firmware size against this value before the
	 * update is scheduled, so use the exact user bank size rather than a
	 * min/max range */
	fu_device_set_firmware_size(device, (guint64)self->user_fw_size << 16);
	return TRUE;
}
 ...
 * @error: (nullable): optional return location for an error
 *
 * Disables ISP mode and re-attaches the device on the USB bus by toggling
 * the attach bit.
 *
 * Returns: %TRUE on success
 **/
...
fu_realtek_billboard_device_attach(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	if (!fu_realtek_billboard_device_disable_isp(self, error)) {
		g_prefix_error_literal(error, "failed to disable ISP mode: ");
		return FALSE;
	}
 ...
 * @error: (nullable): optional return location for an error
 *
 * Queries the user start bank, user firmware size and user flag address from
 * the device.
 *
 * Returns: %TRUE on success
 **/
...
/**
 * fu_realtek_billboard_device_detach:
 * @device: a #FuDevice
 * @progress: a #FuProgress
 * @error: (nullable): optional return location for an error
 *
 * Enables ISP mode before the firmware write.
 *
 * Returns: %TRUE on success
 **/
static gboolean
fu_realtek_billboard_device_detach(FuDevice *device, FuProgress *progress, GError **error)
{
	FuRealtekBillboardDevice *self = FU_REALTEK_BILLBOARD_DEVICE(device);
					   ...
					   GError **error)
{
	...
	/* the dual bank info is queried in ->setup() and ISP mode is enabled
	 * in ->detach() */
	...
	/* the daemon already checked the firmware size against the value set
	 * in ->setup() before the update was scheduled */
...
fu_realtek_billboard_device_class_init(FuRealtekBillboardDeviceClass *klass)
{
	...
	device_class->detach = fu_realtek_billboard_device_detach;
