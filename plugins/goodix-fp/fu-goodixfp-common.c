...
#define REFLECT_DATA(X)         ((guint8) reflect ((X), 8))
#define REFLECT_REMAINDER(X)    ((guint32) reflect ((X), WIDTH))


guint8
gx_proto_crc8_calc (guint8 *lubp_date, guint32 lui_len)
{
  const guint8 *data = lubp_date;
  unsigned int crc = 0;
  int i, j;
...
static guint32 s_crc_table[256] =
...
static guint32
reflect (guint32 data, guint8 n_bits)
{
...
static void
crc32_update (gf_crc32_context *ctx, const guint8 *message, guint32 n_bytes)
{
  guint8 data;
  guint32 byte;
...
static void
crc32_final (gf_crc32_context *ctx, guint32 *md)
{
...
guint8
gx_proto_crc32_calc (guint8 *pchMsg, guint32 wDataLen, guint32 *pchMsgDst)
{
                 ...
                 )
{
  ...
  pheader->crc8 = gx_proto_crc8_calc ((guint8 *) pheader, 6);
...
gboolean
gx_proto_parse_body (guint8 cmd, guint8 *buffer, guint32 buffer_len, pgxfp_cmd_response_t presp)
{
  g_assert (buffer != NULL);
  g_assert (presp != NULL);
  g_assert (buffer_len >= 1);
