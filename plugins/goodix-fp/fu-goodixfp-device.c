			 ...
			 GError **error)
{
	...
	fu_goodixfp_build_header(&header, request->len, cmd0, cmd1, pkg_eop);
	g_byte_array_append(buf, (guint8 *)&header, sizeof(header));
	g_byte_array_append(buf, request->data, request->len);
	fu_goodixfp_crc32(buf->data, sizeof(header) + request->len, &crc_calc);
			 ...
			 GError **error)
{
	...
	*/
	while (1)
	{
		...
		ret = fu_goodixfp_parse_header(reply->data, actual_len, &header);
		if (!ret) {
			g_set_error_literal(error,
					    FWUPD_ERROR,
					    FWUPD_ERROR_INTERNAL,
		...
		fu_goodixfp_crc32(reply->data, offset, &crc32_calc);

		if (crc32_calc != GUINT32_FROM_LE(*(guint32 *)(reply->data + offset))) {
			g_set_error_literal(error,
					    FWUPD_ERROR,
					    FWUPD_ERROR_INTERNAL,
					    "Invalid checksum");
		...
		/* parse package data */
		ret = fu_goodixfp_parse_body(header.cmd0, reply->data + sizeof(header), header.len, presponse);
		if (!ret) {
			g_set_error_literal(error,
					    FWUPD_ERROR,
					    FWUPD_ERROR_INTERNAL,
