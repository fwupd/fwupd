...
fu_synaptics_rmi_firmware_parse_v10(FuFirmware *firmware, GBytes *fw, GError **error)
{
	...
	for (guint32 i = 0; i < cntrs_len; i++) {
		...
		guint32 addr;
		guint32 length;
		if (!fu_common_read_uint32_safe(data, sz, offset, &addr, G_LITTLE_ENDIAN, error))
			return FALSE;
...
fu_synaptics_rmi_firmware_write_v0x(FuFirmware *firmware, GError **error)
{
	...
	fu_common_write_uint32(buf->data + RMI_IMG_FW_OFFSET + 0x0,
			       0xdead,
			       G_LITTLE_ENDIAN); /* img */
	fu_common_write_uint32(buf->data + RMI_IMG_FW_OFFSET + bufsz,
			       0xbeef,
...
fu_synaptics_rmi_firmware_write_v10(FuFirmware *firmware, GError **error)
{
	...
	fu_common_write_uint32(buf->data + RMI_IMG_FW_OFFSET + 0x44,
			       0xfeed,
