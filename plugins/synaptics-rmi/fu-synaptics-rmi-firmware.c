...
static gboolean
fu_synaptics_rmi_firmware_parse_sbl_container_v10(FuFirmware *firmware,
						  GInputStream *stream,
						  const guint8 *buf,
						  gsize bufsz,
					...
					GError **error)
{
	...
	if (sig_sz != 0) {
		...
		fu_firmware_add_image(firmware, img, error);
	}
	return TRUE;
}
	...
	for (guint32 i = 0; i < cntrs_len; i++) {
		...
		case FU_RMI_CONTAINER_ID_BL:
			if (!fu_memread_uint8_safe(buf,
						   ...
						   error))
				return FALSE;
			if (!fu_synaptics_rmi_firmware_parse_sbl_container_v10(firmware,
									       stream,
									       buf,
									       bufsz,
									       offset,
						  ...
						  GError **error)
{
	...
	for (guint32 i = 0; i < cntrs_len; i++) {
		...
		g_info("RmiContainerDescriptor 0x%02x @ 0x%x (len 0x%x) sig_size 0x%x",
		       container_id,
		       content_addr,
		       length,
		       signature_size);
		...
		if (container_id == FU_RMI_CONTAINER_ID_BL_IMAGE) {
			if (signature_size != 0) {
				g_info("SBL signature size : 0x%x\n", signature_size);
			}
