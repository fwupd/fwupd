...
fu_ccgx_pure_hid_magic_unlock(FuDevice *device, gpointer user_data, GError **error)
{
	...
	// Ignoring error
	fu_hid_device_set_report(FU_HID_DEVICE(device),
				      bridge_buf[0],
				      bridge_buf,
				      sizeof(bridge_buf),
				      FU_CCGX_PURE_HID_DEVICE_TIMEOUT,
...
fu_ccgx_pure_hid_get_fw_info(FuDevice *device, gpointer user_data, GError **error)
{
	...
	HidFwInfo *info = (HidFwInfo *)(void *)buf;
	...
	g_autofree gchar *ver1 = fu_ccgx_detailed_version_to_string(info->image_1_ver);
	...
	g_autofree gchar *ver2 = fu_ccgx_detailed_version_to_string(info->image_2_ver);
	g_debug("Image 2 Ver:     %s", ver2);
	g_debug("Image 1 Row:     0x%08x", info->image_1_row);
	g_debug("Image 2 Row:     0x%08x", info->image_2_row);
	g_debug("Device UID:      0x%012x", info->device_uid);
...
fu_ccgx_pure_hid_device_init(FuCcgxPureHidDevice *self)
{
	...
	//fu_device_retry_set_delay(FU_DEVICE(self), FU_CCGX_PURE_HID_DEVICE_RETRY_DELAY);
}
