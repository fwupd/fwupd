				 ...
				 GError **error)
{
	...
	update_str_version = g_strdup_printf (
		"%u.%u.%u",
		(guint32)update_fw_version.ver.major,
		(guint32)update_fw_version.ver.minor,
		(guint32)update_fw_version.ver.build);

	g_return_val_if_fail (update_str_version != NULL, FALSE);

	g_debug ("Update version %s", update_str_version);

	/* update version of device */
	fu_device_set_version (FU_DEVICE (self), update_str_version, FWUPD_VERSION_FORMAT_TRIPLET);

	g_debug ("Writing Firmware Completed");
	return TRUE;
}
			  ...
			  GError **error)
{
	...
	if(self->dm_device != DM_DEVICE_PD_I2C) { 
		...
		g_warning ("not supported device type");
		return FALSE;
	}
	...
	g_return_val_if_fail (fw_buffer != NULL, FALSE);

	if (fw_size <= 0) {
		g_set_error(error,FWUPD_ERROR,
			    FWUPD_ERROR_NOT_SUPPORTED,
			    "firmware size error");
		g_warning("firmware size error");
		return FALSE;
	}
...
fu_ccgx_dock_bb_probe (FuDevice *device, GError **error)
{
	...
	devid_bb = g_strdup_printf ("USB\\VID_%04X&PID_%04X&bb",
			 	   (guint) vid, (guint)pid);
	g_return_val_if_fail (devid_bb != NULL, FALSE);
	...
	if (fu_device_has_custom_flag(FU_DEVICE(device),"cy-device-external-bb")) {
		...
		g_warning ("not supported device");
		return FALSE;
	}
	...
	if(self->dm_device  == DM_DEVICE_PD_I2C) {
		if (fu_device_has_custom_flag(FU_DEVICE(device),"cy-fw-image-single")) {
			...
			g_set_error(error, FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "not supported fw image");
			g_warning ("not supported fw image");
			return FALSE;
		}
...
fu_ccgx_dock_bb_setup (FuDevice *device, GError **error)
{
	...
	if (self->dm_device == DM_DEVICE_EXTERNAL_BB) { /* dm device type is external BB */
		...
		g_return_val_if_fail (device_str_version != NULL, FALSE);

		g_debug ("Parsed version %s", device_str_version);

		/* update version of device */
		fu_device_set_version (FU_DEVICE (self), device_str_version, FWUPD_VERSION_FORMAT_TRIPLET);
...
fu_ccgx_dock_bb_finalize (GObject *object)
{
	...
	if (self->fw_update_message) {
		g_free (self->fw_update_message);
	}
	if (self->fw_update_message_primary) {
		g_free (self->fw_update_message_primary);
	}
	if (self->fw_update_message_backup) {
				       ...
				       GError **error)
{
	...
	FuCcgxCyacdFirmware *fw_self = FU_CCGX_CYACD_FIRMWARE (firmware);
	
	//DEBUG_FUNC_NAME;
	fu_ccgx_cyacd_firmware_set_device_info (fw_self, self->fw_image_type, 
	...
	g_debug ("fw prepare parsed version: %s \n", fu_firmware_get_version (firmware));
