...
This plugin is used to update the Intel graphics system controller via
...
* The `INFO` partition is a `FuStructIgscFwuImageMetadataV1`, which
gives the `id` and `version`, and the `FuStructIgscFwuFwImageData` is
also part of that. This then gives the `arb_svn`.
* The `IMGI` partition is a `FuStructIgscFwuGwsImageInfo`, which gives
us `hw_sku`
...
* This is a superset of `FuIfwiFptFirmware`
* The `INFO` partition is a `FuIfwiCpdFirmware` is a
`FuStructIgscFwdataVersion`. This provides `oem_version`,
`major_version` and `data_arb_svn`.
* The `SDTA` (also called a ``GDTA'') data partition is a
`FuIfwiCpdFirmware`
...
This plugin supports the following protocol ID, used by all devices and
sub-devices:
...
The vendor ID is set from the PCI vendor, in this instance set to
`MEI:0x8086`
