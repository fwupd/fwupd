...
static void
fu_devlink_device_add_json(FuDevice *device, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuDevlinkDevice *self = FU_DEVLINK_DEVICE(device);
	GPtrArray *events = fu_device_get_events(device);

	/* add device type identifier */
	fwupd_json_object_add_string(json_object, "GType", "FuDevlinkDevice");

	/* add devlink-specific properties for regular devices */
	if (self->bus_name != NULL)
		fwupd_json_object_add_string(json_object, "BusName", self->bus_name);
	if (self->dev_name != NULL)
		fwupd_json_object_add_string(json_object, "DevName", self->dev_name);
...
static gboolean
fu_devlink_device_from_json(FuDevice *device, FwupdJsonObject *json_object, GError **error)
{
	FuDevlinkDevice *self = FU_DEVLINK_DEVICE(device);
	const gchar *bus_name;
	const gchar *dev_name;
	...
	bus_name = fwupd_json_object_get_string(json_object, "BusName", NULL);
	dev_name = fwupd_json_object_get_string(json_object, "DevName", NULL);
