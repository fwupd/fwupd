				    ...
				    GError **error)
{
	...
	block_str = fwupd_json_object_get_string(json_object, "data", error);
	if (block_str == NULL)
		return FALSE;
...

{
	...
	entity_str = fwupd_json_object_get_string(json_object, "entity", error);
	if (entity_str == NULL)
		return FALSE;
	...
	magic_str = fwupd_json_object_get_string(json_object, "magicStr", NULL);
	if (magic_str == NULL) {
	...
	payload_str = fwupd_json_object_get_string(json_object, "payload", error);
	if (payload_str == NULL)
		return FALSE;
	g_debug("RDFU firmware for entity %u payload = %s", (guint)entity, payload_str);

	model_id_str = fwupd_json_object_get_string(json_object, "modelId", error);
	...
	name_str = fwupd_json_object_get_string(json_object, "name", error);
	if (name_str == NULL)
		return FALSE;

	revision_str = fwupd_json_object_get_string(json_object, "revision", error);
	if (revision_str == NULL)
		return FALSE;
	if (!fu_strtoull(revision_str, &revision, 0, G_MAXUINT8, FU_INTEGER_BASE_AUTO, error))
		return FALSE;

	build_str = fwupd_json_object_get_string(json_object, "build", error);
	if (build_str == NULL)
		return FALSE;
				...
				GError **error)
{
	...
	g_autoptr(FwupdJsonObject) json_payloads_obj = NULL;
	...
	json_node = fwupd_json_parser_load_from_stream(parser, stream, FWUPD_JSON_LOAD_FLAG_NONE, error);
	if (json_node == NULL)
		return FALSE;
	json_object = fwupd_json_node_get_object(json_node, error);
	if (json_object == NULL)
		return FALSE;
	firmware_str = fwupd_json_object_get_string(json_object, "fileVersion", error);
	...
	contents = fwupd_json_object_get_array(json_object, "contents", error);
	if (contents == NULL)
		return FALSE;
	...
	for (guint i = 0; i < fwupd_json_array_get_size(contents); i++) {
		...
		json_object_tmp = fwupd_json_array_get_object(contents, i, error);
		if (json_object_tmp == NULL)
			return FALSE;
		if (!fu_logitech_rdfu_firmware_entry_add(entity_fw, json_object_tmp, error)) {
	...
	json_payloads_obj = fwupd_json_object_get_object(json_object, "payloads", error);
	if (json_payloads_obj == NULL)
		return FALSE;

	keys = fwupd_json_object_get_keys(json_payloads_obj);
	for (guint j = 0; j < keys->len; j++) {
		const gchar *payload_id = g_ptr_array_index(keys, j);
		guint index = 0;
		FuLogitechRdfuFirmware *entity_fw = NULL;
		g_autoptr(FwupdJsonArray) json_blocks = NULL;
		...
		json_object = fwupd_json_object_get_object(json_payloads_obj, payload_id, error);
		if (json_object == NULL)
			return FALSE;
		json_blocks = fwupd_json_object_get_array(json_object, "blocks", error);
		if (json_blocks == NULL) {
			g_prefix_error(error, "payload %s has no blocks: ", payload_id);
			return FALSE;
		}
		if (fwupd_json_array_get_size(contents) < 1) {
			g_set_error(error,
		...
		/* adding blocks to the entity FW */
		for (guint i = 0; i < fwupd_json_array_get_size(json_blocks); i++) {
			g_autoptr(FwupdJsonObject) block = NULL;

			block = fwupd_json_array_get_object(json_blocks, i, error);
			if (block == NULL)
				return FALSE;
