...
static gboolean fu_algoltek_usb_check_magic(FuFirmware *firmware, GBytes *fw, gsize offset,
                                            GError **error) {
    guint64 magic = 0;
    /* First byte is header length */
    offset += 1;

    if (!fu_memread_uint64_safe(g_bytes_get_data(fw, NULL), g_bytes_get_size(fw), offset, &magic,
    ...
    if (magic != FU_ALGOLTEK_HEADER) {
        g_set_error(error, FWUPD_ERROR, FWUPD_ERROR_INVALID_FILE,
                    "invalid magic, expected 0x%08lX got 0x%08lX", (guint64)FU_ALGOLTEK_HEADER,
                    magic);
        return FALSE;
    }
    /* success */
...
static gboolean fu_algoltek_usb_firmware_parse(FuFirmware *firmware, GBytes *fw, gsize offset,
...
static GByteArray *fu_algoltek_usb_firmware_write(FuFirmware *firmware, GError **error) {
...
static void fu_algoltek_usb_firmware_init(FuAlgoltekUsbFirmware *self) {
    // fu_firmware_add_flag(FU_FIRMWARE(self), FU_FIRMWARE_FLAG_HAS_STORED_SIZE);
    // fu_firmware_add_flag(FU_FIRMWARE(self), FU_FIRMWARE_FLAG_HAS_CHECKSUM);
    // fu_firmware_add_flag(FU_FIRMWARE(self), FU_FIRMWARE_FLAG_HAS_VID_PID);
}

static void fu_algoltek_usb_firmware_class_init(FuAlgoltekUsbFirmwareClass *klass) {
    FuFirmwareClass *klass_firmware = FU_FIRMWARE_CLASS(klass);
    klass_firmware->check_magic     = fu_algoltek_usb_check_magic;
    klass_firmware->parse           = fu_algoltek_usb_firmware_parse;
    klass_firmware->write           = fu_algoltek_usb_firmware_write;
}

FuFirmware *fu_algoltek_usb_firmware_new(void) {
