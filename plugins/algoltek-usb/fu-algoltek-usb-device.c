...
static void fu_algoltek_usb_device_to_string(FuDevice *device, guint idt, GString *str) {
    FU_DEVICE_CLASS(fu_algoltek_usb_device_parent_class)->to_string(device, idt, str);
}

static guint8 checkSum(GByteArray *byteArray) {
    guint8 result = 0;
...
static GByteArray *fu_algoltek_device_CMD02(FuAlgoltekUsbDevice *self, int address,
                                            GError **error) {
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_DEVICE_TO_HOST, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD02, address, /* value */
            0xFFFF,                                                                /* idx */
            buf->data, buf->len, NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
...
static GByteArray *fu_algoltek_device_CMD01(FuAlgoltekUsbDevice *self, GError **error) {
    GUsbDevice *usb_device            = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf         = g_byte_array_new();
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_DEVICE_TO_HOST, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD01, 0xFFFF, /* value */
            0xFFFF,                                                               /* idx */
            buf->data, buf->len, &actual_length, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return NULL;
    }

    if (actual_length != buf->len) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA, "got 0x%x but requested 0x%x",
                    (guint)actual_length, (guint)buf->len);
        return NULL;
    }
...
static guint fu_algoltek_readout_info_int(GByteArray *backData) {
...
static gboolean fu_algoltek_device_CMD03(FuAlgoltekUsbDevice *self, GError **error) {
    GUsbDevice *usb_device    = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf = g_byte_array_new();
    ...
    buf->data[10] = checkSum(buf);
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD03, 0, /* value */
            0,                                                               /* idx */
            buf->data, buf->data[0], NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return FALSE;
    }
...
static gboolean fu_algoltek_device_CMD04(FuAlgoltekUsbDevice *self, guint8 number, GError **error) {
    GUsbDevice *usb_device    = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf = g_byte_array_new();
    fu_byte_array_set_size(buf, 11, 0x0);
    buf->data[0]  = 4;
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD04, 0, /* value */
            0,                                                               /* idx */
            buf->data, buf->data[0], NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return FALSE;
...
static gboolean fu_algoltek_device_CMD05(FuAlgoltekUsbDevice *self, int address, int inputValue,
                                         GError **error) {
    GUsbDevice *usb_device    = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf = g_byte_array_new();
    fu_byte_array_set_size(buf, 11, 0x0);
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD05, 0, /* value */
            0,                                                               /* idx */
            buf->data, buf->data[0], NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return FALSE;
...
static gboolean fu_algoltek_device_CMD06(FuAlgoltekUsbDevice *self, GByteArray *ISPData,
                                         int address, GError **error) {
    GUsbDevice *usb_device            = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    ...
    while (restTransferDataSize > 0) {
        ...
        if (!g_usb_device_control_transfer(
                usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
                G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD06, 0, /* value */
                0,                                                               /* idx */
                partIspData->data, partIspData->data[0], NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL,
                error)) {
            g_prefix_error(error, "failed to contact device: ");
            return FALSE;
        }
...
static gboolean fu_algoltek_device_CMD07(FuAlgoltekUsbDevice *self, int address, GError **error) {
    GUsbDevice *usb_device    = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf = g_byte_array_new();
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD07, 0, /* value */
            0,                                                               /* idx */
            buf->data, buf->data[0], NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return FALSE;
...
static gboolean fu_algoltek_device_CMD08(FuAlgoltekUsbDevice *self, GError **error) {
    GUsbDevice *usb_device    = fu_usb_device_get_dev(FU_USB_DEVICE(self));
    g_autoptr(GByteArray) buf = g_byte_array_new();
    ...
    if (!g_usb_device_control_transfer(
            usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
            G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD08, 0, /* value */
            0,                                                               /* idx */
            buf->data, buf->len, NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL, error)) {
        g_prefix_error(error, "failed to contact device: ");
        return FALSE;
...
static gboolean fu_algoltek_device_status_check(FuAlgoltekUsbDevice *self, GError **error) {
...
static gboolean fu_algoltek_device_CMD09(FuAlgoltekUsbDevice *self, GByteArray *firmwareData,
    ...
    while (restTransferDataSize > 0) {
        ...
        if (!g_usb_device_control_transfer(
                usb_device, G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE, G_USB_DEVICE_REQUEST_TYPE_VENDOR,
                G_USB_DEVICE_RECIPIENT_INTERFACE, ALGOLTEK_USB_REQUEST_CMD09, value, /* value */
                index,                                                               /* idx */
                partFirmware->data, partFirmware->len, NULL, ALGOLTEK_DEVICE_USB_TIMEOUT, NULL,
                error)) {
            g_prefix_error(error, "failed to contact device: ");
            return FALSE;
        }
...
static gboolean fu_algoltek_usb_device_setup(FuDevice *device, GError **error) {
...
static gboolean fu_algoltek_usb_device_write_firmware(FuDevice *device, FuFirmware *firmware,
                                                      FuProgress *progress, FwupdInstallFlags flags,
                                                      GError **error) {
    FuAlgoltekUsbDevice *self        = FU_ALGOLTEK_USB_DEVICE(device);
...
static void fu_algoltek_usb_device_set_progress(FuDevice *self, FuProgress *progress) {
    fu_progress_set_id(progress, G_STRLOC);
    fu_progress_add_flag(progress, FU_PROGRESS_FLAG_GUESSED);
...
static void fu_algoltek_usb_device_init(FuAlgoltekUsbDevice *self) {
...
static void fu_algoltek_usb_device_class_init(FuAlgoltekUsbDeviceClass *klass) {
