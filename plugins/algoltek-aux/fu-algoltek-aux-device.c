			   ...
			   GError **error)
{
	...
	chunks = fu_chunk_array_new_from_bytes(fw, 0, 8);
				      ...
				      GError **error)
{
	...
	/* write ISP image */
	bytes_isp = fu_firmware_get_image_by_id_bytes(firmware, "isp", error);
	if (bytes_isp == NULL)
		return FALSE;
	...
	/* write payload image */
	bytes_payload = fu_firmware_get_image_by_id_bytes(firmware, FU_FIRMWARE_ID_PAYLOAD, error);
