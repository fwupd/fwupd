...
#define FU_SYNAPTICS_CAPE_DEVICE_USB_RESET_DELAY_MS	300   /* ms */

#define FU_SYNAPTICS_CAPE_DEVICE_GOLEM_REPORT_ID 1
...
/* CAPE HID message structure. Little endian.*/
typedef struct __attribute__((packed)) {
	gint16 data_len : 16; /*number of dwords*/
...
/*GET flag*/
#define FU_SYNAPTICS_CMD_GET_FLAG 0x100

/* CAPE Fuupd device structure*/
struct _FuSynapticsCapeDevice {
	FuHidDevice parent_instance;
	guint32 active_partition; /*active partition, 0 or 1*/
	gchar *version;
};
...
fu_synaptics_cape_device_to_string(FuDevice *device, guint idt, GString *str)
{
	...
	fu_common_string_append_ku(str, idt, "Active Partition", self->active_partition);
	fu_common_string_append_kv(str, idt, "FW version", self->version);
}
...
fu_synaptics_cape_device_reset(FuSynapticsCapeDevice *self, GError **error)
{
	...
	if (!(result =
		  fu_synaptics_cape_device_sendcmd(self,
					  ...
					  GError **error)
{
	...
	/*check file size*/
	if (bufsz < sizeof(FuCapeHidFileHeader) * 2) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_FILE,
					...
					GError **error)
{
	...
	g_debug("syna: fu_synaptics_cape_device_write_firmware()\n");
...
static gboolean
fu_synaptics_cape_device_attach(FuDevice *device, GError **error)
{
	FuSynapticsCapeDevice *self = FU_SYNAPTICS_CAPE_DEVICE(device);
	g_autoptr(GError) error_local = NULL;
				    ...
				    GError **error)
{
	...
	memcpy(&report.cmd, req, sizeof(report.cmd));

	/*Set data length to MAX for any GET commands*/
	if (FU_SYNAPTICS_CMD_GET_FLAG & report.cmd.cmd_id) {
		report.cmd.data_len = GUINT32_TO_LE(FU_SYNAPTICS_CAPE_CMD_MAX_DATA_LEN);
	}
	report.cmd.data_len = GUINT32_TO_LE(report.cmd.data_len);
	...
	report.cmd.data_len = GUINT32_FROM_LE(report.cmd.data_len);

	/* copy returned data if it is GET command*/
	if (FU_SYNAPTICS_CMD_GET_FLAG & report.cmd.cmd_id && report.cmd.reply) {
		memcpy(req, &report.cmd, sizeof(report.cmd));
	}
...
fu_synaptics_cape_device_setup_version(FuSynapticsCapeDevice *self, GError **error)
{
	...
	version_raw = (cmd.data[0] & 0xff) << 24 | (cmd.data[1] & 0xff) << 16 |
		      (cmd.data[2] & 0x8) << 24 | (cmd.data[3] & 0xff);

	self->version = fu_common_version_from_uint32(version_raw, FWUPD_VERSION_FORMAT_QUAD);
	fu_device_set_version(FU_DEVICE(self), self->version);
...
static void
fu_synpatics_cape_device_finalize(GObject *object)
{
	FuSynapticsCapeDevice *self = FU_SYNAPTICS_CAPE_DEVICE(object);
	g_free(self->version);
	G_OBJECT_CLASS(fu_synaptics_cape_device_parent_class)->finalize(object);
}
...
fu_synaptics_cape_device_class_init(FuSynapticsCapeDeviceClass *klass)
{
	...
	object_class->finalize = fu_synpatics_cape_device_finalize;
	...
	klass_device->attach = fu_synaptics_cape_device_attach;
