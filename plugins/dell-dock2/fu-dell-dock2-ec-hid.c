...
gboolean
fu_dell_dock2_ec_hid_write(FuDevice *device, GBytes *buf, GError **error)
{
	...
	return fu_hid_device_set_report(FU_HID_DEVICE(device),
					...
					data,
					data_sz,
					DELL_DOCK2_EC_HID_TIMEOUT,
					FU_HID_DEVICE_FLAG_RETRY_FAILURE,
					error);
}

GBytes *
fu_dell_dock2_hid_fwup_pkg_new(GBytes *fw, guint8 dev_type, guint8 dev_identifier)
{
	g_autoptr(GByteArray) fwbuf = g_byte_array_new();
	gsize fw_size = g_bytes_get_size(fw);

	/* header */
	fu_byte_array_append_uint8(fwbuf, DELL_DOCK2_EC_HID_CMD_FWUPDATE);
	fu_byte_array_append_uint8(fwbuf, DELL_DOCK2_EC_HID_EXT_FWUPDATE);
	fu_byte_array_append_uint32(fwbuf, 7 + fw_size, G_BIG_ENDIAN); // 7 = sizeof(command)

	/* command */
	fu_byte_array_append_uint8(fwbuf, DELL_DOCK2_EC_HID_SUBCMD_FWUPDATE);
...
static gboolean
fu_dell_dock2_hid_set_report_cb(FuDevice *self, gpointer user_data, GError **error)
{
	...
	return fu_hid_device_set_report(FU_HID_DEVICE(self),
					...
					outbuffer,
					192,
					DELL_DOCK2_EC_HID_TIMEOUT * 3,
					FU_HID_DEVICE_FLAG_NONE,
					error);
}

static gboolean
fu_dell_dock2_hid_set_report(FuDevice *self, guint8 *outbuffer, GError **error)
{
	return fu_device_retry(self,
			       fu_dell_dock2_hid_set_report_cb,
			       DELL_DOCK2_EC_HID_MAX_RETRIES,
			       outbuffer,
			       error);
}

static gboolean
fu_dell_dock2_hid_get_report_cb(FuDevice *self, gpointer user_data, GError **error)
{
...
static gboolean
fu_dell_dock2_hid_get_report(FuDevice *self, guint8 *inbuffer, GError **error)
{
	return fu_device_retry(self,
			       fu_dell_dock2_hid_get_report_cb,
			       DELL_DOCK2_EC_HID_MAX_RETRIES,
			       inbuffer,
			       error);
}
...
gboolean
fu_dell_dock2_hid_raise_mcu_clock(FuDevice *self, gboolean enable, GError **error)
{
	...
	if (!fu_dell_dock2_hid_set_report(self, (guint8 *)&cmd_buffer, error)) {
...
gboolean
fu_dell_dock2_hid_erase_bank(FuDevice *self, guint8 idx, GError **error)
{
	FuEcHIDCmdBuffer cmd_buffer = {
	    .cmd = HUB_CMD_WRITE_DATA,
...
gboolean
fu_dell_dock2_hid_i2c_write(FuDevice *self,
			    const guint8 *input,
			    gsize write_size,
			    ...
			    GError **error)
{
	...
	return fu_dell_dock2_hid_set_report(self, (guint8 *)&cmd_buffer, error);
}

gboolean
fu_dell_dock2_hid_i2c_read(FuDevice *self,
			   guint32 cmd,
			   GByteArray *res,
			   const FuHIDI2CParameters *parameters,
			   guint delayms,
			   GError **error)
{
	...
	if (!fu_dell_dock2_hid_get_report(self, cmd_buffer.data, error))
		return FALSE;
