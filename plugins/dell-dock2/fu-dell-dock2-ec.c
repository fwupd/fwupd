...
const FuHIDI2CParameters dell_dock2_ec_base_settings = {
    .i2ctargetaddr = DELL_DOCK2_EC_HID_I2C_ADDRESS,
    .regaddrlen = 1,
    .i2cspeed = I2C_SPEED_250K | 0x80,
};
...
typedef struct __attribute__((packed)) {
	...
	guint8 unused[35];
} FuDellDock2DockDataStructure;

typedef struct __attribute__((packed)) {
	struct FuDellDock2V2DockInfoHeader {
		guint8 total_devices;
		guint8 first_index;
		guint8 last_index;
	} header;
	struct FuDellDock2EcQueryEntry {
		struct FuDellDock2V2EcAddrMap {
			guint8 location;
			guint8 device_type;
			guint8 sub_type;
			guint8 arg;
...
} FuDellDock2DockInfoStructure;

/* Private structure */
struct _FuDellDock2Ec {
	FuDevice parent_instance;
	FuDellDock2DockDataStructure *dock_data;
	FuDellDock2DockInfoStructure *dock_info;
	FuDellDock2DockFWVersion *raw_versions;
	FuDellDock2BaseType base_type;
	guint8 base_sku;
...
{
	switch (device_type) {
	case DELL_DOCK2_EC_DEV_TYPE_MAIN_EC:
		return "EC";
	case DELL_DOCK2_EC_DEV_TYPE_PD:
		if (sub_type == DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI) {
			if (instance == DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP5)
				return "PD UP5";
			if (instance == DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP15)
				return "PD UP15";
			if (instance == DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP17)
				return "PD UP17";
		}
		return NULL;
	case DELL_DOCK2_EC_DEV_TYPE_USBHUB:
		if (sub_type == DELL_DOCK2_EC_DEV_USBHUB_SUBTYPE_RTS5480)
			return "RTS5480 USB Hub";
		if (sub_type == DELL_DOCK2_EC_DEV_USBHUB_SUBTYPE_RTS5485)
			return "RTS5485 USB Hub";
		return NULL;
	case DELL_DOCK2_EC_DEV_TYPE_MST:
		if (sub_type == DELL_DOCK2_EC_DEV_MST_SUBTYPE_VMM8430)
			return "MST VMM8430";
		if (sub_type == DELL_DOCK2_EC_DEV_MST_SUBTYPE_VMM9430)
			return "MST VMM9430";
		return NULL;
	case DELL_DOCK2_EC_DEV_TYPE_TBT:
		if (sub_type == DELL_DOCK2_EC_DEV_TBT_SUBTYPE_TR)
			return "Titan Ridge";
		if (sub_type == DELL_DOCK2_EC_DEV_TBT_SUBTYPE_GR)
			return "Goshen Ridge";
		if (sub_type == DELL_DOCK2_EC_DEV_TBT_SUBTYPE_BR)
			return "Barlow Ridge";
		return NULL;
	case DELL_DOCK2_EC_DEV_TYPE_QI:
		return "Qi";
	...
	case DELL_DOCK2_EC_DEV_TYPE_WTPD:
		return "Weltrend PD";
	default:
		return NULL;
	}
...
static gboolean
fu_dell_dock2_ec_read(FuDevice *device, guint32 cmd, GByteArray *res, GError **error)
{
	if (!fu_dell_dock2_hid_i2c_read(device,
					cmd,
					res,
					&dell_dock2_ec_base_settings,
					800,
					error)) {
...
static gboolean
fu_dell_dock2_ec_write(FuDevice *device, GByteArray *buf, GError **error)
{
	g_return_val_if_fail(device != NULL, FALSE);
	g_return_val_if_fail(buf->len > 1, FALSE);

	if (!fu_dell_dock2_hid_i2c_write(device,
					 buf->data,
					 buf->len,
					 &dell_dock2_ec_base_settings,
					 error)) {
...
static gboolean
fu_dell_dock2_ec_create_node(FuDevice *ec_device, FuDevice *new_device, GError **error)
{
...
static gboolean
fu_dell_dock2_ec_probe_package(FuDevice *ec_dev, GError **error)
{
	g_autoptr(FuDellDock2Package) pkg_dev = NULL;

	pkg_dev = fu_dell_dock2_package_new(ec_dev);
	return fu_dell_dock2_ec_create_node(ec_dev, FU_DEVICE(pkg_dev), error);
}

static gboolean
fu_dell_dock2_ec_probe_pd(FuDevice *ec_dev,
			  DellDock2EcDevType dev_type,
			  DellDock2EcDevPdSubtype subtype,
			  guint8 instance,
			  GError **error)
{
	g_autoptr(FuDellDock2Pd) pd_dev = NULL;

	if (fu_dell_dock2_ec_dev_entry(ec_dev, dev_type, subtype, instance) == NULL)
		return TRUE;

	pd_dev = fu_dell_dock2_pd_new(ec_dev, subtype, instance);
	return fu_dell_dock2_ec_create_node(ec_dev, FU_DEVICE(pd_dev), error);
}

static gboolean
fu_dell_dock2_ec_probe_subcomponents(FuDevice *device, GError **error)
{
	g_return_val_if_fail(device != NULL, FALSE);

	/* Package */
	if (!fu_dell_dock2_ec_probe_package(device, error))
		return FALSE;

	/* PD UP5 */
	if (!fu_dell_dock2_ec_probe_pd(device,
				       DELL_DOCK2_EC_DEV_TYPE_PD,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP5,
				       error))
		return FALSE;
	...
	if (!fu_dell_dock2_ec_probe_pd(device,
				       DELL_DOCK2_EC_DEV_TYPE_PD,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP15,
				       error))
		return FALSE;
	...
	if (!fu_dell_dock2_ec_probe_pd(device,
				       DELL_DOCK2_EC_DEV_TYPE_PD,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI,
				       DELL_DOCK2_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP17,
				       error))
		return FALSE;

	/* DP MUX */
	if (fu_dell_dock2_ec_dev_entry(device, DELL_DOCK2_EC_DEV_TYPE_DP_MUX, 0, 0) != NULL) {
		g_autoptr(FuDellDock2Dpmux) dpmux_device = NULL;

		dpmux_device = fu_dell_dock2_dpmux_new(device);
		if (!fu_dell_dock2_ec_create_node(device, FU_DEVICE(dpmux_device), error))
			return FALSE;
	}

	/* WELTREND PD */
	if (fu_dell_dock2_ec_dev_entry(device, DELL_DOCK2_EC_DEV_TYPE_WTPD, 0, 0) != NULL) {
		g_autoptr(FuDellDock2Wtpd) weltrend_device = NULL;
...
static gboolean
fu_dell_dock2_ec_dock_type_extract(FuDevice *device, GError **error)
{
	FuDellDock2BaseType dock_type = fu_dell_dock2_ec_get_dock_type(device);
	guint8 dev_type = DELL_DOCK2_EC_DEV_TYPE_MAIN_EC;

	/* don't change error type, the plugin ignores it */
	if (dock_type != FU_DELL_DOCK2_BASE_TYPE_K2) {
...
static gboolean
fu_dell_dock2_ec_dock_type_cmd(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
	gsize length = 1;
	g_autoptr(GByteArray) res = g_byte_array_new_take(g_malloc0(length), length);
	guint8 cmd = DELL_DOCK2_EC_HID_CMD_GET_DOCK_TYPE;

	/* expect response 1 byte */
	if (!fu_dell_dock2_ec_read(device, cmd, res, error)) {
	...
	return fu_dell_dock2_ec_dock_type_extract(device, error);
}

static gboolean
fu_dell_dock2_ec_dock_info_extract(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
	...
	for (guint i = 0; i < self->dock_info->header.total_devices; i++) {
		struct FuDellDock2EcQueryEntry dev_entry = self->dock_info->devices[i];
		const gchar *type_str;
		const gchar *location_str;
		guint32 version32 = GUINT32_FROM_BE(dev_entry.version.version_32);

		/* name the component */
		type_str = fu_dell_dock2_ec_devicetype_to_str(dev_entry.ec_addr_map.device_type,
		...
		location_str = (dev_entry.ec_addr_map.location == DELL_DOCK2_EC_LOCATION_BASE)
				   ? "Base"
				   : "Module";
		...
		g_debug("\tVersion32: %08x, Version8: %s",
			...
			fu_hex_version_from_uint32(version32, FWUPD_VERSION_FORMAT_QUAD));
	}
	return TRUE;
}

static gboolean
fu_dell_dock2_ec_dock_info_cmd(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
	gsize length = sizeof(FuDellDock2DockInfoStructure);
	g_autoptr(GByteArray) res = g_byte_array_new_take(g_malloc0(length), length);
	guint8 cmd = DELL_DOCK2_EC_HID_CMD_GET_DOCK_INFO;
	...
	memcpy(self->dock_info, res->data, length);
	if (!fu_dell_dock2_ec_dock_info_extract(device, error))
		return FALSE;
	return TRUE;
}

static gboolean
fu_dell_dock2_ec_dock_data_extract(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
...
static gboolean
fu_dell_dock2_ec_dock_data_cmd(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
	gsize length = sizeof(FuDellDock2DockDataStructure);
	g_autoptr(GByteArray) res = g_byte_array_new_take(g_malloc0(length), length);
	guint8 cmd = DELL_DOCK2_EC_HID_CMD_GET_DOCK_DATA;
	...
	memcpy(self->dock_data, res->data, length);
	if (!fu_dell_dock2_ec_dock_data_extract(device, error))
		return FALSE;
	return TRUE;
}
...
fu_dell_dock2_ec_is_dock_ready4update(FuDevice *device, GError **error)
{
	...
	if (!fu_dell_dock2_ec_dock_data_cmd(device, error))
		return FALSE;
...
static void
fu_dell_dock2_ec_to_string(FuDevice *device, guint idt, GString *str)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);
...
fu_dell_dock2_ec_modify_lock(FuDevice *device, gboolean lock, GError **error)
{
	...
	fu_byte_array_append_uint8(req, DELL_DOCK2_EC_HID_CMD_SET_MODIFY_LOCK);
	...
	if (!fu_dell_dock2_ec_write(device, req, &error_local)) {
...
static gboolean
fu_dell_dock2_ec_activate(FuDevice *device, FuProgress *progress, GError **error)
{
	/* (TODO) reset ec only */
	return TRUE;
}
...
fu_dell_dock2_ec_run_passive_update(FuDevice *device, GError **error)
{
	...
	fu_byte_array_append_uint8(req, DELL_DOCK2_EC_HID_CMD_SET_PASSIVE);
...
static gboolean
fu_dell_dock2_ec_set_dock_sku(FuDevice *device, GError **error)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(device);

	switch (self->base_type) {
	case FU_DELL_DOCK2_BASE_TYPE_K2:
		/* TBT type yet available, do workaround */
		if (fu_dell_dock2_ec_dev_entry(device,
					       DELL_DOCK2_EC_DEV_TYPE_TBT,
					       DELL_DOCK2_EC_DEV_TBT_SUBTYPE_BR,
					       0) != NULL) {
		...
		if (fu_dell_dock2_ec_dev_entry(device,
					       DELL_DOCK2_EC_DEV_TYPE_TBT,
					       DELL_DOCK2_EC_DEV_TBT_SUBTYPE_GR,
					       0) != NULL) {
...
fu_dell_dock2_ec_commit_package(FuDevice *device, GBytes *blob_fw, GError **error)
{
	...
	if (length != sizeof(FuDellDock2DockFWVersion)) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
	...
	fu_byte_array_append_uint8(req, DELL_DOCK2_EC_HID_CMD_SET_DOCK_PKG);
	...
	if (!fu_dell_dock2_ec_write(device, req, error)) {
...
static gboolean
fu_dell_dock2_ec_write_firmware(FuDevice *device,
				FuFirmware *firmware,
				FuProgress *progress,
				...
				GError **error)
{
	...
	if (!fu_dell_dock2_hid_raise_mcu_clock(device, TRUE, error))
		return FALSE;
	...
	fw_whdr = fu_dell_dock2_hid_fwup_pkg_new(fw, DELL_DOCK2_EC_DEV_TYPE_MAIN_EC, 0);

	/* prepare the chunks */
	chunks = fu_chunk_array_new_from_bytes(fw_whdr, 0, DELL_DOCK2_EC_HID_DATA_PAGE_SZ);

	/* erase */
	if (!fu_dell_dock2_hid_erase_bank(device, 0xff, error))
		return FALSE;
	fu_progress_step_done(progress);
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		if (!fu_dell_dock2_ec_hid_write(device, fu_chunk_get_bytes(chk), error))
			return FALSE;
	}
	fu_progress_step_done(progress);

	if (!fu_dell_dock2_hid_raise_mcu_clock(device, FALSE, error))
		return FALSE;
	fu_progress_step_done(progress);
...
static gboolean
fu_dell_dock2_ec_query_cb(FuDevice *device, gpointer user_data, GError **error)
{
	/* dock data */
	if (!fu_dell_dock2_ec_dock_data_cmd(device, error))
		return FALSE;
	...
	/* dock info */
	if (!fu_dell_dock2_ec_dock_info_cmd(device, error))
		return FALSE;

	/* set internal dock sku, must after dock info */
	if (!fu_dell_dock2_ec_set_dock_sku(device, error))
		return FALSE;
...
static gboolean
fu_dell_dock2_ec_reload(FuDevice *device, GError **error)
{
	/* if query looks bad, wait a few seconds and retry */
	if (!fu_device_retry_full(device, fu_dell_dock2_ec_query_cb, 10, 2000, NULL, error)) {
...
static gboolean
fu_dell_dock2_ec_setup(FuDevice *device, GError **error)
{
	guint32 ec_version = 0;

	/* FuUsbDevice->setup */
	if (!FU_DEVICE_CLASS(fu_dell_dock2_ec_parent_class)->setup(device, error))
		return FALSE;

	/* get dock type */
	if (!fu_dell_dock2_ec_dock_type_cmd(device, error))
		return FALSE;
	...
	/* setup version */
	ec_version = GUINT32_FROM_BE(fu_dell_dock2_ec_get_ec_version(device));
	fu_device_set_version_raw(device, ec_version);

	/* create the subcomponents */
	if (!fu_dell_dock2_ec_probe_subcomponents(device, error))
		return FALSE;

	g_debug("dell-dock2-ec->setup done successfully");
	return TRUE;
}

static gchar *
fu_dell_dock2_ec_convert_version(FuDevice *device, guint64 version_raw)
{
...
static void
fu_dell_dock2_ec_finalize(GObject *object)
{
	FuDellDock2Ec *self = FU_DELL_DOCK2_EC(object);
	g_free(self->dock_data);
	g_free(self->dock_info);
	g_free(self->raw_versions);
	g_free(self->ec_minimum_version);
	G_OBJECT_CLASS(fu_dell_dock2_ec_parent_class)->finalize(object);
}

static void
fu_dell_dock2_ec_set_progress(FuDevice *self, FuProgress *progress)
{
	fu_progress_set_id(progress, G_STRLOC);
...
static void
fu_dell_dock2_ec_init(FuDellDock2Ec *self)
{
	self->dock_data = g_new0(FuDellDock2DockDataStructure, 1);
	self->raw_versions = g_new0(FuDellDock2DockFWVersion, 1);
	self->dock_info = g_new0(FuDellDock2DockInfoStructure, 1);
...
static void
fu_dell_dock2_ec_class_init(FuDellDock2EcClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuDeviceClass *device_class = FU_DEVICE_CLASS(klass);
	object_class->finalize = fu_dell_dock2_ec_finalize;
	device_class->activate = fu_dell_dock2_ec_activate;
	device_class->to_string = fu_dell_dock2_ec_to_string;
	device_class->open = fu_dell_dock2_ec_open;
	device_class->setup = fu_dell_dock2_ec_setup;
	device_class->write_firmware = fu_dell_dock2_ec_write_firmware;
	device_class->reload = fu_dell_dock2_ec_reload;
	device_class->set_progress = fu_dell_dock2_ec_set_progress;
	device_class->convert_version = fu_dell_dock2_ec_convert_version;
}

FuDellDock2Ec *
fu_dell_dock2_ec_new(FuDevice *device)
{
	FuDellDock2Ec *self = NULL;
	FuContext *ctx = fu_device_get_context(device);

	self = g_object_new(FU_TYPE_DELL_DOCK2_EC, "context", ctx, NULL);
