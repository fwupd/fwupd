...
static gchar *
fu_dell_dock2_dpmux_convert_version(FuDevice *device, guint64 version_raw)
{
	return fu_hex_version_from_uint32(version_raw, fu_device_get_version_format(device));
}

static gboolean
fu_dell_dock2_dpmux_setup(FuDevice *device, GError **error)
{
	...
	guint8 dev_type = DELL_DOCK2_EC_DEV_TYPE_DP_MUX;
	...
	/* version */
	dpmux_version = fu_dell_dock2_ec_get_dpmux_version(proxy);
...
static gboolean
fu_dell_dock2_dpmux_write(FuDevice *device,
			  FuFirmware *firmware,
			  FuProgress *progress,
...
static void
fu_dell_dock2_dpmux_set_progress(FuDevice *self, FuProgress *progress)
{
	fu_progress_set_id(progress, G_STRLOC);
...
static void
fu_dell_dock2_dpmux_init(FuDellDock2Dpmux *self)
{
	...
	fu_device_set_firmware_gtype(FU_DEVICE(self), FU_TYPE_DELL_DOCK2_DPMUX_FIRMWARE);
}

static void
fu_dell_dock2_dpmux_class_init(FuDellDock2DpmuxClass *klass)
{
	FuDeviceClass *device_class = FU_DEVICE_CLASS(klass);
	device_class->write_firmware = fu_dell_dock2_dpmux_write;
	device_class->setup = fu_dell_dock2_dpmux_setup;
	device_class->set_progress = fu_dell_dock2_dpmux_set_progress;
	device_class->convert_version = fu_dell_dock2_dpmux_convert_version;
}
