...
struct _FuDellDock2Wtpd {
	FuDevice parent_instance;
};

G_DEFINE_TYPE(FuDellDock2Wtpd, fu_dell_dock2_wtpd, FU_TYPE_DEVICE)

static gchar *
fu_dell_dock2_wtpd_convert_version(FuDevice *device, guint64 version_raw)
{
	return fu_hex_version_from_uint32(version_raw, fu_device_get_version_format(device));
}

static gboolean
fu_dell_dock2_wtpd_setup(FuDevice *device, GError **error)
{
	FuDevice *proxy = fu_device_get_proxy(device);
	g_autofree const gchar *devname = NULL;
	guint32 wtpd_version;
	guint8 devtype = DELL_DOCK2_EC_DEV_TYPE_WTPD;
	FuDellDock2BaseType dock_type = fu_dell_dock2_ec_get_dock_type(proxy);
	guint8 dock_sku = fu_dell_dock2_ec_get_dock_sku(proxy);
	guint8 dev_type = DELL_DOCK2_EC_DEV_TYPE_WTPD;

	/* name */
	devname = g_strdup_printf("%s", fu_dell_dock2_ec_devicetype_to_str(devtype, 0, 0));
	...
	/* version */
	wtpd_version = fu_dell_dock2_ec_get_wtpd_version(proxy);
...
static gboolean
fu_dell_dock2_wtpd_write(FuDevice *device,
			 FuFirmware *firmware,
			 FuProgress *progress,
			 ...
			 GError **error)
{
	...
	fw_whdr = fu_dell_dock2_hid_fwup_pkg_new(fw, DELL_DOCK2_EC_DEV_TYPE_WTPD, 0);
...
static void
fu_dell_dock2_wtpd_set_progress(FuDevice *self, FuProgress *progress)
{
...
static void
fu_dell_dock2_wtpd_init(FuDellDock2Wtpd *self)
{
...
static void
fu_dell_dock2_wtpd_class_init(FuDellDock2WtpdClass *klass)
{
	FuDeviceClass *device_class = FU_DEVICE_CLASS(klass);
	device_class->write_firmware = fu_dell_dock2_wtpd_write;
	device_class->setup = fu_dell_dock2_wtpd_setup;
	device_class->set_progress = fu_dell_dock2_wtpd_set_progress;
	device_class->convert_version = fu_dell_dock2_wtpd_convert_version;
}
...
fu_dell_dock2_wtpd_new(FuDevice *proxy)
{
	...
	FuDellDock2Wtpd *self = NULL;
	self = g_object_new(FU_TYPE_DELL_DOCK2_WTPD, "context", ctx, NULL);
