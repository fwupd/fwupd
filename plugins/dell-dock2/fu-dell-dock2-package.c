...
#define FU_DELL_DOCK2_PACKAGE_VERSION_OFFSET 0x14

struct _FuDellDock2Package {
	FuDevice parent_instance;
	guint64 blob_version_offset;
};

G_DEFINE_TYPE(FuDellDock2Package, fu_dell_dock2_package, FU_TYPE_DEVICE)

static gchar *
fu_dell_dock2_package_convert_version(FuDevice *device, guint64 version_raw)
{
	return fu_hex_version_from_uint32(version_raw, fu_device_get_version_format(device));
}

static gboolean
fu_dell_dock2_package_setup(FuDevice *device, GError **error)
{
	...
	/* setup version */
	pkg_version_raw = fu_dell_dock2_ec_get_package_version(proxy);
...
static gboolean
fu_dell_dock2_package_write(FuDevice *device,
			    FuFirmware *firmware,
			    FuProgress *progress,
			    ...
			    GError **error)
{
	...
	if (!fu_memcpy_safe((guint8 *)&status_version,
			    ...
			    data,
			    length,
			    FU_DELL_DOCK2_PACKAGE_VERSION_OFFSET,
			    sizeof(status_version),
			    error))
		return FALSE;
	dynamic_version =
	    fu_hex_version_from_uint32(status_version, fu_device_get_version_format(device));
	g_info("writing package status version %s", dynamic_version);

	if (!fu_dell_dock2_ec_commit_package(proxy, fw, error))
		return FALSE;
...
static void
fu_dell_dock2_package_finalize(GObject *object)
{
	G_OBJECT_CLASS(fu_dell_dock2_package_parent_class)->finalize(object);
}

static void
fu_dell_dock2_package_set_progress(FuDevice *self, FuProgress *progress)
{
...
static void
fu_dell_dock2_package_init(FuDellDock2Package *self)
{
...
static void
fu_dell_dock2_package_class_init(FuDellDock2PackageClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);
	FuDeviceClass *device_class = FU_DEVICE_CLASS(klass);
	object_class->finalize = fu_dell_dock2_package_finalize;
	device_class->write_firmware = fu_dell_dock2_package_write;
	device_class->setup = fu_dell_dock2_package_setup;
	device_class->set_progress = fu_dell_dock2_package_set_progress;
	device_class->convert_version = fu_dell_dock2_package_convert_version;
}
...
fu_dell_dock2_package_new(FuDevice *proxy)
{
	...
	FuDellDock2Package *self = NULL;
	self = g_object_new(FU_TYPE_DELL_DOCK2_PACKAGE, "context", ctx, NULL);
