				  ...
				  GError **error)
{
	...
	/* progress -- in reality this depends on the unit, e.g. A1=0+100, A2=35+65, AB=0+100 */
	fu_progress_set_id(progress, G_STRLOC);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_BUSY, 35, NULL);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 65, "main");
	...
	if (!fu_byte_array_append_safe(
		...
		buf,
		bufsz,
		fu_struct_himax_tp_hid_fw_unit_get_bin_start_offset(st_unit) * FU_KiB(1),
		fu_struct_himax_tp_hid_fw_unit_get_bin_size(st_unit) * FU_KiB(1),
		error))
		return FALSE;
	if (!fu_himax_tp_hid_device_set_feature(self,
				     ...
				     GError **error)
{
	...
	/* progress */
	fu_progress_set_id(progress, G_STRLOC);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 1, "initial-handshake-unlock");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_RESTART, 1, NULL);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 86, "main");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 12, "commit");
				      ...
				      GError **error)
{
	...
	/* progress */
	fu_progress_set_id(progress, G_STRLOC);
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 87, "main");
	fu_progress_add_step(progress, FWUPD_STATUS_DEVICE_WRITE, 13, "bootloader");
