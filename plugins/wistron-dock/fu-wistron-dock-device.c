...
fu_wistron_dock_device_parse_wdit_img(FuWistronDockDevice *self,
				      ...
				      GError **error)
{
	FuStruct *st = fu_struct_lookup(self, "WistronDockWditImg");
...
{
	FuStruct *st = fu_struct_lookup(self, "WistronDockWdit");
	...
	/* unpack */
	if (!fu_struct_unpack_full(st, buf, sizeof(buf), 0x0, FU_STRUCT_FLAG_NONE, error))
		return FALSE;
	if (fu_struct_get_u16(st, "tag_id") != FU_WISTRON_DOCK_WDIT_TAG_ID) {
		g_set_error(error,
			    G_IO_ERROR,
			    G_IO_ERROR_INVALID_DATA,
			    "WDIT tag invalid, expected 0x%x, got 0x%x",
			    (guint)FU_WISTRON_DOCK_WDIT_TAG_ID,
			    fu_struct_get_u16(st, "tag_id"));
		return FALSE;
	}

	/* verify VID & PID */
	if (fu_struct_get_u16(st, "vid") != fu_usb_device_get_vid(FU_USB_DEVICE(self)) ||
	    fu_struct_get_u16(st, "pid") != fu_usb_device_get_pid(FU_USB_DEVICE(self))) {
		g_set_error(error,
			    G_IO_ERROR,
			    G_IO_ERROR_INVALID_DATA,
			    ...
			    fu_struct_get_u16(st, "vid"),
			    fu_struct_get_u16(st, "pid"));
		return FALSE;
	}

	/* image mode */
	self->imgmode = fu_struct_get_u8(st, "imgmode");
	if (self->imgmode == 0)
	...
	/* update state */
	update_state = fu_struct_get_u8(st, "update_state");
	...
	/* status code */
	self->status_code = fu_struct_get_u8(st, "status_code");
	...
	fu_device_set_version_from_uint32(FU_DEVICE(self),
					  fu_struct_get_u32(st, "composite_version"));

	/* for debugging only */
	if (!fu_wistron_dock_device_parse_wdit_img(self,
						   buf,
						   sizeof(buf),
						   fu_struct_size(st) + 0x1,
						   MIN(fu_struct_get_u8(st, "device_cnt"), 32),
						   error)) {
