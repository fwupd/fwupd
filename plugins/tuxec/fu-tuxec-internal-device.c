...
struct _FuTuxecInternalDevice {
	FuTuxecDevice		 parent_instance;
};

G_DEFINE_TYPE (FuTuxecInternalDevice, fu_tuxec_internal_device,
	       FU_TYPE_TUXEC_DEVICE)

static void
fu_tuxec_internal_device_init (FuTuxecInternalDevice *self)
{
	...
	fu_device_set_physical_id (FU_DEVICE (self), "tuxec");
...
static gboolean
fu_tuxec_internal_device_prepare (FuDevice *device,
				     ...
				     GError **error)
{
	...
	if (!g_file_test (firmware_orig, G_FILE_TEST_EXISTS)) {
		FuTuxecDevice *parent = FU_TUXEC_DEVICE (device);
		struct flashrom_flashctx *flashctx = fu_tuxec_device_get_flashctx (parent);
		gsize flash_size = fu_tuxec_device_get_flash_size (parent);
...
static gboolean
fu_tuxec_internal_device_write_firmware (FuDevice *device,
					    FuFirmware *firmware,
					    FwupdInstallFlags flags,
					    GError **error)
{
...
static void
fu_tuxec_internal_device_class_init (FuTuxecInternalDeviceClass *klass)
{
	FuDeviceClass *klass_device = FU_DEVICE_CLASS (klass);
	klass_device->prepare = fu_tuxec_internal_device_prepare;
	klass_device->write_firmware = fu_tuxec_internal_device_write_firmware;
}

FuDevice *
fu_tuxec_internal_device_new (void)
{
	return FU_DEVICE (g_object_new (FU_TYPE_TUXEC_INTERNAL_DEVICE, NULL));
}
