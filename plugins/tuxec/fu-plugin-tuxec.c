...
#include "fu-tuxec-internal-device.h"
...
static int
fu_plugin_tuxec_debug_cb (enum flashrom_log_level lvl, const char *fmt, va_list args)
{
...
#pragma clang diagnostic pop
	...
	case FLASHROM_MSG_DEBUG2:
		if (g_getenv ("FWUPD_TUXEC_VERBOSE") != NULL)
			g_debug ("%s", str);
		break;
...
static void
fu_plugin_tuxec_device_set_version (FuPlugin *plugin, FuDevice *device)
{
	FuContext *ctx = fu_plugin_get_context (plugin);
	...
	if (version_major != NULL && version_minor != NULL) {
		g_autofree gchar *tmp = g_strdup_printf ("1.%s.%s",
							 version_major,
							 version_minor);
...
static void
fu_plugin_tuxec_device_set_bios_info (FuPlugin *plugin, FuDevice *device)
{
	FuContext *ctx = fu_plugin_get_context (plugin);
	const guint8 *buf;
	gsize bufsz;
...
static void
fu_plugin_tuxec_device_set_hwids (FuPlugin *plugin, FuDevice *device)
{
	FuContext *ctx = fu_plugin_get_context (plugin);
...
fu_plugin_coldplug (FuPlugin *plugin, GError **error)
{
	...
	g_autoptr(FuDevice) device = fu_tuxec_internal_device_new ();

	fu_device_set_context (device, ctx);
	fu_device_set_name (device, fu_context_get_hwid_value (ctx, "Embedded Controller"));
	...
	fu_plugin_tuxec_device_set_version (plugin, device);
	fu_plugin_tuxec_device_set_hwids (plugin, device);
	fu_plugin_tuxec_device_set_bios_info (plugin, device);
	fu_tuxec_device_set_programmer_name (FU_TUXEC_DEVICE (device), "tuxec");
...
fu_plugin_startup (FuPlugin *plugin, GError **error)
{
	...
	fu_plugin_add_flag (plugin, FWUPD_PLUGIN_FLAG_UNLOCK_REQUIRED);
	flashrom_set_log_callback (fu_plugin_tuxec_debug_cb);
	return TRUE;
}
