...
static gboolean
fu_st_stm32_device_send_command(FuStStm32Device *self, FuStStm32Cmd cmd_base, GError **error)
{
	FuStStm32Cmd cmd = 0;
	guint8 buf[2] = {cmd, cmd ^ 0xFF};
...
fu_st_stm32_device_ensure_version_bootloader(FuStStm32Device *self, GError **error)
{
	...
	if (!fu_st_stm32_device_send_command(self, FU_ST_STM32_CMD_GET_VERSION, error))
		return FALSE;
...
fu_st_stm32_device_inc_variable_length(FuStStm32Device *self, gsize *bufsz, GError **error)
{
	...
	if (!fu_st_stm32_device_send_command(self, FU_ST_STM32_CMD_GET, error))
		return FALSE;
	if (!fu_i2c_device_read(FU_I2C_DEVICE(self), buf, sizeof(buf), error))
		return FALSE;
	if (bufsz != NULL)
...
fu_st_stm32_device_ensure_chip_id(FuStStm32Device *self, GError **error)
{
	...
	if (!fu_st_stm32_device_send_command(self, FU_ST_STM32_CMD_GET_ID, error))
		return FALSE;
	if (!fu_i2c_device_read(FU_I2C_DEVICE(self), buf, sizeof(buf), error))
		return FALSE;
	if (!fu_st_stm32_device_get_ack(self, error))
...
static gboolean
fu_st_stm32_device_stm32_go(FuStStm32Device *self, guint32 address, GError **error)
{
	g_autoptr(FuStructStStm32Addr) st = fu_struct_st_stm32_addr_new();

	if (!fu_st_stm32_device_send_command(self, FU_ST_STM32_CMD_GO, error))
		return FALSE;
...
static gboolean
fu_st_stm32_device_stm32_run_raw_code(FuStStm32Device *self,
				      ...
				      GError **error)
{
	...
	return fu_st_stm32_device_stm32_go(self, target_address, error);
}
...
fu_st_stm32_device_reset(FuStStm32Device *self, GError **error)
{
	...
	if (fu_device_has_private_flag(FU_DEVICE(self),
		...
		return fu_st_stm32_device_stm32_run_raw_code(self,
							     self->sram_addr,
							     code_obll,
							     sizeof(code_obll),
							     error);
				  ...
				  GError **error)
{
	...
	if (fu_device_has_private_flag(device, FU_ST_STM32_DEVICE_FLAG_NO_MASS_ERASE))
		self->mem_addr = 0x100;
