...
static guint8
fu_blestechtp_hid_device_calc_checksum(const guint8 *buf, gsize bufsz)
{
	guint8 checksum = 0;
				    ...
				    GError **error)
{
	...
	if (wbuf != NULL && wbufsz > 0) {
		g_autoptr(FuBlestechtpSetHdr) set_hdr = fu_blestechtp_set_hdr_new();
		guint8 checksum = 0;

		fu_blestechtp_set_hdr_set_report_id(set_hdr, REPORT_ID);
		fu_blestechtp_set_hdr_set_pack_len(set_hdr, wbufsz + SET_REPORT_PACK_FIX_SIZE);
		fu_blestechtp_set_hdr_set_write_len(set_hdr, wbufsz);
		fu_blestechtp_set_hdr_set_read_len(set_hdr, rbufsz);
		if (!fu_blestechtp_set_hdr_set_data(set_hdr, wbuf, wbufsz, error))
			return FALSE;

		/* checksum */
		checksum = fu_blestechtp_hid_device_calc_checksum(
		    set_hdr->buf->data + FU_BLESTECHTP_SET_HDR_OFFSET_FRAME_FLAG,
		    (gsize)(set_hdr->buf->data[FU_BLESTECHTP_SET_HDR_OFFSET_PACK_LEN] - 1));
		fu_blestechtp_set_hdr_set_checksum(set_hdr, checksum);
...
static gboolean
fu_blestechtp_hid_device_get_ver(FuBlestechtpHidDevice *self, guint8 *fw_ver, GError **error)
{
	guint8 fw_cmd = FU_BLESTECHTP_CMD_GET_FW_VER;

	if (!fu_blestechtp_hid_device_write_read(self, &fw_cmd, 1, fw_ver, FW_VER_LEN, 0, error)) {
...
static gboolean
fu_blestechtp_hid_device_update_start(FuBlestechtpHidDevice *self, GError **error)
{
	g_autoptr(FuBlestechtpUpdateStart) st = fu_blestechtp_update_start_new();
	guint8 ret_buff[2] = {0};
...
static gboolean
fu_blestechtp_hid_device_switch_boot(FuBlestechtpHidDevice *self, GError **error)
{
	guint8 fw_ver[FW_VER_LEN] = {0};
	g_autoptr(FuBlestechtpSwitchBoot) st = fu_blestechtp_switch_boot_new();
...
static gboolean
fu_blestechtp_hid_device_program_page_cb(FuDevice *device, gpointer user_data, GError **error)
{
	...
	for (guint16 i = 0; i < chk_sz; i += PROGRAM_PACK_LEN) {
		...
		if (!fu_blestechtp_hid_device_write_read(self,
			...
			g_prefix_error(error,
				       "addr:%08X program failed: ",
				       (guint16)(addr + i * chk_sz));
			return FALSE;
		}
	...
	checksum = fu_blestechtp_hid_device_calc_checksum(chk_data, chk_sz);
	page = addr / chk_sz;
	cmd_buf[1] = page;
				 ...
				 GError **error)
{
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		if (i < boot_pack_nums || i == APP_CONFIG_PAGE)
			continue;
		if (!fu_blestechtp_hid_device_program_chunk(self, chunks, i, error)) {
	...
	if (!fu_blestechtp_hid_device_program_chunk(self, chunks, APP_CONFIG_PAGE, error)) {
		g_prefix_error(error, "Program Page %d Failed: ", APP_CONFIG_PAGE);
		return FALSE;
	}
	fu_progress_step_done(progress);
...
fu_blestechtp_hid_device_program_finish(FuBlestechtpHidDevice *self,
					...
					GError **error)
{
	FuBlestechtpFirmware *blestechtp_firmware = FU_BLESTECHTP_FIRMWARE(firmware);
	guint16 calc_checksum;
	guint16 bin_checksum = fu_blestechtp_firmware_get_checksum(blestechtp_firmware);
	guint8 cmd_buf[4] = {FU_BLESTECHTP_CMD_PROGRAM_CHECKSUM,
			     (guint8)bin_checksum,
			     (guint8)(bin_checksum >> 8),
			     0};
	guint8 read_buf[4] = {0};
	...
	if (!fu_blestechtp_hid_device_write_read(self, cmd_buf, 1, NULL, 0, 0, error)) {
					...
					GError **error)
{
	...
	/* write image */
	if (!fu_blestechtp_hid_device_program(self,
					      firmware,
