...
fu_ti_tps6598x_pd_device_ensure_version(FuTiTps6598xPdDevice *self, GError **error)
{
	...
	if (proxy == NULL)
		return FALSE;
	buf = fu_ti_tps6598x_device_read_target_register(FU_TI_TPS6598X_DEVICE(proxy),
							 self->target,
					...
					GError **error)
{
	...
	if (proxy == NULL)
		return FALSE;
	return fu_ti_tps6598x_device_write_firmware(proxy, firmware, progress, flags, error);
}
