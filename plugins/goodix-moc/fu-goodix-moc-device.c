			      ...
			      GError **error)
{
	...
	crc_hdr = ~fu_crc8(FU_CRC_KIND_B8_STANDARD, st->buf->data, st->buf->len);
	...
	while (1) {
		...
		crc_actual =
		    fu_crc32(FU_CRC_KIND_B32_STANDARD,
			     reply->data,
			     FU_STRUCT_GOODIX_MOC_PKG_HEADER_SIZE + header_len - GX_SIZE_CRC32);
		crc_calculated = fu_struct_goodix_moc_pkg_header_get_crc8(st);
		...
		g_byte_array_append(res,
				    st->buf->data + FU_STRUCT_GOODIX_MOC_PKG_HEADER_SIZE,
				    st->buf->len - FU_STRUCT_GOODIX_MOC_PKG_HEADER_SIZE);
