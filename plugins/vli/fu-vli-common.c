...
gboolean
fu_vli_common_i2c_status_check (FuVliDeviceI2cStatus status, GError **error)
{
	if (status == FU_VLI_DEVICE_I2C_STATUS_OK)
		return TRUE;
	if (status == FU_VLI_DEVICE_I2C_STATUS_HEADER) {
	...
	if (status == FU_VLI_DEVICE_I2C_STATUS_COMMAND) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_INTERNAL,
	...
	if (status == FU_VLI_DEVICE_I2C_STATUS_ADDRESS) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_INTERNAL,
	...
	if (status == FU_VLI_DEVICE_I2C_STATUS_PACKETSIZE) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_INTERNAL,
	...
	if (status == FU_VLI_DEVICE_I2C_STATUS_CHECKSUM) {
		g_set_error_literal (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_INTERNAL,
