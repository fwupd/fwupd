...
fu_telink_dfu_ble_device_create_dfu_packet(guint16 preamble, const guint8 *buf, gsize bufsz)
{
				      ...
				      GError **error)
{
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		if (chk == NULL)
			return FALSE;
		pkt = fu_telink_dfu_ble_device_create_dfu_packet((guint16)(i >> 4),
...
fu_telink_dfu_ble_device_ota_start(FuTelinkDfuBleDevice *self, GError **error)
{
	...
	pkt = fu_telink_dfu_ble_device_create_dfu_packet(FU_TELINK_DFU_CMD_OTA_START, NULL, 0);
	if (!fu_bluez_device_write(FU_BLUEZ_DEVICE(self),
				   FU_TELINK_DFU_BLE_DEVICE_UUID_OTA,
				   pkt,
				   error))
...
fu_telink_dfu_ble_device_ota_stop(FuTelinkDfuBleDevice *self, guint number_chunks, GError **error)
{
	...
	pkt = fu_telink_dfu_ble_device_create_dfu_packet(FU_TELINK_DFU_CMD_OTA_END,
							 pkt_stop_data,
				    ...
				    GError **error)
{
	...
	// FIXME: why?
	pkt = fu_telink_dfu_ble_device_create_dfu_packet(FU_TELINK_DFU_CMD_OTA_FW_VERSION, NULL, 0);
	if (!fu_bluez_device_write(FU_BLUEZ_DEVICE(self),
				   FU_TELINK_DFU_BLE_DEVICE_UUID_OTA,
				   pkt,
				   error))
...
{
	self->start_addr = FU_TELINK_DFU_HID_DEVICE_START_ADDR;
