				...
				GError **error)
{
	...
	filename = fwupd_json_object_get_string(json_object, "file", error);
	if (filename == NULL)
		return FALSE;
	...
	bootloader_name = fwupd_json_object_get_string(json_object, "bootloader_type", error);
	...
	board_name = fwupd_json_object_get_string(json_object, "board", error);
	if (board_name == NULL)
		return FALSE;
	...
	version = fwupd_json_object_get_string(json_object, "image_version", error);
	if (version == NULL)
		return FALSE;
			    ...
			    GError **error)
{
	...
	if (json_object == NULL)
		return FALSE;
	if (!fwupd_json_object_has_node(json_object, "format-version")) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_FILE,
	...
	if (!fwupd_json_object_get_integer_with_default(json_object, "format-version", &manifest_ver, 0, error))
		return FALSE;
