...
static const gchar *
fu_nordic_hid_archive_parse_file_get_bootloader_name(FwupdJsonObject *json_object, GError **error)
{
	if (fwupd_json_object_has_node(json_object, "version_B0"))
		return "B0";
	if (fwupd_json_object_has_node(json_object, "version_MCUBOOT"))
		return "MCUBOOT";
	if (fwupd_json_object_has_node(json_object, "version_MCUBOOT+XIP"))
		return "MCUBOOT+XIP";
...
static gchar *
fu_nordic_hid_archive_parse_file_get_board_name(FwupdJsonObject *json_object,
						gint64 manifest_ver,
						GError **error)
{
	...
	board_name_readout = fwupd_json_object_get_string(json_object, "board", error);
...
static gboolean
fu_nordic_hid_archive_parse_file_get_flash_area_id_v1(FwupdJsonObject *json_object,
						      guint *flash_area_id,
						      ...
						      GError **error)
{
	...
	image_idx_str = fwupd_json_object_get_string(json_object, "image_index", error);
	...
	slot_str = fwupd_json_object_get_string(json_object, "slot", error);
	if (slot_str == NULL)
		return FALSE;
			    ...
			    GError **error)
{
	...
	if (!fwupd_json_object_get_integer(json_object, "format-version", &manifest_ver, error))
		return FALSE;
	...
	json_files = fwupd_json_object_get_array(json_object, "files", error);
	if (json_files == NULL)
		return FALSE;
	...
	for (guint i = 0; i < files_cnt; i++) {
		...
		json_object_tmp = fwupd_json_array_get_object(json_files, i, error);
		if (json_object_tmp == NULL)
			return FALSE;
		filename = fwupd_json_object_get_string(json_object_tmp, "file", error);
		if (filename == NULL) {
		...
		bootloader_name =
		    fu_nordic_hid_archive_parse_file_get_bootloader_name(json_object_tmp, error);
		...
		board_name = fu_nordic_hid_archive_parse_file_get_board_name(json_object_tmp,
									     manifest_ver,
									     error);
		if (board_name == NULL)
			return FALSE;

		if (!fu_nordic_hid_archive_parse_file_get_flash_area_id(json_object_tmp,
									&flash_area_id,
									manifest_ver,
									i,
									bootloader_name,
		...
		if (!fwupd_json_object_get_integer_with_default(json_object_tmp,
								"load_address",
								&image_addr,
								-1,
								error))
