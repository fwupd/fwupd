...
struct _FuFocalMocTransport {
	FuFocalMocTransportSendFunc send_cb;
	FuFocalMocTransportReceiveFunc receive_cb;
	gpointer user_data;
	FuFocalMocTransportState state;
	FuFocalMocTransportKeyLoadFunc key_load_cb;
	FuFocalMocTransportKeySaveFunc key_save_cb;
	guint32 tx_sequence;
	guint32 rx_sequence;
...
static void
fu_focal_moc_transport_clear(gpointer data, gsize data_sz)
{
	volatile guint8 *ptr = data;
						 ...
						 GError **error)
{
	...
	if (plaintext_sz == 0 ||
		...
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
			    "cipher plaintext size invalid: 0x%x",
			    (guint)plaintext_sz);
		return NULL;
	}
						   ...
						   GError **error)
{
	...
	if (gnutls_memcmp(mac, frame->data + mac_offset, sizeof(mac)) != 0) {
		self->mac_failures =
		    MIN((guint)self->mac_failures + 1, FU_FOCAL_MOC_TRANSPORT_MAC_FAILURE_LIMIT);
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_AUTH_FAILED,
			    "cipher MAC mismatch: failures=%u",
			    self->mac_failures);
		fu_focal_moc_transport_clear(mac, sizeof(mac));
	...
	if (sequence != self->rx_sequence) {
		...
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_AUTH_FAILED,
			    "cipher sequence invalid: got=%u expected=%u failures=%u",
			    sequence,
			    self->rx_sequence,
			    self->mac_failures);
		fu_focal_moc_transport_clear(mac, sizeof(mac));
	...
	if (!fu_focal_moc_transport_derive_iv_internal(self->key_iv_m2d, sequence, iv, error)) {
		fu_focal_moc_transport_clear(mac, sizeof(mac));
		return NULL;
	}
	decrypted = g_malloc0(encrypted_sz);
	if (!fu_focal_moc_transport_aes_ctr_internal(self->key_aes_m2d,
						     iv,
						     frame->data +
...
fu_focal_moc_transport_key_exchange(FuFocalMocTransport *self, GError **error)
{
		...
		return FALSE;
	{
		...
		guint8 host_key[FU_FOCAL_MOC_TRANSPORT_HOST_KEY_SIZE] = {0};
		...
		if (self->key_load_cb != NULL &&
		    !self->key_load_cb(self->user_data, host_key, &have_recorded, error)) {
		...
		if (have_recorded) {
			...
			if (self->key_save_cb != NULL) {
				...
				if (!fu_focal_moc_transport_gnutls_error(
					...
					error))
					return FALSE;
				if (!fu_focal_moc_transport_pack_coordinate(host_key,
									    &key_x,
									    error) ||
				    !fu_focal_moc_transport_pack_coordinate(host_key + coord,
									    &key_y,
									    error) ||
				    !fu_focal_moc_transport_pack_coordinate(host_key + (2 * coord),
									    &key_k,
									    error))
					return FALSE;
				if (!self->key_save_cb(self->user_data, host_key, error)) {
					fu_focal_moc_transport_clear(host_key, sizeof(host_key));
					return FALSE;
				}
				fu_focal_moc_transport_clear(host_key, sizeof(host_key));
	...
	request = fu_focal_moc_packet_new(FU_FOCAL_MOC_CMD_TRANSPORT_KEY_EXCHANGE,
					  ...
					  sizeof(self->public_key_host),
					  error);
	if (request == NULL || !self->send_cb(self->user_data,
					      request->data,
					      request->len,
					      FU_FOCAL_MOC_TRANSPORT_KEX_TIMEOUT_MS,
					      error))
		return FALSE;
	response = self->receive_cb(self->user_data, FU_FOCAL_MOC_TRANSPORT_KEX_TIMEOUT_MS, error);
	if (response == NULL)
		return FALSE;
...
fu_focal_moc_transport_key_confirm(FuFocalMocTransport *self, GError **error)
{
	...
	if (frame == NULL)
		return FALSE;
	if (!self->send_cb(self->user_data,
			   frame->data,
			   frame->len,
		...
		return FALSE;
	self->tx_sequence++;
	response = self->receive_cb(self->user_data, FU_FOCAL_MOC_TRANSPORT_KC_TIMEOUT_MS, error);
	if (response == NULL)
		return FALSE;
	response_plaintext =
				       ...
				       GError **error)
{
	...
	for (guint chunk = 0;; chunk++) {
		...
		frame = self->receive_cb(self->user_data, timeout_ms, error);
		if (frame == NULL)
			return NULL;
					  ...
					  GError **error)
{
	...
	if (!fu_memcpy_safe(transport.key_aes_m2d,
			    sizeof(transport.key_aes_m2d),
			    0,
			    key_aes,
			    sizeof(transport.key_aes_m2d),
			    0,
			    sizeof(transport.key_aes_m2d),
			    error) ||
	    !fu_memcpy_safe(transport.key_mac_m2d,
			    sizeof(transport.key_mac_m2d),
			    0,
			    key_mac,
			    sizeof(transport.key_mac_m2d),
			    0,
			    sizeof(transport.key_mac_m2d),
			    error) ||
	    !fu_memcpy_safe(transport.key_iv_m2d,
			    sizeof(transport.key_iv_m2d),
			    0,
			    key_iv,
			    sizeof(transport.key_iv_m2d),
			    0,
	...
	ret = fu_focal_moc_transport_cipher_frame_parse_internal(&transport, frame, error);
	fu_focal_moc_transport_teardown(&transport);
	return ret;
#else
	g_set_error_literal(error,
			   ...
			   gpointer user_data)
{
	...
	g_return_val_if_fail(send_cb != NULL, NULL);
	g_return_val_if_fail(receive_cb != NULL, NULL);
...
{
	g_return_if_fail(self != NULL);
	self->key_load_cb = load_cb;
	self->key_save_cb = save_cb;
}
...
fu_focal_moc_transport_handshake(FuFocalMocTransport *self, GError **error)
{
	...
	for (guint i = 0; i < FU_FOCAL_MOC_TRANSPORT_DRAIN_LIMIT; i++) {
		...
		stale = self->receive_cb(self->user_data,
			       ...
			       GError **error)
{
	...
	if ((payload_sz > 0 && payload == NULL) ||
		...
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_INVALID_DATA,
			    "encrypted command payload invalid: 0x%x",
			    (guint)payload_sz);
		return NULL;
	}
	...
	if (!self->send_cb(self->user_data, frame->data, frame->len, timeout_ms, error)) {
		fu_focal_moc_transport_teardown(self);
		return NULL;
	}
	self->tx_sequence++;
