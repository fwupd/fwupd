...
*/
typedef void(*BulkErrorCallback)(gint error_code, BulkInterface bulk_int, const gchar *data, guint32 size,
                                                                                                void *user_data);
...
*        The folder location of file should be the same folder set indicated in SetFileTransferLocation
* @param size - size of byte array data.
* @param state - the state of file transfer- either one of the following
                                                      transfer_started|transfer_faield|transfer_complete
*/
typedef void(*BulkFileTransferCallback)(FileTransferState state, gint progress, BulkInterface bulk_intf,
                                                                                                void *user_data);

/**
* @brief Defines the callback signature to send data over sync
*/
typedef void (*SendDataSyncCallback)(gint error_code, gint status, gint transaction_id, void *user_data);
...
////Public Methods
GType                 logibulkcontroller_get_type                     (void);
...
*/
LogiBulkController*   logibulkcontroller_create_bulk_controller       (gint vid,
                                                                       gint pid,
...
* status of device
*/
gint                  logibulkcontroller_open_device                  (LogiBulkController *logibulkcontroller);
...
*/
gint                  logibulkcontroller_close_device                 (LogiBulkController *logibulkcontroller);

/**
* @brief Method for sending file using UPD bulk interface
* @param file_data - file data to send
* @param size - size of byte array data.
* @param start_update - to enable/disable firmware update upon file transfer. By default set to false
*/
gint                  logibulkcontroller_send_file_upd                (LogiBulkController *logibulkcontroller,
                                                                       GBytes* file_data,
                                                                       gsize size,
...
* @param size - size of byte array data.
*@return : retunns a pair of values. 1st value is transaction ID of the request and 2nd value is the error code.
*/
ReturnValue*          logibulkcontroller_send_data_sync               (LogiBulkController *logibulkcontroller,
                                                                       const gchar *data,
                                                                       guint32 size);
#endif
