...
#define LOGFN   g_debug("Method : %s", __func__); //Testing Purpose
...
#define LOG_ERROR(id, error_code) std::make_pair(id, error_code)

#define VARIABLE_BUFFER_SIZE(size)            size

#define PAYLOAD_SIZE_SYNC(size) size - PACKET_HEADER_SIZE - sizeof(gint)

#define LOCK(cond, mutex) g_cond_wait_until (cond, mutex, g_get_monotonic_time () + TIME_OUT_5S * G_TIME_SPAN_SECOND)

#define UNLOCK(cond) g_cond_signal(cond)

#define CREATE_PACKET(packet, code, size, id, data) \
                                                     memset(packet, 0, sizeof(UsbPacketSync)); \
                                                     (packet)->cmd = code; \
                                                     (packet)->length = size; \
                                                     (packet)->sequence_id = id; \
                                                     if (data != NULL) { \
