...
{
	if (!fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DOSCI, error))
		return FALSE;
	if (!fu_superio_device_ec_write (self, val, error))
		return FALSE;
	return TRUE;
}
...
{
	if (!fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DOCMI, error))
		return FALSE;
	if (!fu_superio_device_ec_write (self, val, error))
		return FALSE;
	return TRUE;
...
fu_superio_device_ec_read_status (FuSuperioDevice *self, GError **error)
{
	...
	if (!fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DO, error))
		return FALSE;
	...
	/* wait for write */
	do {
		if (!fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DI, error))
			return FALSE;
	...
	/* watch SCI events */
	return fu_superio_device_ec_write (self,   SIO_EC_PMC_PM1DISCI, error);
}
...
fu_superio_it89_device_erase_addr (FuSuperioDevice *self, guint addr, GError **error)
{
	...
	/* watch SCI events */
	if (!fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DISCI, error))
		return FALSE;
	return fu_superio_device_ec_read_status (self, error);
}
...
fu_superio_it89_device_attach (FuDevice *device, GError **error)
{
	...
	if (!fu_superio_device_ec_write (self, SIO_CMD_EC_ENABLE_HOST_WA, error))
		return FALSE;
...
fu_superio_it89_device_detach (FuDevice *device, GError **error)
{
	...
	if (!fu_superio_device_ec_write (self, SIO_CMD_EC_DISABLE_HOST_WA, error))
		return FALSE;
	if (!fu_superio_device_ec_read (self, &tmp, error))
		return FALSE;
	if (tmp != 0x33) {
...
fu_superio_it89_device_get_jedec_id (FuSuperioDevice *self, guint8 *id, GError **error)
{
	...
	if (!fu_superio_it89_device_ec_pm1do_sci (self, SIO_SPI_CMD_JEDEC_ID, error))
		return FALSE;
	if (!fu_superio_device_ec_read (self, &id[0], error))
		return FALSE;
	if (!fu_superio_device_ec_read (self, &id[1], error))
		return FALSE;
	if (!fu_superio_device_ec_read (self, &id[2], error))
		return FALSE;
	if (!fu_superio_device_ec_read (self, &id[3], error))
		return FALSE;
	return fu_superio_device_ec_write (self, SIO_EC_PMC_PM1DISCI, error);
}
