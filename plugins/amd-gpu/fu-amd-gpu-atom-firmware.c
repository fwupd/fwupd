					     ...
					     GError **error)
{
	...
	if (!fu_size_checked_inc(&offset, sizeof(BIOS_VERSION_PREFIX) - 1, error)) {
		g_prefix_error_literal(error, "AMD GPU ATOM BIOS version offset overflow: ");
		return FALSE;
	}
					...
					GError **error)
{
	...
	for (i = 0; i < num_str; i++) {
		...
		str = fu_memstrsafe(buf, bufsz, idx, STRLEN_NORMAL - 1, error);
		if (str == NULL)
			return FALSE;
	...
	model = fu_memstrsafe(buf, bufsz, idx, STRLEN_LONG - 1, error);
	if (model == NULL)
		return FALSE;
...
fu_amd_gpu_atom_firmware_parse_config_filename(FuAmdGpuAtomFirmware *self,
					       GBytes *blob,
					       FuStructAtomRom21Header *atom_header,
					       GError **error)
{
	gsize bufsz = 0;
	const guint8 *buf = g_bytes_get_data(blob, &bufsz);
