...
fu_uefi_dbx_snapd_notifier_init(FuUefiDbxSnapdNotifier *self)
{
	...
	curl_easy_setopt(self->curl_template, CURLOPT_UNIX_SOCKET_PATH, "/run/snapd-snap.socket");
				      ...
				      GError **error)
{
	...
	if (res != CURLE_OK) {
		...
		g_warning("failed to notify snapd: %s", curl_easy_strerror(res));
		return FALSE;
	}
...
fu_uefi_dbx_snapd_notifier_dbx_manager_startup(FuUefiDbxSnapdNotifier *self, GError **error)
{
	...
	g_debug("snapd notify secureboot manager startup");
					      ...
					      GError **error)
{
	...
	g_debug("snapd notify prepare, with %" G_GSIZE_FORMAT " bytes of data", bufsz);
