...
binary files from a zip archive payload. In this zip file there must be
two files:
...
The dock is then rebooted where the firmware is written, and then the
dock re-appears.
...
The PID to expect from the image, falling back to the device PID if not
set.
