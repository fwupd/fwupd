...
struct _FuLenovoDockChildDevice {
	FuDevice parent_instance;
	guint8 chip_idx;
};

G_DEFINE_TYPE(FuLenovoDockChildDevice, fu_lenovo_dock_child_device, FU_TYPE_DEVICE)
...
static void
fu_lenovo_dock_child_device_to_string(FuDevice *device, guint idt, GString *str)
{
	FuLenovoDockChildDevice *self = FU_LENOVO_DOCK_CHILD_DEVICE(device);
					   ...
					   GError **error)
{
	...
	if (!fu_device_prepare_firmware(parent, fw, flags, error))
		return NULL;
	return NULL;
}
					 ...
					 GError **error)
{
	...
	return fu_lenovo_dock_mcu_device_write_firmware_with_idx(FU_LENOVO_DOCK_MCU_DEVICE(parent),
								 firmware,
								 self->chip_idx,
								 progress,
								 flags,
...
static void
fu_lenovo_dock_child_device_init(FuLenovoDockChildDevice *self)
{
	fu_device_add_internal_flag(FU_DEVICE(self), FU_DEVICE_INTERNAL_FLAG_USE_PARENT_FOR_OPEN);
}

static void
fu_lenovo_dock_child_device_class_init(FuLenovoDockChildDeviceClass *klass)
{
	FuDeviceClass *klass_device = FU_DEVICE_CLASS(klass);
	klass_device->to_string = fu_lenovo_dock_child_device_to_string;
...
{
	return g_object_new(FU_TYPE_LENOVO_DOCK_CHILD_DEVICE, "context", ctx, NULL);
}
