...
fu_lenovo_dock_device_get_report_cb(FuDevice *device, gpointer user_data, GError **error)
{
	...
	fu_hid_device_set_interface(FU_HID_DEVICE(self),
				    buf->len == FU_LENOVO_DOCK_DEVICE_IFACE2_LEN ? 0x02 : 0x01);
...
static GByteArray *
fu_lenovo_dock_device_get_report(FuLenovoDockDevice *self,
				 gsize ifacesz,
				 guint retry_delay,
				 GError **error)
{
				 ...
				 GError **error)
{
	...
	fu_hid_device_set_interface(FU_HID_DEVICE(self),
				    buf2->len == FU_LENOVO_DOCK_DEVICE_IFACE2_LEN ? 0x02 : 0x01);
...
static GByteArray *
fu_lenovo_dock_device_txfer1(FuLenovoDockDevice *self,
			     GByteArray *buf,
			     guint retry_delay,
			     GError **error)
{
	if (!fu_lenovo_dock_device_set_report(self, buf, FU_LENOVO_DOCK_DEVICE_IFACE1_LEN, error))
		return NULL;
	return fu_lenovo_dock_device_get_report(self,
...
static GByteArray *
fu_lenovo_dock_device_txfer2(FuLenovoDockDevice *self,
			     GByteArray *buf,
			     guint retry_delay,
			     GError **error)
{
					      ...
					      GError **error)
{
	...
	buf = fu_lenovo_dock_device_txfer2(self, st_req->buf, FU_LENOVO_DOCK_DEVICE_DELAY, error);
	if (buf == NULL)
		return FALSE;
				  ...
				  GError **error)
{
	...
	buf = fu_lenovo_dock_device_txfer1(self, st_req->buf, FU_LENOVO_DOCK_DEVICE_DELAY, error);
	if (buf == NULL)
		return FALSE;
					...
					GError **error)
{
	...
	for (gsize i = 0; i < datasz; i += FU_STRUCT_LENOVO_DOCK_FLASH_READ_RES_N_ELEMENTS_DATA) {
		...
		buf = fu_lenovo_dock_device_txfer2(self,
						   st_req->buf,
					 ...
					 GError **error)
{
	...
	for (gsize i = 0; i < chunks->len; i++) {
		...
		if (g_log_get_debug_enabled()) {
			g_autofree gchar *str =
			    fu_struct_lenovo_dock_flash_erase_req_to_string(st_req);
			g_log("FuStruct", G_LOG_LEVEL_DEBUG, "%s", str);
		}
		buf = fu_lenovo_dock_device_txfer2(self,
						   st_req->buf,
					 ...
					 GError **error)
{
	...
	for (gsize i = 0; i < chunks->len; i++) {
		...
		if (g_log_get_debug_enabled()) {
			g_autofree gchar *str =
			    fu_struct_lenovo_dock_flash_program_req_to_string(st_req);
			g_log("FuStruct", G_LOG_LEVEL_DEBUG, "%s", str);
		}
		buf = fu_lenovo_dock_device_txfer2(self,
						   st_req->buf,
					     ...
					     GError **error)
{
	...
	buf = fu_lenovo_dock_device_txfer1(self,
					   st_req->buf,
					   FU_LENOVO_DOCK_DEVICE_DELAY_LONG,
					   error);
	if (buf == NULL)
		return FALSE;
	st_res =
