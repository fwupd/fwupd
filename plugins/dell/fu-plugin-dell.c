...
fu_dell_supported (FuPlugin *plugin)
{
	...
	if (!(da_values->supported_cmds & (1 << DACI_FLASH_INTERFACE_CLASS))) {
		g_debug ("unable to access flash interface. supported "
			 "commands: 0x%x",
...
fu_plugin_backend_device_added (FuPlugin *plugin, FuDevice *device, GError **error)
{
	...
	for (guint i = 0; i < dock_info->component_count; i++) {
		...
		fw_str = fu_common_version_from_uint32 (
		    dock_info->components[i].fw_version, version_format);
		if (!fu_plugin_dock_node (
			plugin, platform, buf.record->dock_info_header.dock_type,
			component_guid, component_name, fw_str, version_format)) {
	...
	if (old_ec)
		flash_ver_str = fu_common_version_from_uint32 (
		    dock_info->flash_pkg_version, version_format);
	if (!fu_plugin_dock_node (plugin, platform, buf.record->dock_info_header.dock_type,
				  DOCK_FLASH_GUID, NULL, flash_ver_str, version_format)) {
...
fu_plugin_dell_get_tpm_capability (ESYS_CONTEXT *ctx, guint32 query)
{
	...
	if (capability->data.tpmProperties.tpmProperty[0].property != query) {
		g_debug ("wrong query returned (got %x expected %x)",
			 capability->data.tpmProperties.tpmProperty[0].property, query);
		return NULL;
	}
...
fu_plugin_dell_detect_tpm (FuPlugin *plugin, GError **error)
{
	...
	if ((out->status & TPM_OWN_MASK) == 0 && out->flashes_left > 0) {
		...
		fu_device_set_update_error (dev, "Updating disabled due to TPM "
						 "ownership");
	}
	...
	fu_plugin_add_report_metadata (plugin, "TpmFamily",
				       fu_device_get_metadata (dev, "TpmFamil"
								    "y"));
	...
	if (can_switch_modes) {
		...
		if ((out->status & TPM_OWN_MASK) == 0 && out->flashes_left > 0) {
			...
			fu_device_set_update_error (dev_alt, "mode switch "
							     "disabled due to "
							     "TPM ownership");
		}
...
fu_plugin_init (FuPlugin *plugin)
{
	...
	tmp = g_strdup_printf ("%d.%d", smbios_get_library_version_major (),
