...
#define FU_UEFI_CAPSULE_EXTRA_SIZE_REQUIRED FU_MiB(1) /* bytes */
	...
	if (sz_reqd == 0) {
		if (fu_device_has_private_flag(FU_DEVICE(self),
			...
			g_info("minimal additional ESP free space required, using %uMB + %uMB",
			       (guint)fu_firmware_get_size(firmware) / FU_MiB(1),
			       (guint)FU_UEFI_CAPSULE_EXTRA_SIZE_REQUIRED / FU_MiB(1));
			sz_reqd =
