...
fu_uefi_update_esp_valid_func(void)
{
	...
	g_autoptr(GBytes) blob_padded = fu_bytes_pad(blob, FU_MiB(4), 0xFF);
	g_autoptr(GError) error = NULL;
	g_autoptr(GInputStream) stream = g_memory_input_stream_new_from_bytes(blob_padded);

	/* enough to fit the firmware */
	fu_volume_set_filesystem_free(volume_esp, FU_MiB(10));
...
fu_uefi_update_esp_invalid_func(void)
{
	...
	/* enough to fit the firmware */
	fu_volume_set_filesystem_free(volume_esp, FU_MiB(1));
...
fu_uefi_update_esp_no_backup_func(void)
{
	...
	/* enough to fit the firmware */
	fu_volume_set_filesystem_free(volume_esp, FU_MiB(6));
...
fu_uefi_capsule_fake_esp_new(FuTemporaryDirectory *tmpdir)
{
	...
	/* enough to fit the firmware */
	fu_volume_set_filesystem_free(esp, FU_MiB(10));
...
fu_uefi_update_info_xml_func(void)
{
	...
	ret = fu_firmware_roundtrip_from_filename(filename,
						  "18e8c43a912d3918498723340ae80a57d8b0657c",
