				     ...
				     GError **error)
{
	...
	if (!fu_redfish_request_perform(request,
					...
					FU_REDFISH_REQUEST_PERFORM_FLAG_LOAD_JSON,
					error))
		/* If we cannot poll the BMC, let's try again, it's probably updating itself */
		return TRUE;
	...
	if (json_object_has_member(json_obj, "Oem")) {
		...
		if (oem != NULL && json_object_has_member(oem, "Hpe")) {
			...
			if (g_strcmp0(status, "Error") == 0) {
				 ...
				 * in fu_redfish_device_poll_set_message_id if we can
				 */
				...
				if (json_object_has_member(hpe, "Result")) {
					...
					fu_redfish_device_poll_set_message_id(self,
									      ctx,
									      message_id);
				}
				return FALSE;
				     ...
				     GError **error)
{
	...
	if (!fu_redfish_backend_init_session(backend, error))
		return FALSE;
...
{
	fu_device_set_summary(FU_DEVICE(self), "Redfish HPE device");
}
