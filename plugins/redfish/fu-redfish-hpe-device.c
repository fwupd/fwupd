...
fu_redfish_hpe_device_attach(FuDevice *dev, FuProgress *progress, GError **error)
{
	...
	json_oem = fwupd_json_object_get_object(json_object, "Oem", NULL);
	if (json_oem != NULL)
		json_oem_hpe = fwupd_json_object_get_object(json_oem, "Hpe", NULL);
	if (json_oem_hpe != NULL) {
		const gchar *status = fwupd_json_object_get_string(json_oem_hpe, "State", NULL);
				     ...
				     GError **error)
{
	...
	pc = fwupd_json_object_get_integer_with_default(json_oem_hpe,
				     ...
				     GError **error)
{
	...
	/* create header */
	fwupd_json_object_add_boolean(json_object, "UpdateRepository", FALSE);
	fwupd_json_object_add_boolean(json_object, "UpdateTarget", TRUE);
	fwupd_json_object_add_string(json_object, "ETag", "atag");

	/* export as a string */
	json_str = fwupd_json_object_to_string(json_object, FWUPD_JSON_EXPORT_FLAG_INDENT);
