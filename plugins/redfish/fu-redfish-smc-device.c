...
static const gchar *
fu_redfish_smc_device_get_task(FwupdJsonObject *json_object)
{
	const gchar *msgid;
	...
	json_object_accepted = fwupd_json_object_get_object(json_object, "Accepted", NULL);
	...
	if (json_object_message == NULL)
		return NULL;
	msgid = fwupd_json_object_get_string(json_object_accepted, "MessageId", NULL);
...
fu_redfish_smc_device_get_parameters(FuRedfishSmcDevice *self)
{
	...
	fwupd_json_object_add_array(json_object, "Targets", json_array_targets);
	...
	fwupd_json_object_add_object(json_object, "Oem", json_oem);

	/* export as a string */
	return fwupd_json_object_to_string(json_object, FWUPD_JSON_EXPORT_FLAG_NONE);
}
...
fu_redfish_smc_device_start_update(FuRedfishSmcDevice *self, FuProgress *progress, GError **error)
{
	...
	location = fu_redfish_smc_device_get_task(json_object);
	if (location == NULL) {
		g_set_error(error,
			    FWUPD_ERROR,
