...
fu_redfish_multipart_device_get_parameters(FuRedfishMultipartDevice *self)
{
	...
	if (!fu_device_has_private_flag(FU_DEVICE(self), FU_REDFISH_DEVICE_FLAG_WILDCARD_TARGETS)) {
		...
		fwupd_json_array_add_string(json_array, logical_id);
	}
	fwupd_json_object_add_array(json_object, "Targets", json_array);
	...
	/* export as a string */
	return fwupd_json_object_to_string(json_object, FWUPD_JSON_EXPORT_FLAG_INDENT);
}
					   ...
					   GError **error)
{
	...
	if (location == NULL || g_utf8_strlen(location, 1) == 0) {
		...
		if (fwupd_json_object_has_node(json_object, "TaskMonitor")) {
			const gchar *tmp =
			    fwupd_json_object_get_string(json_object, "TaskMonitor", NULL);
			g_debug("task manager for cleanup is %s", tmp);
		}

		/* poll the task for progress */
		if (!fwupd_json_object_has_node(json_object, "@odata.id")) {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_INVALID_FILE,
		...
		location = fwupd_json_object_get_string(json_object, "@odata.id", error);
		if (location == NULL)
			return FALSE;
	}
