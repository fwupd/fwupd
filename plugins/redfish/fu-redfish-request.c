...
fu_redfish_request_get_json_object(FuRedfishRequest *self)
{
	...
	return fwupd_json_object_ref(self->json_object);
}
...
fu_redfish_request_load_json(FuRedfishRequest *self, GByteArray *buf, GError **error)
{
	...
	if (json_node == NULL)
		return FALSE;
	self->json_object = fwupd_json_node_get_object(json_node, error);
	if (self->json_object == NULL)
		return FALSE;

	/* dump for humans */
	str = fwupd_json_object_to_string(self->json_object, FWUPD_JSON_EXPORT_FLAG_INDENT);
	g_debug("response: %s", str->str);

	/* unauthorized */
	json_error = fwupd_json_object_get_object(self->json_object, "error", NULL);
	if (json_error != NULL) {
				...
				GError **error)
{
	...
	if (flags & FU_REDFISH_REQUEST_PERFORM_FLAG_USE_ETAG) {
		const gchar *etag;
		g_autoptr(FwupdJsonObject) json_object2 = NULL;
		...
		json_object2 = fu_redfish_request_get_json_object(self);
		etag = fwupd_json_object_get_string(json_object, "@odata.etag", NULL);
		if (etag != NULL)
