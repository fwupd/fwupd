					...
					GError **error)
{
	...
	if (helper->path != NULL) {
		...
		g_autoptr(GVariant) vpath = NULL;

		vpath = g_dbus_proxy_get_cached_property (proxy, "Path");
		if (vpath == NULL)
			return TRUE;
		path = g_variant_get_string (vpath, NULL);

		/* verify */
		if (g_getenv ("FWUPD_REDFISH_VERBOSE") != NULL)
			g_debug ("path=%s", path);
		if (g_strcmp0 (path, helper->path) == 0)
			helper->device = fu_redfish_network_device_new (object_path);
	}
...
FuRedfishNetworkDevice *
fu_redfish_network_device_for_path (const gchar *path, GError **error)
{
	FuRedfishNetworkMatchHelper helper = {
		.path = path,
	};
	if (!fu_redfish_network_device_match (&helper, error)) {
		g_prefix_error (error, "missing %s: ", path);
		return NULL;
	}
	return helper.device;
}
