...
fu_redfish_device_probe_related_pcie_item(FuRedfishDevice *self, const gchar *uri, GError **error)
{
	...
	if (!fu_redfish_request_perform(request,
					...
					error))
		return FALSE;
	json_object = fu_redfish_request_get_json_object(request);

	/* optional properties */
	tmp = fwupd_json_object_get_string(json_object, "DeviceClass", NULL);
	if (tmp != NULL && tmp[0] != '\0')
		fu_redfish_device_set_device_class(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "VendorId", NULL);
	if (tmp != NULL && tmp[0] != '\0') {
		if (!fu_strtoull(tmp, &vendor_id, 0, G_MAXUINT16, FU_INTEGER_BASE_AUTO, error))
			return FALSE;
	}
	tmp = fwupd_json_object_get_string(json_object, "DeviceId", NULL);
	if (tmp != NULL && tmp[0] != '\0') {
		if (!fu_strtoull(tmp, &model_id, 0, G_MAXUINT16, FU_INTEGER_BASE_AUTO, error))
			return FALSE;
	}
	tmp = fwupd_json_object_get_string(json_object, "SubsystemVendorId", NULL);
	if (tmp != NULL && tmp[0] != '\0') {
		if (!fu_strtoull(tmp,
				 &subsystem_vendor_id,
				 0,
	...
	tmp = fwupd_json_object_get_string(json_object, "SubsystemId", NULL);
	if (tmp != NULL && tmp[0] != '\0') {
		if (!fu_strtoull(tmp,
				 &subsystem_model_id,
				 0,
					       ...
					       GError **error)
{
	...
	if (!fu_redfish_request_perform(request,
		...
		return FALSE;
	json_object = fu_redfish_request_get_json_object(request);
	members_array = fwupd_json_object_get_array(json_object, "Members", NULL);
	if (members_array != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(members_array); i++) {
			const gchar *id;
			g_autoptr(FwupdJsonObject) json_member = NULL;

			json_member = fwupd_json_array_get_object(members_array, i, error);
			if (json_member == NULL)
				return FALSE;
			id = fwupd_json_object_get_string(json_member, "@odata.id", NULL);
			if (id != NULL) {
...
fu_redfish_device_probe_related_item(FuRedfishDevice *self, const gchar *uri, GError **error)
{
	...
	/* optional properties */
	tmp = fwupd_json_object_get_string(json_object, "SerialNumber", NULL);
	...
	if (fwupd_json_object_has_node(json_object, "hotpluggable")) {
		gboolean hotpluggable = FALSE;
		if (!fwupd_json_object_get_boolean(json_object,
						   "hotpluggable",
						   &hotpluggable,
						   error))
			return FALSE;
	...
	json_pcie = fwupd_json_object_get_object(json_object, "PCIeFunctions", NULL);
	if (json_pcie != NULL) {
		const gchar *id = fwupd_json_object_get_string(json_object, "@odata.id", NULL);
		if (id != NULL) {
...
static gboolean
fu_redfish_device_probe_oem_hpe(FuRedfishDevice *self, FwupdJsonObject *json_object, GError **error)
{
	const gchar *tmp;
	...
	json_array = fwupd_json_object_get_array(json_object, "Targets", NULL);
	if (json_array != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
			tmp = fwupd_json_array_get_string(json_array, i, NULL);
			if (tmp != NULL)
				fu_device_add_instance_id(FU_DEVICE(self), tmp);
		}
	}
...
static gboolean
fu_redfish_device_probe_oem_dell(FuRedfishDevice *self,
				 ...
				 GError **error)
{
	...
	/* ignore */
	software_info = fwupd_json_object_get_object(json_object, "DellSoftwareInventory", NULL);
...
fu_redfish_device_probe(FuDevice *dev, GError **error)
{
	...
	FuRedfishDevicePrivate *priv = GET_PRIVATE(self);
	FwupdJsonObject *json_member = priv->json_member;
	const gchar *tmp;
	...
	tmp = fwupd_json_object_get_string(json_member, "@odata.id", error);
	if (tmp == NULL)
		return FALSE;
	fu_device_set_physical_id(dev, "Redfish-Inventory");
	fu_device_set_logical_id(dev, tmp);

	tmp = fwupd_json_object_get_string(json_member, "Id", NULL);
	if (tmp != NULL)
		fu_device_set_backend_id(dev, tmp);
	fu_device_add_instance_str(dev, "ID", fu_device_get_backend_id(dev));

	/* device properties */
	tmp = fwupd_json_object_get_string(json_member, "Manufacturer", NULL);
	if (tmp != NULL && tmp[0] != '\0')
	...
	tmp = fwupd_json_object_get_string(json_member, "Version", NULL);
	if (tmp != NULL && tmp[0] != '\0')
		fu_redfish_device_set_version(self, tmp);

	/* ReleaseDate may or may not have a timezone */
	tmp = fwupd_json_object_get_string(json_member, "ReleaseDate", NULL);
	...
	software_id = fwupd_json_object_get_string(json_member, "SoftwareId", NULL);
	if (software_id != NULL) {
	...
	if (json_oem != NULL) {
		...
		json_dell = fwupd_json_object_get_object(json_oem, "Dell", error);
		if (json_dell != NULL) {
	...
	tmp = fwupd_json_object_get_string(json_member, "Name", NULL);
	if (tmp != NULL && tmp[0] != '\0')
		fu_redfish_device_set_name(self, tmp);
	tmp = fwupd_json_object_get_string(json_member, "LowestSupportedVersion", NULL);
	if (tmp != NULL && tmp[0] != '\0')
		fu_redfish_device_set_version_lowest(self, tmp);
	tmp = fwupd_json_object_get_string(json_member, "Description", NULL);
	if (tmp != NULL && tmp[0] != '\0')
		fu_device_set_summary(dev, tmp);

	/* reasons why the device might not be updatable */
	if (!fwupd_json_object_get_boolean_with_default(json_member,
							"Updateable",
							&updateable,
							TRUE,
							error))
	...
	if (updateable && fwupd_json_object_has_node(json_member, "RelatedItem")) {
		g_autoptr(FwupdJsonArray) json_array_related = NULL;
		json_array_related = fwupd_json_object_get_array(json_member, "RelatedItem", NULL);
...
fu_redfish_device_poll_task_once(FuRedfishDevice *self, FuRedfishDevicePollCtx *ctx, GError **error)
{
	...
	json_msgs = fwupd_json_object_get_array(json_object, "Messages", NULL);
	if (json_msgs != NULL) {
	...
	if (!fwupd_json_object_has_node(json_object, "TaskState")) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
	...
	state_tmp = fwupd_json_object_get_string(json_object, "TaskState", NULL);
...
fu_redfish_device_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	...
	case PROP_BACKEND:
		...
		break;
	case PROP_MEMBER:
		g_value_set_pointer(value, priv->json_member);
		break;
	default:
...
static void
fu_redfish_device_set_member(FuRedfishDevice *self, FwupdJsonObject *json_member)
{
	FuRedfishDevicePrivate *priv = GET_PRIVATE(self);
	if (priv->json_member != NULL) {
		fwupd_json_object_unref(priv->json_member);
		priv->json_member = NULL;
	}
	if (json_member != NULL)
		priv->json_member = fwupd_json_object_ref(json_member);
}
...
fu_redfish_device_class_init(FuRedfishDeviceClass *klass)
{
	 ...
	 *
	 * The JSON root json_member for the device.
	 */
	pspec =
