...
fu_redfish_legacy_device_detach(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	/* create header */
	fwupd_json_object_add_boolean(json_object, "HttpPushUriTargetsBusy", TRUE);
	fwupd_json_array_add_string(json_array, fu_device_get_logical_id(device));
	fwupd_json_object_add_array(json_object, "HttpPushUriTargets", json_array);
...
fu_redfish_legacy_device_attach(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	/* create header */
	fwupd_json_object_add_boolean(json_object, "HttpPushUriTargetsBusy", FALSE);
	fwupd_json_object_add_array(json_object, "HttpPushUriTargets", json_array);
