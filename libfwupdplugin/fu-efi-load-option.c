...
static void
fu_efi_load_option_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuEfiLoadOption *self = FU_EFI_LOAD_OPTION(codec);
	g_autoptr(FuFirmware) dp_list = NULL;

	fwupd_codec_json_append(json_object, "Name", fu_firmware_get_id(FU_FIRMWARE(self)));
	if (self->kind != FU_EFI_LOAD_OPTION_KIND_UNKNOWN) {
	...
	if (g_hash_table_size(self->metadata) > 0) {
		...
		g_hash_table_iter_init(&iter, self->metadata);
		while (g_hash_table_iter_next(&iter, (gpointer *)&key, (gpointer *)&value))
			fwupd_codec_json_append(json_object_tmp, key, value);
		fwupd_json_object_add_object(json_object, "Metadata", json_object_tmp);
	}
	dp_list =
	    fu_firmware_get_image_by_gtype(FU_FIRMWARE(self), FU_TYPE_EFI_DEVICE_PATH_LIST, NULL);
	if (dp_list != NULL)
		fwupd_codec_to_json(FWUPD_CODEC(dp_list), json_object, flags);
}
