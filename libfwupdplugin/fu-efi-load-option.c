			 ...
			 GError **error)
{
	...
	/* add header size with overflow checking */
	...
	while (offset < streamsz) {
		...
		if (!fu_size_checked_inc(&offset, 2, error)) {
			g_prefix_error_literal(error, "load option description offset overflow: ");
			return FALSE;
		}
	}
	...
	/* add null terminator size with overflow checking */
	if (!fu_size_checked_inc(&offset, 2, error)) {
		g_prefix_error_literal(error, "load option description offset overflow: ");
		return FALSE;
	}
	...
	/* add device path size with overflow checking */
...
static void
fu_efi_load_option_add_json(FwupdCodec *codec, FwupdJsonObject *json_obj, FwupdCodecFlags flags)
{
	...
	if (dp_list != NULL)
		fwupd_codec_to_json(FWUPD_CODEC(dp_list), json_obj, flags);
}
