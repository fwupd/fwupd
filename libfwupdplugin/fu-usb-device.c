...
static gboolean
fu_usb_device_from_json(FuDevice *device, FwupdJsonObject *json_object, GError **error)
{
	FuUsbDevice *self = FU_USB_DEVICE(device);
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	const gchar *tmp;
	...
	/* optional properties */
	tmp = fwupd_json_object_get_string(json_object, "PlatformId", NULL);
	if (tmp != NULL)
	...
	if (!fwupd_json_object_get_integer_with_default(json_object, "Device", &value, 0x0, error))
		return FALSE;
	priv->desc.bcdDevice = value;
	if (!fwupd_json_object_get_integer_with_default(json_object, "USB", &value, 0x0, error))
		return FALSE;
	...
	if (!fwupd_json_object_get_integer_with_default(json_object, "Product", &value, 0x0, error))
		return FALSE;
	...
	json_array_bos = fwupd_json_object_get_array(json_object, "UsbBosDescriptors", NULL);
	if (json_array_bos != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array_bos); i++) {
			...
			json_object_tmp = fwupd_json_array_get_object(json_array_bos, i, error);
	...
	json_array_cfg = fwupd_json_object_get_array(json_object, "UsbConfigDescriptors", NULL);
	if (json_array_cfg != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array_cfg); i++) {
			...
			json_object_tmp = fwupd_json_array_get_object(json_array_cfg, i, error);
			if (json_object_tmp == NULL)
				return FALSE;
	...
	json_array_hid = fwupd_json_object_get_array(json_object, "UsbHidDescriptors", NULL);
	if (json_array_hid != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array_hid); i++) {
			...
			if (json_node_tmp == NULL)
				return FALSE;
			fwupd_json_object_add_node(json_object_tmp, "Data", json_node_tmp);
			if (!fwupd_codec_from_json(FWUPD_CODEC(hid_descriptor), json_object_tmp, error))
				return FALSE;
			g_ptr_array_add(priv->hid_descriptors, g_object_ref(hid_descriptor));
		}
	}
	...
	/* array of interfaces */
	json_array_ifaces = fwupd_json_object_get_array(json_object, "UsbInterfaces", NULL);
	if (json_array_ifaces != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array_ifaces); i++) {
			g_autoptr(FwupdJsonObject) json_object_tmp = NULL;
			g_autoptr(FuUsbInterface) iface = g_object_new(FU_TYPE_USB_INTERFACE, NULL);

			json_object_tmp = fwupd_json_array_get_object(json_array_ifaces, i, error);
			if (json_object_tmp == NULL)
				return FALSE;
			if (!fwupd_codec_from_json(FWUPD_CODEC(iface),  json_object_tmp, error))
				return FALSE;
			fu_usb_device_add_interface_internal(self, iface);
		}
	}
	...
	/* array of events */
	json_array_events = fwupd_json_object_get_array(json_object, "UsbEvents", NULL);
	if (json_array_events != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array_events); i++) {
			...
			json_object_tmp = fwupd_json_array_get_object(json_array_events, i, error);
			if (json_object_tmp == NULL)
				return FALSE;
			if (!fwupd_codec_from_json(FWUPD_CODEC(event), json_object_tmp, error))
				return FALSE;
...
static void
fu_usb_device_add_json(FuDevice *device, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuUsbDevice *self = FU_USB_DEVICE(device);
	FuUsbDevicePrivate *priv = GET_PRIVATE(self);
	...
	/* optional properties */
	fwupd_codec_json_append(json_object, "GType", "FuUsbDevice");
	...
	if (fu_device_get_created_usec(FU_DEVICE(self)) != 0) {
		...
		fwupd_codec_json_append(json_object, "Created", str);
	}
	...
	if (priv->desc.bcdDevice != 0)
		fwupd_codec_json_append_int(json_object, "Device", priv->desc.bcdDevice);
	if (priv->desc.bcdUSB != 0)
		fwupd_codec_json_append_int(json_object, "USB", priv->desc.bcdUSB);
	if (priv->desc.iManufacturer != 0)
		fwupd_codec_json_append_int(json_object, "Manufacturer", priv->desc.iManufacturer);
	if (priv->desc.bDeviceClass != 0)
		fwupd_codec_json_append_int(json_object, "DeviceClass", priv->desc.bDeviceClass);
	...
	if (priv->desc.iProduct != 0)
		fwupd_codec_json_append_int(json_object, "Product", priv->desc.iProduct);
	if (priv->desc.iSerialNumber != 0)
		fwupd_codec_json_append_int(json_object, "SerialNumber", priv->desc.iSerialNumber);
	...
	if (priv->bos_descriptors->len > 0) {
		...
		for (guint i = 0; i < priv->bos_descriptors->len; i++) {
			...
			g_autoptr(FwupdJsonObject) json_object_tmp = fwupd_json_object_new();
			fwupd_codec_to_json(FWUPD_CODEC(bos_descriptor), json_object_tmp, flags);
			fwupd_json_array_add_object(json_array, json_object_tmp);
		}
		fwupd_json_object_add_array(json_object, "UsbBosDescriptors", json_array);
	}
	...
	if (priv->cfg_descriptors->len > 0) {
		...
		for (guint i = 0; i < priv->cfg_descriptors->len; i++) {
			...
			g_autoptr(FwupdJsonObject) json_object_tmp = fwupd_json_object_new();
			fwupd_codec_to_json(FWUPD_CODEC(cfg_descriptor), json_object_tmp, flags);
			fwupd_json_array_add_object(json_array, json_object_tmp);
		}
		fwupd_json_object_add_array(json_object, "UsbConfigDescriptors", json_array);
	}
	...
	if (!fu_usb_device_ensure_hid_descriptors(self, &error_hid)) {
		...
		for (guint i = 0; i < priv->hid_descriptors->len; i++) {
			...
			fwupd_codec_to_json(FWUPD_CODEC(hid_descriptor), json_object_tmp, flags);
			/* weirdly, this is encoded as a node, not an object */
			json_node_tmp = fwupd_json_object_get_node(json_object_tmp, "Data", NULL);
			if (json_node_tmp != NULL)
				fwupd_json_array_add_node(json_array, json_node_tmp);
		}
		fwupd_json_object_add_array(json_object, "UsbHidDescriptors", json_array);
	}
	...
	if (interfaces == NULL) {
		...
		for (guint i = 0; i < interfaces->len; i++) {
			...
			g_autoptr(FwupdJsonObject) json_object_tmp = fwupd_json_object_new();
			fwupd_codec_to_json(FWUPD_CODEC(iface), json_object_tmp, flags);
			fwupd_json_array_add_object(json_array, json_object_tmp);
		}
		fwupd_json_object_add_array(json_object, "UsbInterfaces", json_array);
	}
	...
	if (events->len > 0) {
		...
		fwupd_json_object_add_array(json_object, "UsbEvents", json_array);
	}
}
