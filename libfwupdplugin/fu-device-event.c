...
static void
fu_device_event_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuDeviceEvent *self = FU_DEVICE_EVENT(codec);

	if (self->id_uncompressed != NULL && (flags & FWUPD_CODEC_FLAG_COMPRESSED) == 0) {
		fwupd_json_object_add_string(json_object, "Id", self->id_uncompressed);
	} else if (self->id != NULL) {
		fwupd_json_object_add_string(json_object, "Id", self->id);
	}

	for (guint i = 0; i < self->values->len; i++) {
		FuDeviceEventBlob *blob = g_ptr_array_index(self->values, i);
		if (blob->gtype == G_TYPE_INT) {
			fwupd_json_object_add_integer(json_object, blob->key, *((gint64 *)blob->data));
		} else if (blob->gtype == G_TYPE_BYTES || blob->gtype == G_TYPE_STRING) {
			fwupd_json_object_add_string(json_object, blob->key, (const gchar *)blob->data);
		} else {
...
static gboolean
fu_device_event_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	FuDeviceEvent *self = FU_DEVICE_EVENT(codec);
	for (guint i = 0; i < fwupd_json_object_get_size(json_object); i++) {
		GRefString *key = fwupd_json_object_get_key_for_index(json_object, i, NULL);
		g_autoptr(FwupdJsonNode) json_node = NULL;

		json_node = fwupd_json_object_get_node_for_index(json_object, i, error);
		if (json_node == NULL)
			return FALSE;
		if (fwupd_json_node_get_kind(json_node) == FWUPD_JSON_NODE_KIND_STRING) {
			...
			if (str == NULL)
				return FALSE;
			if (!fu_strtoll(str, &value, 0, G_MAXINT64, FU_INTEGER_BASE_AUTO, error)) {
				g_prefix_error(error, "cannot parse %s: ", str);
				return FALSE;
			}
			g_ptr_array_add(
			    self->values,
	...
	fwupd_json_object_clear(json_object);
