...
fu_sbatlevel_section_write(FuFirmware *firmware, GError **error)
{
	...
	if (img_prev == NULL)
		return NULL;
	buf_prev = fu_firmware_write_array(img_prev, error);
	if (buf_prev == NULL)
		return NULL;
	fu_byte_array_append_array(st->buf, buf_prev);
	fu_byte_array_append_uint8(st->buf, 0x0);

	/* latest */
	fu_struct_sbat_level_section_header_set_latest(st,
						       (sizeof(guint32) * 2) + buf_prev->len + 1);
	img_ltst = fu_firmware_get_image_by_id(firmware, "latest", error);
	if (img_ltst == NULL)
		return NULL;
	buf_ltst = fu_firmware_write_array(img_ltst, error);
	if (buf_ltst == NULL)
		return NULL;
	fu_byte_array_append_array(st->buf, buf_ltst);
	fu_byte_array_append_uint8(st->buf, 0x0);
