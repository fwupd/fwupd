...
fu_context_hwid_fdt_setup(FuContext *self, GError **error)
{
	...
	for (guint i = 0; map[i].key != NULL; i++) {
		...
		fu_context_add_hwid_value(self, map[i].hwid, tmp);
	}

	/* fallback */
	if (g_strv_length(compatible) > 0)
		fu_context_add_hwid_value(self, FU_HWIDS_KEY_MANUFACTURER, compatible[0]);
	if (g_strv_length(compatible) > 1)
		fu_context_add_hwid_value(self, FU_HWIDS_KEY_PRODUCT_NAME, compatible[1]);
	if (g_strv_length(compatible) > 2)
		fu_context_add_hwid_value(self, FU_HWIDS_KEY_FAMILY, compatible[2]);
	if (fu_context_get_chassis_kind(self) == FU_SMBIOS_CHASSIS_KIND_UNKNOWN) {
		if (fu_fdt_image_get_attr_str(FU_FDT_IMAGE(fdt_img), "battery", NULL, NULL))
			fu_context_set_chassis_kind(self, FU_SMBIOS_CHASSIS_KIND_PORTABLE);
	}
	fdt_img_fwver =
	...
	if (fdt_img_fwver != NULL) {
		...
		fu_context_add_hwid_value(self, FU_HWIDS_KEY_BIOS_VERSION, version);
	}
