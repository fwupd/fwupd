...
static gboolean
fu_intel_me_device_from_json(FuDevice *device, JsonObject *json_object, GError **error)
{
	FuIntelMeDevice *self = FU_INTEL_ME_DEVICE(device);
	const gchar *version;
	...
	for (guint i = 1; i < G_N_ELEMENTS(self->hfsts); i++) {
		...
		tmp64 = json_object_get_int_member_with_default(json_object, title, 0);
...
static void
fu_intel_me_device_add_json(FuDevice *device, JsonBuilder *builder, FwupdCodecFlags flags)
{
	FuIntelMeDevice *self = FU_INTEL_ME_DEVICE(device);
	fwupd_codec_json_append(builder, "GType", "FuIntelMeDevice");
	fwupd_codec_json_append(builder, "BackendName", "udev");
