...
static gboolean
fu_usb_endpoint_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	...
	if (!fwupd_json_object_get_integer_with_default(json_object, "Refresh", &tmpi, 0x0, error))
		return FALSE;
	self->endpoint_descriptor.bRefresh = tmpi;
	if (!fwupd_json_object_get_integer_with_default(json_object, "Interval", &tmpi, 0x0, error))
		return FALSE;
...
static void
fu_usb_endpoint_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuUsbEndpoint *self = FU_USB_ENDPOINT(codec);
