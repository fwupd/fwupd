...
static void
fu_plugin_device_added_cb(FuPlugin *plugin, FuDevice *device, gpointer user_data)
{
...
fu_plugin_config_func(void)
{
	...
	/* load context */
	ret = fu_context_load(ctx, progress, FU_CONTEXT_LOAD_FLAG_FIX_PERMISSIONS, &error);
	g_assert_no_error(error);
	g_assert_true(ret);
...
static void
fu_plugin_backend_proxy_device_added_cb(FuPlugin *plugin, FuDevice *device, gpointer user_data)
{
	FuDevice **dev = (FuDevice **)user_data;
	*dev = g_object_ref(device);
}
...
fu_plugin_backend_proxy_device_func(void)
{
	...
	g_signal_connect(plugin,
			 "device-added",
			 G_CALLBACK(fu_plugin_backend_proxy_device_added_cb),
			 &device_new);
