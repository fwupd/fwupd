...
static gboolean
fu_backend_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	...
	/* if recorded */
	fwupd_version = fwupd_json_object_get_string(json_object, "FwupdVersion", NULL);
	...
	json_array = fwupd_json_object_get_array(json_object, "UsbDevices", NULL);
	if (json_array == NULL) {
	...
	for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
		...
		/* sanity check */
		object_tmp = fwupd_json_array_get_object(json_array, i, error);
		if (object_tmp == NULL)
			return FALSE;
...
static void
fu_backend_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuBackend *self = FU_BACKEND(codec);
	FuBackendPrivate *priv = GET_PRIVATE(self);
	...
	/* remain compatible with all the old emulation files */
	fwupd_codec_json_append(json_object, "FwupdVersion", PACKAGE_VERSION);
	...
	fwupd_json_object_add_array(json_object, "UsbDevices", json_array);
}
