...
static gboolean
fu_bios_settings_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	FuBiosSettings *self = FU_BIOS_SETTINGS(codec);
	g_autoptr(FwupdJsonArray) json_array = NULL;

	/* this has to exist */
	json_array = fwupd_json_object_get_array(json_object, "BiosSettings", error);
	if (json_array == NULL)
		return FALSE;
