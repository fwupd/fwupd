...
fu_fdt_firmware_parse_dt_struct(FuFdtFirmware *self, GBytes *fw, GByteArray *strtab, GError **error)
{
	...
	while (offset < bufsz) {
		...
		/* add token size with overflow checking */
		...
		if (token == FU_FDT_TOKEN_PROP) {
			...
			/* add property structure size with overflow checking */
			if (!fu_size_checked_inc(&offset, st_prp->buf->len, error)) {
				g_prefix_error_literal(error, "FDT property offset overflow: ");
				return FALSE;
			}
				 ...
				 GError **error)
{
	...
	while (offset < streamsz) {
		...
		/* add reserve entry size with overflow checking */
		if (!fu_size_checked_inc(&offset, FU_STRUCT_FDT_RESERVE_ENTRY_SIZE, error)) {
			g_prefix_error_literal(error, "FDT reserve entry offset overflow: ");
			return FALSE;
		}
	}
