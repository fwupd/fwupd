...
static void
fu_uefi_device_add_json(FuDevice *device, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuUefiDevice *self = FU_UEFI_DEVICE(device);
	FuUefiDevicePrivate *priv = GET_PRIVATE(self);
	...
	/* optional properties */
	fwupd_codec_json_append(json_object, "GType", "FuUefiDevice");
	if (fu_device_get_backend_id(device) != NULL)
		fwupd_json_object_add_string(json_object,
					     "BackendId",
					     fu_device_get_backend_id(device));
	if (priv->guid != NULL)
		fwupd_json_object_add_string(json_object, "Guid", priv->guid);
	if (priv->name != NULL)
		fwupd_json_object_add_string(json_object, "Name", priv->name);
	...
	if (fu_device_get_created_usec(device) != 0) {
		...
		fwupd_json_object_add_string(json_object, "Created", str);
	}
#endif
	...
	if (events->len > 0) {
		...
		for (guint i = 0; i < events->len; i++) {
			...
			fwupd_codec_to_json(FWUPD_CODEC(event), json_object_tmp, flags);
...
static gboolean
fu_uefi_device_from_json(FuDevice *device, FwupdJsonObject *json_object, GError **error)
{
	FuUefiDevice *self = FU_UEFI_DEVICE(device);
	const gchar *tmp;
	g_autoptr(FwupdJsonArray) json_array_events = NULL;

	tmp = fwupd_json_object_get_string(json_object, "Guid", NULL);
	if (tmp != NULL)
		fu_uefi_device_set_guid(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "Name", NULL);
	if (tmp != NULL)
...
#if GLIB_CHECK_VERSION(2, 80, 0)
	tmp = fwupd_json_object_get_string(json_object, "Created", NULL);
	if (tmp != NULL) {
