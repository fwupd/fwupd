...
fu_path_from_kind(FuPathKind path_kind)
{
	...
	/* this is very inefficient -- use the FuPathContext in FuContext! */
	g_debug("creating a local FuPathContext, use fu_context_get_path_context() instead!");
	fu_path_context_load_defaults(pathctx);
	fu_path_context_load_from_env(pathctx);
	dir = fu_path_context_get_dir(pathctx, path_kind);
	g_debug("returned %s -> %s", fu_path_kind_to_string(path_kind), dir);
	return g_strdup(dir);
}
