...
fu_bios_setting_set_id(FuBiosSetting *self, const gchar *id)
{
	...
	/* fallback used when no AppStream ID quirk is available */
	if (g_strcmp0(id, "com.thinklmi.WindowsUEFIFirmwareUpdate") == 0) {
		fu_bios_setting_set_description(
		    self,
		    /* TRANSLATORS: description of a BIOS firmware update setting */
		    _("BIOS updates delivered via LVFS or Windows Update"));
	}
...
fu_bios_setting_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_CONTEXT:
		...
		if (priv->ctx != NULL)
			g_object_add_weak_pointer(G_OBJECT(priv->ctx), (gpointer *)&priv->ctx);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void
fu_bios_setting_finalize(GObject *object)
{
	FuBiosSetting *self = FU_BIOS_SETTING(object);
	FuBiosSettingPrivate *priv = GET_PRIVATE(self);
	if (priv->ctx != NULL)
		g_object_remove_weak_pointer(G_OBJECT(priv->ctx), (gpointer *)&priv->ctx);
	G_OBJECT_CLASS(fu_bios_setting_parent_class)->finalize(object);
}
...
fu_bios_setting_class_init(FuBiosSettingClass *klass)
{
	...
	object_class->finalize = fu_bios_setting_finalize;
