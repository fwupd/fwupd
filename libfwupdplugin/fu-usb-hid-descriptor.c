...
static gboolean
fu_usb_hid_descriptor_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	FuUsbHidDescriptor *self = FU_USB_HID_DESCRIPTOR(codec);
	const gchar *tmp;

	tmp = fwupd_json_object_get_string(json_object, "Data", NULL);
	if (tmp != NULL) {
		gsize bufsz = 0;
...
static void
fu_usb_hid_descriptor_add_json(FwupdCodec *codec,
			       ...
			       FwupdCodecFlags flags)
{
	...
	fwupd_json_object_add_string(json_object, "Data", str);
}
