			      ...
			      GError **error)
{
	...
	if (!fu_struct_unpack_from("<[TB]", error, buf, bufsz, &offset, &size, &priv->type))
		return FALSE;
	if (size < offset) {
		g_set_error(error,
			    FWUPD_ERROR,
	...
	if (priv->type == FU_EFI_FIRMWARE_SECTION_TYPE_GUID_DEFINED) {
		...
		guint16 section_offset = 0;
		g_autofree gchar *guid_str = NULL;

		if (!fu_struct_unpack_from("<GHH",
					   error,
					   buf,
					   bufsz,
					   &offset,
...
fu_efi_firmware_section_write(FuFirmware *firmware, GError **error)
{
	...
	/* header */
	if (priv->type != FU_EFI_FIRMWARE_SECTION_TYPE_GUID_DEFINED) {
		buf = fu_struct_pack("<TB",
				     error,
				     fu_struct_size("<TB", NULL) + g_bytes_get_size(blob),
				     priv->type);
		if (buf == NULL)
			return NULL;
	} else {
		...
		if (!fwupd_guid_from_string(fu_firmware_get_id(firmware),
					    ...
					    error))
			return NULL;
		buf = fu_struct_pack("<TBGHH",
				     error,
				     fu_struct_size("<TBGHH", NULL) + g_bytes_get_size(blob),
				     priv->type,
				     guid,
				     fu_struct_size("<TBGHH", NULL), /* section_offset */
				     0x0);			     /* attr */
		if (buf == NULL)
			return NULL;
	}
