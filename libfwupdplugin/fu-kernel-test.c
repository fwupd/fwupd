...
fu_kernel_lockdown_func(void)
{
	...
	fu_path_context_set_dir(pathctx, FU_PATH_KIND_SYSFSDIR_SECURITY, old_kernel_dir);
	ret = fu_kernel_locked_down(pathctx);
	g_assert_false(ret);

	locked_dir = g_test_build_filename(G_TEST_DIST, "tests", "lockdown", "locked", NULL);
	fu_path_context_set_dir(pathctx, FU_PATH_KIND_SYSFSDIR_SECURITY, locked_dir);
	ret = fu_kernel_locked_down(pathctx);
	g_assert_true(ret);

	none_dir = g_test_build_filename(G_TEST_DIST, "tests", "lockdown", "none", NULL);
	fu_path_context_set_dir(pathctx, FU_PATH_KIND_SYSFSDIR_SECURITY, none_dir);
	ret = fu_kernel_locked_down(pathctx);
	g_assert_false(ret);
}
