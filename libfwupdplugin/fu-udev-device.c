...
static void
fu_udev_device_add_json(FuDevice *device, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FuUdevDevice *self = FU_UDEV_DEVICE(device);
	FuUdevDevicePrivate *priv = GET_PRIVATE(self);
	...
	/* optional properties */
	fwupd_codec_json_append(json_object, "GType", "FuUdevDevice");
	if (fu_udev_device_get_sysfs_path(self) != NULL) {
	...
	if (priv->device_file != NULL)
		fwupd_json_object_add_string(json_object, "DeviceFile", priv->device_file);
	if (priv->subsystem != NULL)
		fwupd_json_object_add_string(json_object, "Subsystem", priv->subsystem);
	if (priv->devtype != NULL)
		fwupd_json_object_add_string(json_object, "Devtype", priv->devtype);
	if (priv->driver != NULL)
		fwupd_json_object_add_string(json_object, "Driver", priv->driver);
	if (priv->bind_id != NULL)
		fwupd_json_object_add_string(json_object, "BindId", priv->bind_id);
	if (fu_device_get_vid(device) != 0)
		fwupd_json_object_add_integer(json_object, "Vendor", fu_device_get_vid(device));
	if (fu_device_get_pid(device) != 0)
		fwupd_json_object_add_integer(json_object, "Model", fu_device_get_pid(device));
	...
	if (events->len > 0) {
		...
		fwupd_json_object_add_array(json_object, "Events", json_array);
	}
}

static gboolean
fu_udev_device_from_json(FuDevice *device, FwupdJsonObject *json_object, GError **error)
{
	FuUdevDevice *self = FU_UDEV_DEVICE(device);
	const gchar *tmp;
	gint64 tmp64 = 0;
	...
	tmp = fwupd_json_object_get_string(json_object, "BackendId", NULL);
	if (tmp != NULL)
		fu_device_set_backend_id(device, tmp);
	tmp = fwupd_json_object_get_string(json_object, "Subsystem", NULL);
	if (tmp != NULL)
		fu_udev_device_set_subsystem(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "Devtype", NULL);
	if (tmp != NULL)
		fu_udev_device_set_devtype(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "Driver", NULL);
	if (tmp != NULL)
		fu_udev_device_set_driver(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "BindId", NULL);
	if (tmp != NULL)
		fu_udev_device_set_bind_id(self, tmp);
	tmp = fwupd_json_object_get_string(json_object, "DeviceFile", NULL);
	if (tmp != NULL)
		fu_udev_device_set_device_file(self, tmp);
	if (!fwupd_json_object_get_integer_with_default(json_object, "Vendor", &tmp64, 0x0, error))
		return FALSE;
	if (tmp64 != 0)
		fu_device_set_vid(device, tmp64);
	if (!fwupd_json_object_get_integer_with_default(json_object, "Model", &tmp64, 0x0, error))
		return FALSE;
	if (tmp64 != 0)
		fu_device_set_pid(device, tmp64);

	/* array of events */
	json_array_events = fwupd_json_object_get_array(json_object, "Events", NULL);
