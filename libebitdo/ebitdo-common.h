...
typedef struct __attribute__((packed)) {
	...
	guint8		subtype;		/* EbitdoPktSubtype */
	guint16		cmd_len;
...
typedef enum {
	EBITDO_PKT_SUBTYPE_ACK			= 0x14, /* acknowledge */
	EBITDO_PKT_SUBTYPE_NAK			= 0x15, /* negative acknowledge */
	EBITDO_PKT_SUBTYPE_UPDATE_FIRMWARE_DATA	= 0x16, /* update firmware data */
	EBITDO_PKT_SUBTYPE_TRANSFER_ABORT	= 0x18, /* aborts transfer */
	EBITDO_PKT_SUBTYPE_VERIFICATION_ID	= 0x19, /* verification id (only BT?) */
	EBITDO_PKT_SUBTYPE_GET_VERIFICATION_ID	= 0x1a, /* verification id (only BT) */
	EBITDO_PKT_SUBTYPE_VERIFY_ERROR		= 0x1b, /* verification error */
	EBITDO_PKT_SUBTYPE_VERIFY_OK		= 0x1c, /* verification successful */
	EBITDO_PKT_SUBTYPE_TRANSFER_TIMEOUT	= 0x1d, /* send or recieve data timeout */
	EBITDO_PKT_SUBTYPE_GET_VERSION		= 0x21, /* get fw ver, joystick mode */
	EBITDO_PKT_SUBTYPE_GET_VERSION_RESPONSE	= 0x22, /* get fw version response */
	EBITDO_PKT_SUBTYPE_LAST
} EbitdoPktSubtype;
...
const gchar	*ebitdo_pkt_subtype_to_string	(EbitdoPktSubtype	 subtype);
