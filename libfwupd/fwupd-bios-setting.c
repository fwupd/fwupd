...
static gboolean
fwupd_bios_setting_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	FwupdBiosSetting *self = FWUPD_BIOS_SETTING(codec);
	gboolean tmpb = FALSE;
	gint64 tmpi = 0;
	...
	fwupd_bios_setting_set_path(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_FILENAME, NULL));
	fwupd_bios_setting_set_current_value(
	    self,
	    fwupd_json_object_get_string(json_object,
					 FWUPD_RESULT_KEY_BIOS_SETTING_CURRENT_VALUE,
					 NULL));
	json_array = fwupd_json_object_get_array(json_object,
						 FWUPD_RESULT_KEY_BIOS_SETTING_POSSIBLE_VALUES,
						 NULL);
	if (json_array != NULL) {
		for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
			const gchar *tmp = fwupd_json_array_get_string(json_array, i, NULL);
...
static void
fwupd_bios_setting_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FwupdBiosSetting *self = FWUPD_BIOS_SETTING(codec);
	FwupdBiosSettingPrivate *priv = GET_PRIVATE(self);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_FILENAME, priv->path);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_BIOS_SETTING_ID, priv->id);
	...
	fwupd_json_object_add_boolean(json_object,
				      FWUPD_RESULT_KEY_BIOS_SETTING_READ_ONLY,
				      priv->read_only);
	fwupd_json_object_add_integer(json_object, FWUPD_RESULT_KEY_BIOS_SETTING_TYPE, priv->kind);
	if (priv->kind == FWUPD_BIOS_SETTING_KIND_ENUMERATION) {
		if (priv->possible_values->len > 0) {
