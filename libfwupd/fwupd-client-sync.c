...
fwupd_client_quit(FwupdClient *self, GCancellable *cancellable, GError **error)
{
	...
	if (impl->quit == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->quit");
		return FALSE;
	}
	return impl->quit(self, impl_userdata, cancellable, error);
}
		    ...
		    GError **error)
{
	...
	if (impl->verify == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->verify");
		return FALSE;
	}
	return impl->verify(self, device_id, impl_userdata, cancellable, error);
}
			   ...
			   GError **error)
{
	...
	if (impl->verify_update == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->verify_update");
		return FALSE;
	}
	return impl->verify_update(self, device_id, impl_userdata, cancellable, error);
}
		    ...
		    GError **error)
{
	...
	if (impl->unlock == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->unlock");
		return FALSE;
	}
	return impl->unlock(self, device_id, impl_userdata, cancellable, error);
}
		       ...
		       GError **error)
{
	...
	if (impl->uninhibit == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->uninhibit");
		return FALSE;
	}
	return impl->uninhibit(self, inhibit_id, impl_userdata, cancellable, error);
}
			   ...
			   GError **error)
{
	...
	if (impl->clear_results == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->clear_results");
		return FALSE;
	}
	return impl->clear_results(self, device_id, impl_userdata, cancellable, error);
}
		     ...
		     GError **error)
{
	...
	if (impl->install == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->install");
		return FALSE;
	}
	return impl
	    ->install(self, device_id, filename, install_flags, impl_userdata, cancellable, error);
}
			     ...
			     GError **error)
{
	...
	if (impl->install_release == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->install_release");
		return FALSE;
	}
	return impl->install_release(self,
				     device,
				     release,
				     install_flags,
				     download_flags,
			     ...
			     GError **error)
{
	...
	if (impl->update_metadata == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->update_metadata");
		return FALSE;
	}
	return impl->update_metadata(self,
				     remote_id,
				     metadata_fn,
				     signature_fn,
				     impl_userdata,
			    ...
			    GError **error)
{
	...
	if (impl->refresh_remote == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->refresh_remote");
		return FALSE;
	}
	return impl
	    ->refresh_remote(self, remote, download_flags, impl_userdata, cancellable, error);
}
...
{
	FwupdClientHelper *helper = (FwupdClientHelper *)user_data;
	helper->ret = fwupd_client_clean_remote_finish(FWUPD_CLIENT(source), res, &helper->error);
	g_main_loop_quit(helper->loop);
}
				    ...
				    GError **error)
{
	...
	/* call async version and run loop until complete */
	fwupd_client_clean_remote_async(self,
					remote_id,
					cancellable,
			  ...
			  GError **error)
{
	...
	if (impl->clean_remote == NULL) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "no FwupdClientSyncImpl->clean_remote");
		return FALSE;
	}
	return impl->clean_remote(self, remote_id, impl_userdata, cancellable, error);
}
