...
static gboolean
fwupd_security_attr_from_json(FwupdCodec *codec, FwupdJsonObject *json_object, GError **error)
{
	FwupdSecurityAttr *self = FWUPD_SECURITY_ATTR(codec);
	const gchar *tmp;
	gint64 tmpi = 0;
	...
	/* this has to exist */
	tmp = fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_APPSTREAM_ID, error);
	if (tmp == NULL)
		return FALSE;
	...
	fwupd_security_attr_set_name(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_NAME, NULL));
	fwupd_security_attr_set_title(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_SUMMARY, NULL));
	fwupd_security_attr_set_description(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_DESCRIPTION, NULL));
	fwupd_security_attr_set_plugin(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_PLUGIN, NULL));
	fwupd_security_attr_set_fwupd_version(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_VERSION, NULL));
	fwupd_security_attr_set_url(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_URI, NULL));
	...
	fwupd_security_attr_set_bios_setting_id(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_BIOS_SETTING_ID, NULL));
	fwupd_security_attr_set_bios_setting_target_value(
	    self,
	    fwupd_json_object_get_string(json_object,
					 FWUPD_RESULT_KEY_BIOS_SETTING_TARGET_VALUE,
					 NULL));
	fwupd_security_attr_set_bios_setting_current_value(
	    self,
	...
	fwupd_security_attr_set_kernel_current_value(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_KERNEL_CURRENT_VALUE, NULL));
	fwupd_security_attr_set_kernel_target_value(
	    self,
	    fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_KERNEL_TARGET_VALUE, NULL));

	/* also optional */
	tmp = fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_HSI_RESULT, NULL);
	if (tmp != NULL)
		fwupd_security_attr_set_result(self, fwupd_security_attr_result_from_string(tmp));

	tmp = fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_HSI_RESULT_FALLBACK, NULL);
	if (tmp != NULL) {
		fwupd_security_attr_set_result_fallback(
		    self,
		    fwupd_security_attr_result_from_string(tmp));
	}
	tmp = fwupd_json_object_get_string(json_object, FWUPD_RESULT_KEY_HSI_RESULT_SUCCESS, NULL);
	if (tmp != NULL) {
		fwupd_security_attr_set_result_success(self,
						       fwupd_security_attr_result_from_string(tmp));
	}
	json_array1 = fwupd_json_object_get_array(json_object, FWUPD_RESULT_KEY_FLAGS, NULL);
	if (json_array1 != NULL) {
	...
	json_array2 = fwupd_json_object_get_array(json_object, FWUPD_RESULT_KEY_GUID, NULL);
	if (json_array2 != NULL) {
...
static void
fwupd_security_attr_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FwupdSecurityAttr *self = FWUPD_SECURITY_ATTR(codec);
	FwupdSecurityAttrPrivate *priv = GET_PRIVATE(self);
	...
	fwupd_codec_json_append_int(json_object, FWUPD_RESULT_KEY_HSI_LEVEL, priv->level);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_SUMMARY, priv->title);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_VERSION, priv->fwupd_version);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_URI, priv->url);
	fwupd_codec_json_append(json_object,
				FWUPD_RESULT_KEY_BIOS_SETTING_TARGET_VALUE,
