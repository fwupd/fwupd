				      ...
				      GString *str)
{
	...
	if (node_kind == FWUPD_JSON_NODE_KIND_RAW) {
		...
		g_autoptr(FwupdJsonArray) json_array = fwupd_json_node_get_array(json_node, NULL);
		...
		for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
			g_autoptr(FwupdJsonNode) json_child =
			    fwupd_json_array_get_node(json_array, i, NULL);
		...
		if (g_strcmp0(member_name, "") != 0)
			fwupd_codec_string_append(str, idt, member_name, "");
		for (guint i = 0; i < fwupd_json_object_get_size(json_object); i++) {
			const gchar *member_child = fwupd_json_object_get_key_for_index(json_object, i, NULL);
			g_autoptr(FwupdJsonNode) json_child =
			    fwupd_json_object_get_node_for_index(json_object, i, NULL);
...
fwupd_codec_add_string(FwupdCodec *self, guint idt, GString *str)
{
	...
	if (iface->add_json != NULL) {
		...
		g_autoptr(FwupdJsonNode) json_node = fwupd_json_node_new_object(json_object);
		iface->add_json(self, json_object, FWUPD_CODEC_FLAG_TRUSTED);
 ...
 * @self: a #FwupdCodec
 * @json_object: (not nullable): a JSON object
 ...
 */
gboolean
fwupd_codec_from_json(FwupdCodec *self, FwupdJsonObject *json_object, GError **error)
{
	FwupdCodecInterface *iface;
	...
	return (*iface->from_json)(self, json_object, error);
}
...
fwupd_codec_from_json_string(FwupdCodec *self, const gchar *json, GError **error)
{
	...
	if (json_object == NULL)
		return FALSE;
	return fwupd_codec_from_json(self, json_object, error);
}

/**
 * fwupd_codec_to_json:
 * @self: a #FwupdCodec
 * @json_object: (not nullable): a JSON builder
 ...
 */
void
fwupd_codec_to_json(FwupdCodec *self, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FwupdCodecInterface *iface;
	...
	iface->add_json(self, json_object, flags);
}
...
fwupd_codec_to_json_string(FwupdCodec *self, FwupdCodecFlags flags, GError **error)
{
	...
	fwupd_codec_to_json(self, json_object, flags);
	data = fwupd_json_object_to_string(json_object, FWUPD_JSON_EXPORT_FLAG_INDENT);
 ...
 * @json_object: (not nullable): a #FwupdJsonObject
 * @flags: a #FwupdCodecFlags, e.g. %FWUPD_CODEC_FLAG_TRUSTED
 *
			  ...
			  FwupdCodecFlags flags)
{
	...
	for (guint i = 0; i < array->len; i++) {
		...
		fwupd_codec_to_json(codec, json_object_tmp, flags);
		fwupd_json_array_add_object(json_array, json_object_tmp);
	}
	fwupd_json_object_add_array(json_object, member_name, json_array);
}
 ...
 */
void
fwupd_codec_json_append(FwupdJsonObject *json_object, const gchar *key, const gchar *value)
{
	g_return_if_fail(json_object != NULL);
	g_return_if_fail(key != NULL);
	if (value == NULL)
		return;
	fwupd_json_object_add_string(json_object, key, value);
}
 ...
 */
void
fwupd_codec_json_append_int(FwupdJsonObject *json_object, const gchar *key, guint64 value)
{
	g_return_if_fail(json_object != NULL);
	g_return_if_fail(key != NULL);
	fwupd_json_object_add_integer(json_object, key, value);
}
 ...
 */
void
fwupd_codec_json_append_bool(FwupdJsonObject *json_object, const gchar *key, gboolean value)
{
	g_return_if_fail(json_object != NULL);
	g_return_if_fail(key != NULL);
	fwupd_json_object_add_boolean(json_object, key, value);
}
 ...
 */
void
fwupd_codec_json_append_strv(FwupdJsonObject *json_object, const gchar *key, gchar **value)
{
	g_autoptr(FwupdJsonArray) json_array = fwupd_json_array_new();
	...
	for (guint i = 0; value[i] != NULL; i++)
		fwupd_json_array_add_string(json_array, value[i]);
	fwupd_json_object_add_array(json_object, key, json_array);
}
 ...
 */
void
fwupd_codec_json_append_map(FwupdJsonObject *json_object, const gchar *key, GHashTable *value)
{
	GHashTableIter iter;
	...
	while (g_hash_table_iter_next(&iter, &hash_key, &hash_value)) {
		fwupd_json_object_add_string(json_object_tmp,
					     (const gchar *)hash_key,
					     (const gchar *)hash_value);
	}
	fwupd_json_object_add_object(json_object, key, json_object_tmp);
}
