...
fwupd_json_object_get_key_for_index(FwupdJsonObject *self, guint idx, GError **error)
{
	...
	if (string == NULL) {
		g_autofree char *str = fwupd_rs_json_object_get_key_for_index(self->rs, idx, error);
		if (str == NULL)
			return NULL;

		string = g_ref_string_new(str);
...
fwupd_json_object_get_string(FwupdJsonObject *self, const gchar *key, GError **error)
{
	...
	if (string == NULL) {
		g_autofree char *str = fwupd_rs_json_object_get_string(self->rs, key, error);
		g_autofree char *htkey = g_strdup(key);
		if (str == NULL || htkey == NULL)
			return NULL;
