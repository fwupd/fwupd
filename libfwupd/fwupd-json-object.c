...
struct FwupdJsonObject {
	...
	GHashTable *strings; /* str:GRefString */
	GHashTable *keys;    /* int:GRefString */
};

static void
fwupd_json_object_invalidate_cache(FwupdJsonObject *self, const gchar *key)
{
	if (self->strings != NULL)
		g_hash_table_remove(self->strings, key);
	/* removing a key changes indices of other keys, so we invalidate */
	if (self->keys != NULL && !fwupd_rs_json_object_has_node(self->rs, key))
...
fwupd_json_object_clear(FwupdJsonObject *self)
{
	...
	if (self->keys != NULL)
		g_hash_table_remove_all(self->keys);
	if (self->strings != NULL)
		g_hash_table_remove_all(self->strings);
}
...
fwupd_json_object_get_key_for_index(FwupdJsonObject *self, guint idx, GError **error)
{
	...
	if (self->keys == NULL) {
		self->keys = g_hash_table_new_full(g_direct_hash,
						   g_direct_equal,
						   NULL,
	...
	string = g_hash_table_lookup(self->keys, GUINT_TO_POINTER(idx));
	if (string == NULL) {
		string = fwupd_rs_json_object_get_key_for_index(self->rs, idx, error);
		if (string == NULL)
			return NULL;

		g_hash_table_insert(self->keys, GUINT_TO_POINTER(idx), string);
	}
	return string;
}
...
fwupd_json_object_get_string(FwupdJsonObject *self, const gchar *key, GError **error)
{
	...
	if (self->strings == NULL) {
		self->strings = g_hash_table_new_full(g_str_hash,
						      g_str_equal,
						      g_free,
						      (GDestroyNotify)g_ref_string_release);
		if (self->strings == NULL) {
	...
	string = g_hash_table_lookup(self->strings, key);
	if (string == NULL) {
		g_autofree char *htkey = g_strdup(key);

		if (htkey == NULL)
			return NULL;

		string = fwupd_rs_json_object_get_string(self->rs, key, error);
		if (string == NULL)
			return NULL;

		g_hash_table_insert(self->strings, g_steal_pointer(&htkey), string);
	}
	return string;
}
					  ...
					  GError **error)
{
	...
	string = fwupd_json_object_get_string(self, key, &error_local);
	if (string == NULL) {
...
fwupd_json_object_get_keys(FwupdJsonObject *self)
{
	...
	size = fwupd_rs_json_object_get_size(self->rs);
	for (guint i = 0; i < size; i++) {
		GRefString *key = fwupd_json_object_get_key_for_index(self, i, NULL);
		if (key != NULL)
			g_ptr_array_add(json_keys, g_ref_string_acquire(key));
	}
...
fwupd_json_object_add_node(FwupdJsonObject *self, const gchar *key, FwupdJsonNode *json_node)
{
	...
	fwupd_json_object_invalidate_cache(self, key);
}
