 ...
 * @error: (nullable): optional return location for an error
 *
 * Gets the json_node for a given index position.
 *
 * Returns: (transfer full): a #GRefString, or %NULL on error
 *
 * Since: 2.1.0
 **/
FwupdJsonNode *
 ...
 * @key: (not nullable): dictionary key
 *
 * Finds id a json_node exists in a JSON object.
 *
 ...
 *
 * Returns: %TRUE if a json_node exists with the key.
 *
 * Since: 2.1.0
 **/
gboolean
 ...
 * @error: (nullable): optional return location for an error
 *
 * Gets a json_node from a JSON object.
 *
				   ...
				   FwupdJsonLoadFlags flags)
{
	...
	if (entry != NULL) {
		...
		entry->key = (flags > FWUPD_JSON_LOAD_FLAG_STATIC_KEYS)
 ...
 *
 * Adds a json_node to the JSON object. If the json_node already exists the old one is replaced.
 *
 * Since: 2.1.0
 **/
void
 ...
 *
 * Adds a raw value to the JSON object. If the json_node already exists the old one is replaced.
 *
 * Since: 2.1.0
 **/
void
 ...
 * @value: (not nullable): value
 *
 * Adds a string value to the JSON object. If the json_node already exists the old one is replaced.
 *
 * Since: 2.1.0
 **/
void
 ...
 * @value: boolean
 *
 * Adds an boolean value to the JSON object.
 *
 * Since: 2.1.0
 **/
void
				      ...
				      FwupdJsonObject *json_object)
{
	...
	entry->json_node = fwupd_json_node_new_object(json_object);
}
 ...
 **/
void
fwupd_json_object_add_object(FwupdJsonObject *self, const gchar *key, FwupdJsonObject *json_object)
{
	g_autoptr(FwupdJsonNode) json_node = NULL;
	...
	g_return_if_fail(self != json_object);

	json_node = fwupd_json_node_new_object(json_object);
	fwupd_json_object_add_node(self, key, json_node);
}
				     ...
				     FwupdJsonArray *json_array)
{
	...
	entry->json_node = fwupd_json_node_new_array(json_array);
}
 ...
 **/
void
fwupd_json_object_add_array(FwupdJsonObject *self, const gchar *key, FwupdJsonArray *json_array)
{
	g_autoptr(FwupdJsonNode) json_node = NULL;
	...
	json_node = fwupd_json_node_new_array(json_array);
	fwupd_json_object_add_node(self, key, json_node);
}
 ...
 * @self: a #FwupdJsonObject
 * @str: a #GString
 * @depth: depth, where 0 is the root json_node
				...
				FwupdJsonExportFlags flags)
{
	...
	for (guint i = 0; i < self->items->len; i++) {
		...
		if (kind == FWUPD_JSON_NODE_KIND_OBJECT) {
			...
			g_string_append_c(str, ' ');
			fwupd_json_object_append_string(json_object, str, depth + 1, flags);
		} else if (kind == FWUPD_JSON_NODE_KIND_ARRAY) {
			g_autoptr(FwupdJsonArray) json_array =
			    fwupd_json_node_get_array(json_node, NULL);
			g_string_append_c(str, ' ');
			fwupd_json_array_append_string(json_array, str, depth + 1, flags);
