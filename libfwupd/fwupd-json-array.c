 ...
 *
 * Gets a json_node from a JSON array.
 *
 ...
 *
 * Adds a json_node to a JSON array.
 *
 * Since: 2.1.0
 **/
void
 ...
 **/
void
fwupd_json_array_add_object(FwupdJsonArray *self, FwupdJsonObject *json_object)
{
	g_return_if_fail(self != NULL);
	g_return_if_fail(json_object != NULL);
	g_ptr_array_add(self->nodes, fwupd_json_node_new_object(json_object));
}
 ...
 **/
void
fwupd_json_array_add_array(FwupdJsonArray *self, FwupdJsonArray *json_array)
{
	g_return_if_fail(self != NULL);
	g_return_if_fail(json_array != NULL);
	g_return_if_fail(self != json_array);
	g_ptr_array_add(self->nodes, fwupd_json_node_new_array(json_array));
}
			       ...
			       FwupdJsonExportFlags flags)
{
	...
	for (guint i = 0; i < self->nodes->len; i++) {
		...
		if (kind == FWUPD_JSON_NODE_KIND_OBJECT) {
			...
			const gchar *json_str = fwupd_json_node_get_string(json_node, NULL);
