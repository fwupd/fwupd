...
static void
fwupd_release_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FwupdRelease *self = FWUPD_RELEASE(codec);
	FwupdReleasePrivate *priv = GET_PRIVATE(self);

	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_APPSTREAM_ID, priv->appstream_id);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_RELEASE_ID, priv->id);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_REMOTE_ID, priv->remote_id);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DESCRIPTION, priv->description);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_FILENAME, priv->filename);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_PROTOCOL, priv->protocol);
	if (priv->categories != NULL && priv->categories->len > 0) {
		...
		fwupd_json_object_add_array(json_object, FWUPD_RESULT_KEY_CATEGORIES, json_array);
	}
	if (priv->issues != NULL && priv->issues->len > 0) {
		g_autoptr(FwupdJsonArray) json_array = fwupd_json_array_new();
		for (guint i = 0; i < priv->issues->len; i++) {
	...
	if (priv->checksums != NULL && priv->checksums->len > 0) {
		...
		fwupd_json_object_add_array(json_object, FWUPD_RESULT_KEY_CHECKSUM, json_array);
	}
	if (priv->tags != NULL && priv->tags->len > 0) {
		g_autoptr(FwupdJsonArray) json_array = fwupd_json_array_new();
		for (guint i = 0; i < priv->tags->len; i++) {
			const gchar *tag = g_ptr_array_index(priv->tags, i);
			fwupd_json_array_add_string(json_array, tag);
		}
		fwupd_json_object_add_array(json_object, FWUPD_RESULT_KEY_TAGS, json_array);
	}
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_LICENSE, priv->license);
	if (priv->size > 0)
		fwupd_codec_json_append_int(json_object, FWUPD_RESULT_KEY_SIZE, priv->size);
	if (priv->created > 0)
	...
	if (priv->locations != NULL && priv->locations->len > 0) {
		...
		for (guint i = 0; i < priv->locations->len; i++) {
			...
			fwupd_json_array_add_string(json_array, location);
		}
		fwupd_json_object_add_array(json_object, FWUPD_RESULT_KEY_LOCATIONS, json_array);
	}
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_HOMEPAGE, priv->homepage);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DETAILS_URL, priv->details_url);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_SOURCE_URL, priv->source_url);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_SBOM_URL, priv->sbom_url);
	...
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DETACH_CAPTION, priv->detach_caption);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DETACH_IMAGE, priv->detach_image);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_UPDATE_MESSAGE, priv->update_message);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_UPDATE_IMAGE, priv->update_image);
	...
	if (priv->metadata != NULL) {
		...
		for (GList *l = keys; l != NULL; l = l->next) {
			...
			fwupd_codec_json_append(json_object, key, value);
		}
	}

	/* reports */
	if (priv->reports != NULL && priv->reports->len > 0)
		fwupd_codec_array_to_json(priv->reports, "Reports", json_object, flags);
}
