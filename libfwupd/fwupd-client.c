...
fwupd_client_upload_report_cb(GObject *source, GAsyncResult *res, gpointer user_data)
{
	...
	json_object = fwupd_json_node_get_object(json_root, &error);
	if (json_object == NULL) {
		g_task_return_error(task, g_steal_pointer(&error));
		return;
	}
	...
	server_msg = fwupd_json_object_get_string(json_object, "msg", NULL);
	...
	if (server_msg != NULL) {
		...
		uri = g_strdup(fwupd_json_object_get_string(json_object, "uri", NULL));
	}
				  ...
				  GError **error)
{
	...
	fwupd_json_object_add_string(json_object, "ReportType", "device-list");
	fwupd_json_object_add_integer(json_object, "ReportVersion", 2);
	if (priv->host_machine_id != NULL)
		fwupd_json_object_add_string(json_object, "MachineId", priv->host_machine_id);
	...
	if (g_hash_table_size(metadata) > 0) {
		...
		fwupd_json_object_add_object(json_object, "Metadata", json_metadata);
	}
	...
	fwupd_json_object_add_array(json_object, "Devices", json_devices);
...
fwupd_client_build_report_history_device(FwupdDevice *dev)
{
	...
	for (guint i = 0; checksum_types[i] != 0; i++) {
		...
		if (checksum != NULL) {
			fwupd_json_object_add_string(json_object, "Checksum", checksum);
			break;
		}
	}
	...
	if (checksums->len > 0) {
		...
		fwupd_json_object_add_array(json_object, "ChecksumDevice", json_array);
	}

	/* allow matching the specific component */
	fwupd_json_object_add_string(json_object, "ReleaseId", fwupd_release_get_id(rel));

	/* include the protocol used */
	if (fwupd_release_get_protocol(rel) != NULL) {
		fwupd_json_object_add_string(json_object,
					     "Protocol",
					     fwupd_release_get_protocol(rel));
	}

	/* set the error state of the report */
	fwupd_json_object_add_integer(json_object,
				      "UpdateState",
	...
	if (guids->len > 0) {
		...
		fwupd_json_object_add_array(json_object, "Guid", json_array);
	}

	fwupd_json_object_add_string(json_object, "Plugin", fwupd_device_get_plugin(dev));

	/* report what we're trying to update *from* and *to* */
	fwupd_json_object_add_string(json_object, "VersionOld", fwupd_device_get_version(dev));
	fwupd_json_object_add_string(json_object, "VersionNew", fwupd_release_get_version(rel));

	/* to know the state of the dev we're trying to update */
	fwupd_json_object_add_integer(json_object, "Flags", fwupd_device_get_flags(dev));

	/* to know when the update tried to happen, and how soon after boot */
	fwupd_json_object_add_integer(json_object, "Created", fwupd_device_get_created(dev));
	fwupd_json_object_add_integer(json_object, "Modified", fwupd_device_get_modified(dev));
				  ...
				  GError **error)
{
	...
	/* create header */
	fwupd_json_object_add_string(json_object, "ReportType", "history");
	fwupd_json_object_add_integer(json_object, "ReportVersion", 2);
	if (priv->host_machine_id != NULL)
		fwupd_json_object_add_string(json_object, "MachineId", priv->host_machine_id);
	...
	fwupd_json_object_add_array(json_object, "Reports", json_reports);
				   ...
				   GError **error)
{
	...
	/* create header */
	fwupd_json_object_add_string(json_object, "ReportType", "hsi");
	fwupd_json_object_add_integer(json_object, "ReportVersion", 2);
	if (priv->host_machine_id != NULL)
		fwupd_json_object_add_string(json_object, "MachineId", priv->host_machine_id);
	...
	fwupd_json_object_add_array(json_object, "SecurityAttributes", json_attrs);
