...
fwupd_json_node_unref(FwupdJsonNode *self)
{
	...
	g_clear_pointer(&self->string, g_ref_string_release);
	g_clear_pointer(&self->raw, g_ref_string_release);
	g_free(self);
	return NULL;
}
...
fwupd_json_node_get_raw(FwupdJsonNode *self, GError **error)
{
	...
	if (self->raw == NULL) {
		g_autofree char *str = fwupd_rs_json_node_get_raw(self->rs, error);
		if (str == NULL)
			return NULL;
		self->raw = g_ref_string_new(str);
	}
	return self->raw;
}
...
fwupd_json_node_get_string(FwupdJsonNode *self, GError **error)
{
	...
	if (self->string == NULL) {
		g_autofree char *str = fwupd_rs_json_node_get_string(self->rs, error);
		if (str == NULL)
			return NULL;
		self->string = g_ref_string_new(str);
	}
	return self->string;
}
