 ...
 **/
FwupdJsonNode *
fwupd_json_node_new_object(FwupdJsonObject *json_object)
{
	g_autoptr(FwupdJsonNode) self = fwupd_json_node_new_internal();

	g_return_val_if_fail(json_object != NULL, NULL);

	self->refcount = 1;
	self->kind = FWUPD_JSON_NODE_KIND_OBJECT;
	self->data = fwupd_json_object_ref(json_object);
 ...
 **/
FwupdJsonNode *
fwupd_json_node_new_array(FwupdJsonArray *json_array)
{
	g_autoptr(FwupdJsonNode) self = fwupd_json_node_new_internal();
	...
	self->data = fwupd_json_array_ref(json_array);
 ...
 *
 * Returns: (transfer none): a #FwupdJsonObject, or %NULL if the json_node was the wrong kind.
 *
 * Since: 2.1.0
 **/
FwupdJsonObject *
 ...
 *
 * Returns: (transfer none): a #FwupdJsonArray, or %NULL if the json_node was the wrong kind.
 *
 * Since: 2.1.0
 **/
FwupdJsonArray *
 ...
 *
 * Returns: a #GRefString, or %NULL if the json_node was the wrong kind.
 *
 * Since: 2.1.0
 **/
GRefString *
			      ...
			      FwupdJsonExportFlags flags)
{
	...
	if (self->kind == FWUPD_JSON_NODE_KIND_OBJECT) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_node_get_object(self, NULL);
		fwupd_json_object_append_string(json_object, str, depth + 1, flags);
		return;
	}
	if (self->kind == FWUPD_JSON_NODE_KIND_ARRAY) {
		g_autoptr(FwupdJsonArray) json_array = fwupd_json_node_get_array(self, NULL);
