...
static void
fwupd_report_add_json(FwupdCodec *codec, FwupdJsonObject *json_object, FwupdCodecFlags flags)
{
	FwupdReport *self = FWUPD_REPORT(codec);
	FwupdReportPrivate *priv = GET_PRIVATE(self);
	g_autoptr(GList) keys = NULL;

	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DEVICE_NAME, priv->device_name);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DISTRO_ID, priv->distro_id);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DISTRO_VARIANT, priv->distro_variant);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_DISTRO_VERSION, priv->distro_version);
	fwupd_codec_json_append(json_object, FWUPD_RESULT_KEY_VERSION_OLD, priv->version_old);
