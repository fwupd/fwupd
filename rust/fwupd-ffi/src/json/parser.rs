...
impl FwupdRsJsonParser {
    ...
    fn parser(&self) -> JsonParser {
        ...
        let depth = NonZeroUsize::new(self.max_depth).unwrap_or(NonZeroUsize::new(1).unwrap());
        let items = NonZeroUsize::new(self.max_items).unwrap_or(NonZeroUsize::new(1).unwrap());
        let quoted = NonZeroUsize::new(self.max_quoted).unwrap_or(NonZeroUsize::new(1).unwrap());
        JsonParser::builder()
            .max_depth(depth)
            .max_items(items)
            .max_quoted(quoted)
...
pub extern "C" fn fwupd_rs_json_parser_new() -> *mut FwupdRsJsonParser {
    let parser = FwupdRsJsonParser {
        max_depth: usize::from(u16::MAX),
        max_items: usize::from(u16::MAX),
        max_quoted: usize::from(u16::MAX),
    };
...
pub unsafe extern "C" fn fwupd_rs_json_parser_set_max_depth(
    ...
    if let Some(p) = unsafe { ptr.as_mut() } {
        p.max_depth = usize::try_from(max_depth.max(1)).unwrap_or(1);
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_parser_set_max_items(
    ...
    if let Some(p) = unsafe { ptr.as_mut() } {
        p.max_items = usize::try_from(max_items.max(1)).unwrap_or(1);
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_parser_set_max_quoted(
    ...
    if let Some(p) = unsafe { ptr.as_mut() } {
        p.max_quoted = usize::try_from(max_quoted.max(1)).unwrap_or(1);
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_parser_load_from_data(
    ...
    match p.parser().load_from_str(s, to_load_flags(flags)) {
        Ok(node) => Box::into_raw(Box::new(FwupdRsJsonNode::new(Arc::new(node)))),
        Err(e) => {
...
pub unsafe extern "C" fn fwupd_rs_json_parser_load_from_bytes(
    ...
    if data_len == 0 {
        return load_from_bytes(p, &[], flags, error);
    }
...
pub unsafe extern "C" fn fwupd_rs_json_parser_load_from_stream_impl(
    ...
    // Seek to start but ignore a NotSeekableError, json
    // streams may be decompressor streams and they can't seek, we
    // just assume they start at 0 then.
    if let Err(e) = guard.seek(SeekFrom::Start(0)) {
        let is_not_seekable = e.kind() == std::io::ErrorKind::Other
            && e.get_ref()
                .and_then(|inner| inner.downcast_ref::<fwupd::streams::NotSeekableError>())
                .is_some();
    ...
    match p
        .parser()
        .load_from_reader(guard.as_read(), to_load_flags(flags))
    {
        Ok(node) => Box::into_raw(Box::new(FwupdRsJsonNode::new(Arc::new(node)))),
        Err(e) => {
            GError::convert(error, &(&e).into());
