...
use crate::return_if_fail;

/// Opaque FFI handle for a JSON object.
///
/// Holds an `Arc<JsonObject>` so that cloning is cheap and multiple FFI
/// wrappers can share the same underlying data.
pub struct FwupdRsJsonObject {
    inner: Arc<JsonObject>,
}

impl FwupdRsJsonObject {
    pub(crate) fn new(obj: Arc<JsonObject>) -> Self {
        Self { inner: obj }
    }

    pub(crate) fn inner(&self) -> &Arc<JsonObject> {
        &self.inner
    }

    pub(crate) fn inner_mut(&mut self) -> &mut JsonObject {
...
pub extern "C" fn fwupd_rs_json_object_new() -> *mut FwupdRsJsonObject {
    Box::into_raw(Box::new(FwupdRsJsonObject::new(
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_size(ptr: *const FwupdRsJsonObject) -> c_uint {
    match unsafe { ptr.as_ref() } {
        Some(o) => c_uint::try_from(o.inner.len()).unwrap_or(c_uint::MAX),
        None => 0,
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_key_for_index(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_mut() }) => std::ptr::null_mut());

    if let Some((key, _)) = o.inner.iter().nth(idx as usize) {
        GRefString::from_rust_string(key)
    } else {
        GError::convert(
            error,
            &notfound!("index {} is larger than object size {}", idx, o.inner.len()),
        );
        std::ptr::null_mut()
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_node_for_index(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_ref() }) => std::ptr::null_mut());

    if let Some((_, node)) = o.inner.iter().nth(idx as usize) {
...
pub unsafe extern "C" fn fwupd_rs_json_object_has_node(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_ref() }) => 0);
    return_if_fail!(!key.is_null() => 0);

    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }.to_string_lossy();
    c_int::from(o.inner.get_node(&key_str).is_some())
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_string(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_mut() }) => std::ptr::null_mut());
    return_if_fail!(!key.is_null() => std::ptr::null_mut());
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_node(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }.to_string_lossy();
    if let Some(node) = o.inner.get_node(&key_str) {
        Box::into_raw(Box::new(FwupdRsJsonNode::new(node.clone())))
    } else {
        GError::convert(error, &notfound!("no json_node for key {key_str}"));
        std::ptr::null_mut()
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_string(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_mut() }));
    return_if_fail!(!key.is_null());

    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    if value.is_null() {
        o.inner_mut().add_null(&key_str);
    } else {
        let val_str = unsafe { std::ffi::CStr::from_ptr(value) }
            .to_string_lossy()
            .into_owned();
        o.inner_mut().add_string(&key_str, &val_str);
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_raw(
    ...
    let val_str = unsafe { std::ffi::CStr::from_ptr(value) }
        .to_string_lossy()
        .into_owned();
    o.inner_mut().add_raw(&key_str, &val_str);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_integer(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    o.inner_mut().add_integer(&key_str, value);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_boolean(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    o.inner_mut().add_boolean(&key_str, value != 0);
}

/// Adds an object value to the object. The object is cloned (not consumed).
#[no_mangle]
pub unsafe extern "C" fn fwupd_rs_json_object_add_object(
    ...
    o.inner_mut().add_object(&key_str, inner_obj);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_array(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_mut() }));
    return_if_fail!(!key.is_null());
    return_if_fail!(!arr.is_null());

    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    let inner_arr = unsafe { &*arr }.inner().clone();
    o.inner_mut().add_array(&key_str, inner_arr);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_node(
    ...
    return_if_fail!(Some(o) = (unsafe { ptr.as_mut() }));
    return_if_fail!(!key.is_null());
    return_if_fail!(!node.is_null());

    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    let inner_node = unsafe { &*node }.inner().clone();
    o.inner_mut().add_node(&key_str, inner_node);
}
