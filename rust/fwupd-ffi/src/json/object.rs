...
use fwupd::json::{JsonError, JsonObject};
...
///
/// Holds an `Arc<Mutex<JsonObject>>` to allow shared mutable access across
/// the FFI boundary, preserving the legacy C API's child mutation semantics.
pub struct FwupdRsJsonObject {
    inner: Arc<Mutex<JsonObject>>,
}

impl FwupdRsJsonObject {
    pub(crate) fn new(obj: Arc<Mutex<JsonObject>>) -> Self {
        Self { inner: obj }
    }

    pub(crate) fn inner(&self) -> &Arc<Mutex<JsonObject>> {
        &self.inner
    }
}
...
pub extern "C" fn fwupd_rs_json_object_new() -> *mut FwupdRsJsonObject {
    Box::into_raw(Box::new(FwupdRsJsonObject::new(Arc::new(Mutex::new(
        JsonObject::new(),
    )))))
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_size(ptr: *const FwupdRsJsonObject) -> c_uint {
    match unsafe { ptr.as_ref() } {
        Some(o) => c_uint::try_from(o.inner.lock().unwrap().len()).unwrap_or(c_uint::MAX),
        None => 0,
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_key_for_index(
    ...
    let o = unwrap_or_panic!(unsafe { ptr.as_mut() });
    let guard = o.inner.lock().unwrap();

    if let Some((key, _)) = guard.iter().nth(idx as usize) {
        GRefString::from_rust_string(key)
    } else {
        GError::convert(
            error,
            &notfound!("index {} is larger than object size {}", idx, guard.len()),
        );
        std::ptr::null_mut()
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_node_for_index(
    ...
    let o = unwrap_or_panic!(unsafe { ptr.as_ref() });
    let guard = o.inner.lock().unwrap();

    if let Some((_, node)) = guard.iter().nth(idx as usize) {
        Box::into_raw(Box::new(FwupdRsJsonNode::new(node.clone())))
    } else {
        GError::convert(
            error,
            &notfound!("index {} is larger than object size {}", idx, guard.len()),
...
pub unsafe extern "C" fn fwupd_rs_json_object_has_node(
    ...
    let o = unwrap_or_panic!(unsafe { ptr.as_ref() });
    panic_if!(key.is_null());

    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }.to_string_lossy();
    c_int::from(o.inner.lock().unwrap().get_node(&key_str).is_some())
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_string(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }.to_string_lossy();
    let guard = o.inner.lock().unwrap();
    match guard.get_node(&key_str) {
        None => {
            GError::convert(error, &notfound!("no json_node for key {key_str}"));
            std::ptr::null_mut()
        }
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_integer(
    ...
    match guard.get_node(&key_str) {
        ...
        Some(_) => match guard.get_integer(&key_str) {
            Ok(Some(v)) => {
                if !value.is_null() {
                    unsafe { *value = v };
                }
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_boolean(
    ...
    match guard.get_node(&key_str) {
        ...
        Some(_) => match guard.get_boolean(&key_str) {
            Ok(Some(v)) => {
                if !value.is_null() {
                    unsafe { *value = c_int::from(v) };
                }
                1
            }
            Ok(None) => {
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_node(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }.to_string_lossy();
    let guard = o.inner.lock().unwrap();
    if let Some(node) = guard.get_node(&key_str) {
        Box::into_raw(Box::new(FwupdRsJsonNode::new(node.clone())))
    } else {
        GError::convert(error, &notfound!("no json_node for key {key_str}"));
        std::ptr::null_mut()
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_object(
    ...
    match guard.get_node(&key_str) {
        ...
        Some(_) => match guard.get_object(&key_str) {
...
pub unsafe extern "C" fn fwupd_rs_json_object_get_array(
    ...
    match guard.get_node(&key_str) {
        ...
        Some(_) => match guard.get_array(&key_str) {
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_string(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    let mut guard = o.inner.lock().unwrap();
    if value.is_null() {
        guard.add_null(&key_str);
    } else {
        let val_str = unsafe { std::ffi::CStr::from_ptr(value) }
            .to_string_lossy()
            .into_owned();
        guard.add_string(&key_str, &val_str);
    }
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_raw(
    ...
    let val_str = unsafe { std::ffi::CStr::from_ptr(value) }
        .to_string_lossy()
        .into_owned();
    o.inner.lock().unwrap().add_raw(&key_str, &val_str);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_integer(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    o.inner.lock().unwrap().add_integer(&key_str, value);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_boolean(
    ...
    let key_str = unsafe { std::ffi::CStr::from_ptr(key) }
        .to_string_lossy()
        .into_owned();
    o.inner.lock().unwrap().add_boolean(&key_str, value != 0);
}

/// Adds an object value to the object. The object is shared (not consumed).
#[no_mangle]
pub unsafe extern "C" fn fwupd_rs_json_object_add_object(
    ...
    o.inner.lock().unwrap().add_object(&key_str, inner_obj);
}

/// Adds an array value to the object. The array is shared (not consumed).
#[no_mangle]
pub unsafe extern "C" fn fwupd_rs_json_object_add_array(
    ...
    o.inner.lock().unwrap().add_array(&key_str, inner_arr);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_add_node(
    ...
    o.inner.lock().unwrap().add_node(&key_str, inner_node);
}
...
pub unsafe extern "C" fn fwupd_rs_json_object_to_string(
    ...
    let guard = o.inner.lock().unwrap();
    let s = guard.to_json_string(export_flags);
