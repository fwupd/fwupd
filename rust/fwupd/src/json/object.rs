...
impl JsonObject {
    ...
    ///
    /// Returns [`JsonError::WrongType`] if the key exists but has the wrong type.
    pub fn get_object(&self, key: &str) -> Result<Option<Arc<JsonObject>>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
    ...
    ///
    /// Returns [`JsonError::WrongType`] if the key exists but has the wrong type.
    pub fn get_array(&self, key: &str) -> Result<Option<Arc<JsonArray>>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
    ...
    pub fn add_object(&mut self, key: &str, obj: Arc<JsonObject>) {
        self.add_node(key, Arc::new(JsonNode::Object(obj)));
    }

    /// Adds or replaces an array value.
    pub fn add_array(&mut self, key: &str, arr: Arc<JsonArray>) {
