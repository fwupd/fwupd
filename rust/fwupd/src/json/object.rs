...
use super::append_quoted_string;
use super::array::JsonArray;
use super::error::JsonError;
use super::node::JsonNode;
use super::{push_indent, ExportFlag, ExportFlags, LoadFlag, LoadFlags};
use crate::Bitflags;
...
/// entry is updated in-place (unless [`LoadFlag::Trusted`] is set, in which
/// case duplicates are appended without checking).
#[derive(Debug, Clone, PartialEq)]
pub struct JsonObject {
    entries: Vec<(Arc<str>, Arc<JsonNode>)>,
}
...
impl JsonObject {
    ...
    #[must_use]
    pub fn get_node(&self, key: &str) -> Option<Arc<JsonNode>> {
        self.entries
            .iter()
            .find(|(k, _)| &**k == key)
            .map(|(_, v)| Arc::clone(v))
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if the key exists but has the wrong type.
    pub fn get_string(&self, key: &str) -> Result<Option<&str>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
            Some((_, node)) => node.as_ref().get_string(),
            None => Ok(None),
        }
    }
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if the value is not a raw node, or
    /// [`JsonError::InvalidData`] if the raw value cannot be parsed as an integer.
    pub fn get_integer(&self, key: &str) -> Result<Option<i64>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
            Some((_, node)) => match node.as_ref().get_raw()? {
    ...
    ///
    /// Returns [`JsonError::WrongType`] if the value is not a raw node, or
    /// [`JsonError::InvalidData`] if the raw value cannot be parsed as a boolean.
    ...
    ///
    /// Returns [`JsonError::WrongType`] if the key exists but has the wrong type.
    pub fn get_object(&self, key: &str) -> Result<Option<Arc<Mutex<JsonObject>>>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
            Some((_, node)) => node.as_ref().get_object(),
            None => Ok(None),
        }
    }
    ...
    ///
    /// Returns [`JsonError::WrongType`] if the key exists but has the wrong type.
    pub fn get_array(&self, key: &str) -> Result<Option<Arc<Mutex<JsonArray>>>, JsonError> {
        match self.entries.iter().find(|(k, _)| &**k == key) {
            Some((_, node)) => node.as_ref().get_array(),
            None => Ok(None),
        }
    }
    ...
    pub fn add_node(&mut self, key: &str, node: Arc<JsonNode>) {
    ...
    pub fn add_null(&mut self, key: &str) {
        self.add_node(key, Arc::new(JsonNode::Null));
    }
    ...
    pub fn add_object(&mut self, key: &str, obj: Arc<Mutex<JsonObject>>) {
        self.add_node(key, Arc::new(JsonNode::Object(obj)));
    }

    /// Adds or replaces an array value.
    pub fn add_array(&mut self, key: &str, arr: Arc<Mutex<JsonArray>>) {
        self.add_node(key, Arc::new(JsonNode::Array(arr)));
    }
    ...
    pub(crate) fn add_node_internal(
        ...
        if !flags.contains(LoadFlag::Trusted) {
            if let Some(entry) = self.entries.iter_mut().find(|(k, _)| k == &key) {
                entry.1 = node;
                return;
            }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, (Arc<str>, Arc<JsonNode>)> {
        self.entries.iter()
    }
}

impl<'a> IntoIterator for &'a JsonObject {
    type Item = &'a (Arc<str>, Arc<JsonNode>);
    type IntoIter = std::slice::Iter<'a, (Arc<str>, Arc<JsonNode>)>;
...
impl From<JsonObject> for String {
    fn from(object: JsonObject) -> Self {
        object.to_json_string(ExportFlag::Indent.into())
    }
}
...
mod object_tests {
    ...
    fn parse_integer() {
        ...
        // overflow: i64::MAX and i64::MIN are rejected, as are values beyond range
    ...
    fn parse_boolean() {
        ...
        assert!(super::parse_boolean("true").unwrap());
        assert!(!super::parse_boolean("false").unwrap());
        assert!(super::parse_boolean("TRUE").unwrap());
        assert!(!super::parse_boolean("False").unwrap());
