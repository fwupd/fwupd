...
use super::{ExportFlag, ExportFlags};
use crate::Bitflags;
...
///
/// Arrays and Objects are wrapped in `Mutex` to allow shared mutable access
/// across the FFI boundary, preserving the legacy C API's child mutation
/// semantics.
#[derive(Debug, Clone)]
pub enum JsonNode {
    ...
    Str(String),
    /// A JSON array.
    Array(Arc<Mutex<JsonArray>>),
    /// A JSON object.
    Object(Arc<Mutex<JsonObject>>),
}
...
impl JsonNode {
    ...
    ///
    /// Returns `Ok(None)` if this is a `Null` node.
    ///
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if this node is not a raw value or null.
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if this node is not a string or null.
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if this node is not an object or null.
    pub fn get_object(&self) -> Result<Option<Arc<Mutex<JsonObject>>>, JsonError> {
        match self {
            Self::Object(obj) => Ok(Some(Arc::clone(obj))),
            Self::Null => Ok(None),
            other => Err(JsonError::WrongType(format!(
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::WrongType`] if this node is not an array or null.
    pub fn get_array(&self) -> Result<Option<Arc<Mutex<JsonArray>>>, JsonError> {
        match self {
            Self::Array(arr) => Ok(Some(Arc::clone(arr))),
            Self::Null => Ok(None),
            other => Err(JsonError::WrongType(format!(
    ...
    pub(crate) fn append_to_string(&self, out: &mut String, depth: usize, flags: ExportFlags) {
        match self {
            ...
            Self::Array(arr) => arr.lock().unwrap().append_to_string(out, depth, flags),
            Self::Object(obj) => obj.lock().unwrap().append_to_string(out, depth, flags),
        }
    }
}
...
impl PartialEq for JsonNode {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Raw(a), Self::Raw(b)) | (Self::Str(a), Self::Str(b)) => a == b,
            (Self::Array(a), Self::Array(b)) => {
                // Lock both mutexes and compare the contents
...
impl std::fmt::Display for JsonNode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt_node(self, f, 0, ExportFlags::empty())
    }
}

impl From<JsonNode> for String {
    fn from(node: JsonNode) -> Self {
        node.to_json_string(ExportFlag::Indent.into())
    }
}
...
mod node_tests {
    ...
    fn get_typed_wrong_type() {
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(Mutex::new(JsonArray::new()))).get_raw(),
            Err(JsonError::WrongType(_))
        ));
        assert!(matches!(
            JsonNode::Object(Arc::new(Mutex::new(JsonObject::new()))).get_raw(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(Mutex::new(JsonArray::new()))).get_string(),
            Err(JsonError::WrongType(_))
        ));
        assert!(matches!(
            JsonNode::Object(Arc::new(Mutex::new(JsonObject::new()))).get_string(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(Mutex::new(JsonArray::new()))).get_object(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Object(Arc::new(Mutex::new(JsonObject::new()))).get_array(),
            Err(JsonError::WrongType(_))
        ));
    }
    ...
    fn kind_and_display() {
        ...
        assert_eq!(
            JsonNode::Array(Arc::new(Mutex::new(JsonArray::new()))).kind(),
            NodeKind::Array
        );
        assert_eq!(
            JsonNode::Object(Arc::new(Mutex::new(JsonObject::new()))).kind(),
            NodeKind::Object
        );
