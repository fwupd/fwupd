...
impl JsonNode {
    ...
    pub fn get_object(&self) -> Result<Option<Arc<JsonObject>>, JsonError> {
        match self {
    ...
    pub fn get_array(&self) -> Result<Option<Arc<JsonArray>>, JsonError> {
        match self {
    ...
    pub(crate) fn append_to_string(&self, out: &mut String, depth: usize, flags: ExportFlags) {
        match self {
            ...
            Self::Array(arr) => arr.as_ref().append_to_string(out, depth, flags),
            Self::Object(obj) => obj.as_ref().append_to_string(out, depth, flags),
        }
    }
}
...
mod node_tests {
    ...
    fn get_typed_wrong_type() {
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(JsonArray::new())).get_raw(),
            Err(JsonError::WrongType(_))
        ));
        assert!(matches!(
            JsonNode::Object(Arc::new(JsonObject::new())).get_raw(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(JsonArray::new())).get_string(),
            Err(JsonError::WrongType(_))
        ));
        assert!(matches!(
            JsonNode::Object(Arc::new(JsonObject::new())).get_string(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Array(Arc::new(JsonArray::new())).get_object(),
            Err(JsonError::WrongType(_))
        ));
        ...
        assert!(matches!(
            JsonNode::Object(Arc::new(JsonObject::new())).get_array(),
            Err(JsonError::WrongType(_))
        ));
    }
    ...
    fn kind_and_display() {
        ...
        assert_eq!(
            JsonNode::Array(Arc::new(JsonArray::new())).kind(),
            NodeKind::Array
        );
        assert_eq!(
            JsonNode::Object(Arc::new(JsonObject::new())).kind(),
            NodeKind::Object
        );
