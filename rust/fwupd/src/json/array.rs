...
impl JsonArray {
    ...
    pub fn get_object(&self, idx: usize) -> Result<Option<Arc<Mutex<JsonObject>>>, JsonError> {
    ...
    pub fn get_array(&self, idx: usize) -> Result<Option<Arc<Mutex<JsonArray>>>, JsonError> {
        match self.get_node(idx) {
    ...
    pub fn add_object(&mut self, obj: Arc<Mutex<JsonObject>>) {
        self.nodes.push(Arc::new(JsonNode::Object(obj)));
    }

    /// Adds another array to the end of this array.
    pub fn add_array(&mut self, arr: Arc<Mutex<JsonArray>>) {
...
mod array_tests {
    ...
    fn get_out_of_bounds() {
        ...
        assert!(arr.get_object(0).unwrap().is_none());
        assert!(arr.get_array(0).unwrap().is_none());
    }
