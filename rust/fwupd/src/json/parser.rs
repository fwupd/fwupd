...
struct Helper {
    ...
    depth: usize,
    string_cache: HashMap<Box<str>, Arc<str>>,
}

impl Helper {
    fn new(max_quoted: usize, flags: LoadFlags) -> Self {
        Self {
            flags,
            buf: vec![0u8; 32 * 1024],
            buf_len: 0,
    ...
    fn tokenize(&mut self) -> Option<Token> {
        if self.is_quoted {
            // Clone the accumulator, the values are small and this way
            // we can use acc.clear() which keeps the vec capacity and
            // reduces allocations.
            let s = std::str::from_utf8(&self.acc).map_or_else(
                |_| String::from_utf8_lossy(&self.acc).into_owned(),
                std::borrow::ToOwned::to_owned,
            );
            self.acc.clear();
            // If the string contains a null byte, we quietly drop it and anything after.
            // This matches the old C parser behavior which treated all strings as
            // char and thus implicitly truncated after \0.
            let s = s.split('\0').next().unwrap_or(&s);
            return Some(Token::String(String::from(s)));
        }
        ...
        if self.acc.eq_ignore_ascii_case(b"null") {
            ...
            //
            // See comment above regarding arc.clone()
            let s = std::str::from_utf8(&self.acc)
    ...
    fn process_byte(&mut self, data: u8) -> Result<(Option<Token>, bool), JsonError> {
        ...
        if self.is_quoted {
            ...
            self.newline_count = 0;
            self.whitespace_count = 0;
            if self.acc.len() > self.max_quoted {
        ...
        if (data as char).is_ascii_whitespace() {
    ...
    // Return a cached version of the given string for reuse.
    fn cached_string(&mut self, newstr: &str) -> Arc<str> {
        if let Some(s) = self.string_cache.get(newstr) {
            Arc::clone(s)
        } else {
            let s: Arc<str> = Arc::from(newstr);
            self.string_cache.insert(Box::from(newstr), Arc::clone(&s));
            s
        }
    }
}
...
impl JsonParser {
    ...
    /// # Panics
    ///
    /// Cannot panic -- the default value (`u16::MAX`) is always non-zero.
    #[must_use]
    pub fn builder() -> JsonParserBuilder {
        let default = NonZeroUsize::new(usize::from(u16::MAX)).unwrap();
        JsonParserBuilder {
            max_depth: default,
            max_items: default,
            max_quoted: default,
    ...
    /// # Errors
    ///
    /// Returns [`JsonError::InvalidData`] if the JSON is malformed or exceeds
    /// the configured parser limits.
    pub fn load_from_str(&self, text: &str, flags: LoadFlags) -> Result<JsonNode, JsonError> {
        self.load(&mut text.as_bytes(), flags)
    }
    ...
    ///
    /// Returns [`JsonError::InvalidData`] if the JSON is malformed or exceeds
    /// the configured parser limits, or [`JsonError::IoError`] if the reader
    ...
    fn load(&self, reader: &mut dyn Read, flags: LoadFlags) -> Result<JsonNode, JsonError> {
        let mut helper = Helper::new(self.max_quoted.get(), flags);

        let token = helper.next_token(reader)?;
        match token {
            Token::ObjectStart => {
                let obj = self.load_object(&mut helper, reader)?;
                Ok(JsonNode::Object(Arc::new(Mutex::new(obj))))
            }
            Token::ArrayStart => {
                let arr = self.load_array(&mut helper, reader)?;
                Ok(JsonNode::Array(Arc::new(Mutex::new(arr))))
            }
        ...
        loop {
            ...
            match token {
                ...
                Token::ObjectStart => {
                    ...
                    arr.add_object(Arc::new(Mutex::new(obj)));
                }
                Token::ArrayStart => {
                    let inner = self.load_array(helper, reader)?;
                    arr.add_array(Arc::new(Mutex::new(inner)));
                }
                Token::String(s) => {
                ...
                Token::Null => {
                    return Err(JsonError::InvalidData(
                        "null values not expected in array".to_owned(),
                    ));
                }
                _ => {
                    return Err(JsonError::InvalidData(
    ...
    fn load_object(
        ...
        loop {
            ...
            let key = match token1 {
                ...
                Token::String(s) => helper.cached_string(&s),
                _ => {
            ...
            let node = match token3 {
                Token::ObjectStart => {
                    ...
                    Arc::new(JsonNode::Object(Arc::new(Mutex::new(inner))))
                }
                Token::ArrayStart => {
                    let inner = self.load_array(helper, reader)?;
                    Arc::new(JsonNode::Array(Arc::new(Mutex::new(inner))))
                }
...
impl JsonParserBuilder {
    ...
    pub fn max_depth(mut self, max_depth: NonZeroUsize) -> Self {
        self.max_depth = max_depth;
        self
    }

    /// Sets the maximum number of items in an array or object.
    pub fn max_items(mut self, max_items: NonZeroUsize) -> Self {
        self.max_items = max_items;
        self
    }

    /// Sets the maximum length of a quoted string.
    pub fn max_quoted(mut self, max_quoted: NonZeroUsize) -> Self {
...
mod parser_tests {
    ...
    fn parser_with_limits(depth: usize, items: usize, quoted: usize) -> JsonParser {
        JsonParser::builder()
            .max_depth(NonZeroUsize::new(depth).unwrap())
            .max_items(NonZeroUsize::new(items).unwrap())
            .max_quoted(NonZeroUsize::new(quoted).unwrap())
            .build()
    }

    fn assert_err_contains(input: &str, parser: &JsonParser, expected_substr: &str) {
        let result = parser.load_from_str(input, LoadFlags::empty());
        match result {
    ...
    fn err_token_too_long() {
        ...
        let result = parser.load_from_str("\"abc\"", LoadFlags::empty());
    ...
    fn err_depth_at_limit_passes() {
        ...
        let result = parser.load_from_str("[[1]]", LoadFlags::empty());
    ...
    fn err_top_level_invalid_token() {
        ...
        let result = parser2.load_from_str("[null]", LoadFlags::empty());
    ...
    fn err_too_many_items_array_at_limit_passes() {
        let parser = parser_with_limits(10, 3, 10);
        let result = parser.load_from_str("[1,2,3]", LoadFlags::empty());
    ...
    fn err_too_many_items_object_at_limit_passes() {
        let parser = parser_with_limits(10, 2, 10);
        let result = parser.load_from_str("{\"a\": 1, \"b\": 2}", LoadFlags::empty());
