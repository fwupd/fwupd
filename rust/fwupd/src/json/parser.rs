...
impl Helper {
    fn new(max_quoted: u32, flags: LoadFlags) -> Self {
        Self {
            flags,
            buf: vec![0u8; 32 * 1024],
            buf_len: 0,
    ...
    fn process_byte(&mut self, data: u8) -> Result<(Option<Token>, bool), JsonError> {
        ...
        if self.is_quoted {
            ...
            self.newline_count = 0;
            self.whitespace_count = 0;
            if u32::try_from(self.acc.len()).unwrap_or(u32::MAX) > self.max_quoted {
...
impl JsonParser {
    ...
    pub fn builder() -> JsonParserBuilder {
        let default = NonZeroU32::new(u32::from(u16::MAX)).unwrap();
        JsonParserBuilder {
            max_depth: default,
            max_items: default,
            max_quoted: default,
    ...
    fn load_array(
        ...
        loop {
            ...
            if u32::try_from(arr.len()).unwrap_or(u32::MAX) > self.max_items.get() {
                return Err(JsonError::InvalidData(format!(
    ...
    fn load_object(
        ...
        loop {
            ...
            if u32::try_from(obj.len()).unwrap_or(u32::MAX) > self.max_items.get() {
                return Err(JsonError::InvalidData(format!(
