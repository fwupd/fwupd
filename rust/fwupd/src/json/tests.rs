...
/// Helper to create a `NonZeroUsize` from a usize literal in test code.
...
fn parser_depth() {
    ...
    let result = parser.load_from_str(json, LoadFlags::empty());
...
fn parser_valid() {
    ...
    for input in &data {
        let node = parser
            .load_from_str(input, LoadFlags::empty())
            .unwrap_or_else(|e| panic!("failed to parse '{input}': {e}"));
        let output = node.to_json_string(ExportFlags::empty());
...
fn object_construction() {
    ...
    // export with indent
    let s = obj.to_json_string(ExportFlag::Indent.into());
    let expected = "\
{
  \"one\": \"bob\",
  \"two\": \"clara\\ndave\",
  ...
  \"seven\": null
    ...
    let bytes = obj.to_json_string(ExportFlag::Indent.into());
    ...
    arr.add_string("dave");
    obj.add_array("array", Arc::new(Mutex::new(arr)));

    // add object
    let mut obj2 = JsonObject::new();
    obj2.add_integer("int", 123);
    obj.add_object("object", Arc::new(Mutex::new(obj2)));
    ...
    assert!(obj.get_boolean("XXX").unwrap().unwrap_or(true));

    // final export
    let s2 = obj.to_json_string(ExportFlag::Indent.into());
    let expected2 = "\
{
  \"one\": \"bob\",
  \"two\": \"clara\\ndave\",
...
fn node_types() {
    ...
    assert_eq!(node.to_json_string(ExportFlags::empty()), "dave");
...
fn array_construction() {
    ...
    let s = arr.to_json_string(ExportFlag::Indent.into());
    assert_eq!(s, "[\n  \"hello\",\n  world\n]");
}
...
fn parser_valid_large_limits() {
    ...
    let node = parser.load_from_str(json, LoadFlags::empty()).unwrap();
    let obj = node.get_object().unwrap().unwrap();
    let arr = obj.lock().unwrap().get_array("devices").unwrap().unwrap();
    assert_eq!(arr.lock().unwrap().len(), 1);
    let dev = arr.lock().unwrap().get_object(0).unwrap().unwrap();
    assert_eq!(
        dev.lock().unwrap().get_string("name").unwrap(),
        Some("test")
    );
    assert_eq!(
        dev.lock().unwrap().get_string("version").unwrap(),
        Some("1.0")
    );
    let flags = dev.lock().unwrap().get_array("flags").unwrap().unwrap();
    assert_eq!(flags.lock().unwrap().len(), 2);
    assert_eq!(
        flags.lock().unwrap().get_string(0).unwrap(),
        Some("verified")
    );
    assert_eq!(
        flags.lock().unwrap().get_string(1).unwrap(),
        Some("updatable")
    );
}
...
fn trailing_newline_flag() {
    ...
    let s = obj.to_json_string(ExportFlag::Indent | ExportFlag::TrailingNewline);
    assert!(s.ends_with('\n'));
    let s2 = obj.to_json_string(ExportFlag::Indent.into());
    assert!(!s2.ends_with('\n'));
}
...
fn empty_containers() {
    let parser = make_parser();
    let node = parser.load_from_str("[]", LoadFlags::empty()).unwrap();
    assert_eq!(node.to_json_string(ExportFlags::empty()), "[]");

    let node2 = parser.load_from_str("{}", LoadFlags::empty()).unwrap();
    assert_eq!(node2.to_json_string(ExportFlags::empty()), "{}");
}
...
fn null_node_getters_return_none() {
    ...
    assert!(node.get_object().unwrap().is_none());
    assert!(node.get_array().unwrap().is_none());
    assert_eq!(node.kind(), NodeKind::Null);
    assert_eq!(node.to_json_string(ExportFlags::empty()), "null");
}
...
fn null_in_array_rejected() {
    ...
    let result = parser.load_from_str("[null]", LoadFlags::empty());
...
fn non_ascii_roundtrip() {
    ...
    let obj = node.get_object().unwrap().unwrap();
    assert_eq!(
        obj.lock().unwrap().get_string("name").unwrap(),
        Some("café")
    );
    let output = node.to_json_string(ExportFlags::empty());
...
// Tests for NUL byte handling
// Note: The parser currently accepts literal NUL bytes inside quoted strings
// (matching C parser behavior), but rejects them outside quotes.
#[test]
fn literal_nul_in_quoted_string() {
    let parser = make_parser();
    // Literal NUL byte inside quoted string: "hello\0world"
    let json_bytes: &[u8] = b"\"hello\x00null\"";
    let result = parser.load_from_bytes(json_bytes, LoadFlags::empty());
    // Currently accepted and truncated (matches C parser behavior)
    assert!(result.is_ok());
    if let Ok(node) = result {
        assert_eq!(node.get_string().unwrap(), Some("hello"));
    }
}

#[test]
fn literal_nul_outside_quotes_rejected() {
    let parser = make_parser();
    // Literal NUL byte outside quoted string should be rejected
    let json_bytes: &[u8] = b"\x00{}";
    let result = parser.load_from_bytes(json_bytes, LoadFlags::empty());
    assert!(result.is_err());
}
