...
pub(crate) mod test_helpers {
    ...
    unsafe extern "C" fn test_read(
        ...
        let stream = unsafe { &mut *(handle as *mut TestStream) };
        let slice = unsafe { std::slice::from_raw_parts_mut(buf, count) };
        use std::io::Read;
        stream.inner.read(slice).unwrap_or(0) as isize
    }
    ...
    unsafe extern "C" fn test_seek(
        ...
        let seek_from = match seek_type {
            ...
            G_SEEK_SET => SeekFrom::Start(offset as u64),
            G_SEEK_END => SeekFrom::End(offset),
            _ => return 0,
        };
        use std::io::Seek;
        if stream.inner.seek(seek_from).is_ok() {
            1
        } else {
            0
        }
    ...
    unsafe extern "C" fn test_tell(handle: CStreamHandle) -> i64 {
        let stream = unsafe { &mut *(handle as *mut TestStream) };
        use std::io::Seek;
        stream.inner.stream_position().unwrap_or(0) as i64
    }
