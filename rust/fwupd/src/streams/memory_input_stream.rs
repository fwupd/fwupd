...
pub struct MemoryInputStream<T: AsRef<[u8]>> {
    data: T,
    pos: usize,
}

impl<T: AsRef<[u8]>> MemoryInputStream<T> {
...
impl Default for MemoryInputStream<Vec<u8>> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<T: AsRef<[u8]>> IsSeekable for MemoryInputStream<T> {
    fn is_seekable(&self) -> bool {
        true
    }
}

impl<T: AsRef<[u8]>> Read for MemoryInputStream<T> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        let slice = self.data.as_ref();
        let available = slice.len().saturating_sub(self.pos);
        let count = buf.len().min(available);
        if count > 0 {
            buf[..count].copy_from_slice(&slice[self.pos..self.pos + count]);
            self.pos += count;
        }
        Ok(count)
    }
...
impl<T: AsRef<[u8]>> Seek for MemoryInputStream<T> {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        let slice = self.data.as_ref();
        let new_pos = match pos {
            SeekFrom::Start(n) => i64::try_from(n).ok(),
            SeekFrom::Current(n) => i64::try_from(self.pos).ok().and_then(|p| p.checked_add(n)),
            SeekFrom::End(n) => i64::try_from(slice.len())
                .ok()
                .and_then(|len| len.checked_add(n)),
        }
        .ok_or_else(|| std::io::Error::from(std::io::ErrorKind::InvalidInput))?;
        let len =
            i64::try_from(slice.len()).map_err(|_| io::Error::other("data length overflow"))?;
        if new_pos < 0 || new_pos > len {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("seek to {new_pos} out of range [0, {}]", slice.len()),
            ));
        }
