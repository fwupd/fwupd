...
///
/// Sums bytes in `buf[offset..offset+n]` with 8-bit wrapping.
/// Returns `Err` if the range is out of bounds or overflows.
pub fn sum8_range(buf: &[u8], offset: usize, n: usize) -> Result<u8, Error> {
...
///
/// Sums bytes in `buf[offset..offset+n]` into a 16-bit accumulator.
/// Returns `Err` if the range is out of bounds or overflows.
pub fn sum16_range(buf: &[u8], offset: usize, n: usize) -> Result<u16, Error> {
    let end = offset.checked_add(n).ok_or(Error::Overflow)?;
    if end > buf.len() {
        return Err(Error::InvalidRange);
    }
...
mod tests {
    ...
    #[test]
    fn test_sum8_range() {
        assert_eq!(sum8_range(&TEST_BUF, 0, 5), Ok(0x6A));
        assert_eq!(sum8_range(&TEST_BUF, 0x33, 0x999), Err(Error::InvalidRange));
        assert_eq!(sum8_range(&TEST_BUF, usize::MAX, 1), Err(Error::Overflow));
    }
    ...
    #[test]
    fn test_sum16_range() {
        assert_eq!(sum16_range(&TEST_BUF, 0, 5), Ok(0x016A));
        assert_eq!(
            sum16_range(&TEST_BUF, 0x33, 0x999),
    ...
    fn test_sum16w() {
        ...
        // Odd length
        assert_eq!(
            sum16w(&[0x01, 0x02, 0x03], Endian::Little),
    ...
    fn test_sum32w() {
        ...
        // Not a multiple of 4
        assert_eq!(
            sum32w(&[0x01, 0x02, 0x03], Endian::Little),
            Err(Error::InvalidAlignment)
        );
    }
}
