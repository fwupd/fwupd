...
///
/// XORs bytes in `buf[offset..offset+n]` and combines the result with
/// `value`. Returns `Err` if the range is out of bounds or overflows.
///
...
///
/// ```
/// use fwupd::xor::{xor8_range, Error};
///
/// let buf = [0x12, 0x23, 0x45, 0x67, 0x89];
/// assert_eq!(xor8_range(&buf, 0, 5, 0), Ok(0x9A));
///
/// // XORing again cancels out
/// assert_eq!(xor8_range(&buf, 0, 5, 0x9A), Ok(0x00));
///
/// // Out of bounds
/// assert_eq!(xor8_range(&buf, 0x33, 0x999, 0), Err(Error::InvalidRange));
/// ```
pub fn xor8_range(buf: &[u8], offset: usize, n: usize, value: u8) -> Result<u8, Error> {
...
mod tests {
    ...
    #[test]
    fn xor8_range_basic() {
        assert_eq!(xor8_range(&TEST_BUF, 0, 5, 0), Ok(0x9A));
    }

    #[test]
    fn xor8_range_accumulates() {
        let value = xor8_range(&TEST_BUF, 0, 5, 0).unwrap();
        assert_eq!(value, 0x9A);
        // XOR again cancels out
        let value = xor8_range(&TEST_BUF, 0, 5, value).unwrap();
        assert_eq!(value, 0x00);
    }

    #[test]
    fn xor8_range_out_of_bounds() {
        assert_eq!(
            xor8_range(&TEST_BUF, 0x33, 0x999, 0),
            Err(Error::InvalidRange)
        );
    }

    #[test]
    fn xor8_range_overflow() {
        assert_eq!(
            xor8_range(&TEST_BUF, usize::MAX, 1, 0),
            Err(Error::Overflow)
        );
    }
}
