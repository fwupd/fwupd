...
fu_util_jcat_import(FuUtil *self, gchar **values, GError **error)
{
	...
	if (item == NULL) {
		item = fwupd_jcat_item_new(values[1]);
...
fu_util_jcat_self_sign(FuUtil *self, gchar **values, GError **error)
{
	...
	if (g_strv_length(values) > 3) {
		if (!fu_util_jcat_blob_kind_from_string(values[2], &target, NULL, error))
			return FALSE;
	}
...
fu_util_jcat_verify(FuUtil *self, gchar **values, GError **error)
{
	...
	if (g_strv_length(values) > 1) {
		...
		if (item == NULL)
			return FALSE;
		if (!fu_util_jcat_verify_item(self, item, kind, variant, error))
			return FALSE;
	} else {
