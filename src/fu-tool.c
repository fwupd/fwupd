...
fu_util_get_updates (FuUtilPrivate *priv, gchar **values, GError **error)
{
	...
	for (guint i = 0; i < devices->len; i++) {
		...
		g_print ("%s", fu_util_device_to_string (dev, NULL, NULL));
...
fu_util_get_devices (FuUtilPrivate *priv, gchar **values, GError **error)
{
	...
	for (guint i = 0; i < devs->len; i++) {
		...
		if (priv->show_all_devices || fu_util_is_interesting_device (dev)) {
			g_autofree gchar *tmp = fu_util_device_to_string (dev, NULL, NULL);
			g_print ("%s\n", tmp);
		}
	}
...
fu_util_device_added_cb (FwupdClient *client,
			 ...
			 gpointer user_data)
{
	g_autofree gchar *tmp = fu_util_device_to_string (device, NULL, NULL);
