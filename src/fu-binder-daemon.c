						...
						FuBinderDaemon *self)
{
	...
	if (notify) {
		...
		g_variant_builder_add(&builder,
				      "{sv}",
				      "Percentage",
				      g_variant_new_uint32(fwupd_percentage_is_valid(percentage)
							       ? (guint32)percentage
							       : 0));
...
fu_binder_daemon_get_devices_as_variant(FuBinderDaemon *self, GError **error)
{
	...
	flags = fu_engine_request_get_converter_flags(request);
					   ...
					   GError **error)
{
	...
	for (guint i = 0; property_names[i] != NULL; i++) {
		...
		if (g_strcmp0(key, "DaemonVersion") == 0) {
			...
			FuContext *ctx = fu_engine_get_context(engine);
			val = g_variant_new_boolean(fu_context_get_config_bool(ctx, "OnlyTrusted"));
		} else {
			g_set_error(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "unknown daemon property %s",
				    key);
			return NULL;
		}
					    ...
					    FuBinderDaemon *self)
{
	...
	val = fu_binder_daemon_get_properties_as_variant(self, props, &error);
	...
	if (options != NULL) {
		...
		while (g_variant_iter_next(iter, "{&sv}", &prop_key, &prop_value)) {
			if (g_strcmp0(prop_key, "install-flags") == 0)
				helper->flags = fwupd_variant_get_uint64(prop_value);
					  ...
					  FuBinderDaemon *self)
{
	...
	g_autoptr(GVariant) val = fu_binder_daemon_get_properties_as_variant(self, props, &error);
	fu_binder_bridge_emit_properties_changed(val);
	fu_binder_daemon_set_status(self, status);
