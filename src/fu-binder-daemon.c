...
static void
fu_binder_daemon_log_handler(const gchar *log_domain,
			     ...
			     gpointer user_data)
{
	...
	if (fp == NULL)
		return;
	fprintf(fp, "%s %s: %s\n", timestamp, log_domain != NULL ? log_domain : "fwupd", message);
	fclose(fp);
}
...
fu_binder_daemon_setup(FuDaemon *daemon, const gchar *address, FuProgress *progress, GError **error)
{
	...
	/* logging */
	g_log_set_default_handler(fu_binder_daemon_log_handler, NULL);
