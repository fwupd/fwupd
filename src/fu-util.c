...
static void
fu_util_get_releases_print_json(FuUtil *self, GPtrArray *rels)
{
	...
	for (guint i = 0; i < rels->len; i++) {
		...
		if (!fwupd_release_match_flags(rel,
			...
			continue;
		fwupd_codec_to_json(FWUPD_CODEC(rel), json_object_tmp, FWUPD_CODEC_FLAG_NONE);
		fwupd_json_array_add_object(json_array, json_object_tmp);
	}
	fwupd_json_object_add_array(json_object, "Releases", json_array);
	fu_util_print_json_object(self->console, json_object);
}
...
fu_util_get_devices_as_json(FuUtil *self, GPtrArray *devs)
{
	...
	for (guint i = 0; i < devs->len; i++) {
		...
		/* add to builder */
		fwupd_codec_to_json(FWUPD_CODEC(dev), json_object_tmp, FWUPD_CODEC_FLAG_TRUSTED);
		fwupd_json_array_add_object(json_array, json_object_tmp);
	}
	fwupd_json_object_add_array(json_object, "Devices", json_array);
	fu_util_print_json_object(self->console, json_object);
}
...
fu_util_get_plugins(FuUtil *self, gchar **values, GError **error)
{
	...
	if (self->as_json) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
		fwupd_codec_array_to_json(plugins, "Plugins", json_object, FWUPD_CODEC_FLAG_TRUSTED);
		fu_util_print_json_object(self->console, json_object);
		return TRUE;
	}
			      ...
			      GError **error)
{
	...
	name = fwupd_json_object_get_string_with_default(json_object, "name", "component", error);
	if (name == NULL)
		return FALSE;
	fwupd_json_object_add_string(json_object_result, "name", name);
	protocol = fwupd_json_object_get_string(json_object, "protocol", NULL);
	if (protocol != NULL)
		fwupd_json_object_add_string(json_object_result, "protocol", protocol);

	/* find the device with any of the matching GUIDs */
	json_array = fwupd_json_object_get_array(json_object, "guids", error);
	if (json_array == NULL)
		return FALSE;
	fwupd_json_object_add_array(json_object_result, "guids", json_array);
	for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
		...
		guid = fwupd_json_array_get_string(json_array, i, error);
		if (guid == NULL)
			return FALSE;
	...
	version = fwupd_json_object_get_string(json_object, "version", NULL);
	if (version != NULL) {
	...
	version = fwupd_json_object_get_string(json_object, "version-bootloader", NULL);
	if (version != NULL) {
	...
	version = fwupd_json_object_get_string(json_object, "branch", NULL);
	if (version != NULL) {
			 ...
			 GError **error)
{
	...
	if (helper->use_emulation) {
		...
		url_tmp = fwupd_json_object_get_string(json_object, "emulation-url", NULL);
		if (url_tmp != NULL) {
			...
			emulation_filename = g_strdup(
			    fwupd_json_object_get_string(json_object, "emulation-file", NULL));
			if (emulation_filename == NULL)
				return TRUE;
		}
	...
	url_tmp = fwupd_json_object_get_string(json_object, "url", NULL);
	if (url_tmp != NULL) {
		g_autofree gchar *filename = NULL;
	...
	/* process each step */
	json_array = fwupd_json_object_get_array(json_object, "components", error);
	if (json_array == NULL)
		return FALSE;
	for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
			     ...
			     GError **error)
{
	...
	json_object = fwupd_json_node_get_object(json_node, error);
	if (json_object == NULL)
		return FALSE;

	/* some elements are optional */
	helper->name = fwupd_json_object_get_string(json_object, "name", NULL);
	if (helper->name != NULL)
		fwupd_json_object_add_string(json_object_result, "name", helper->name);
	if (!fwupd_json_object_get_boolean_with_default(json_object,
							"interactive",
							&interactive,
							FALSE,
							error))
	...
	json_archs_cpu = fwupd_json_object_get_array(json_object, "cpu-architectures", NULL);
	...
	json_archs_plat = fwupd_json_object_get_array(json_object, "platform-architectures", NULL);
	...
	/* process each step */
	if (!fwupd_json_object_get_integer_with_default(json_object, "repeat", &repeat, 1, error))
		return FALSE;
	fwupd_json_object_add_integer(json_object_result, "repeat", repeat);

	json_steps = fwupd_json_object_get_array(json_object, "steps", error);
	if (json_steps == NULL)
		return FALSE;
...
fu_util_get_details(FuUtil *self, gchar **values, GError **error)
{
	...
	if (self->as_json) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
		fwupd_codec_array_to_json(array, "Devices", json_object, FWUPD_CODEC_FLAG_TRUSTED);
		fu_util_print_json_object(self->console, json_object);
	}
...
fu_util_get_history(FuUtil *self, gchar **values, GError **error)
{
	...
	if (self->as_json) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
		fwupd_codec_array_to_json(devices, "Devices", json_object, FWUPD_CODEC_FLAG_TRUSTED);
		fu_util_print_json_object(self->console, json_object);
	}
...
fu_util_get_results(FuUtil *self, gchar **values, GError **error)
{
	...
	if (self->as_json) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
		fwupd_codec_to_json(FWUPD_CODEC(rel), json_object, FWUPD_CODEC_FLAG_TRUSTED);
		fu_util_print_json_object(self->console, json_object);
	}
...
fu_util_get_releases(FuUtil *self, gchar **values, GError **error)
{
	...
	/* not for human consumption */
	if (self->as_json) {
		fu_util_get_releases_print_json(self, rels);
		return TRUE;
	}
...
static void
fu_util_get_updates_print_json(FuUtil *self, GPtrArray *devices)
{
	g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
	g_autoptr(FwupdJsonArray) json_array = fwupd_json_array_new();
...
fu_util_get_updates(FuUtil *self, gchar **values, GError **error)
{
	...
	/* not for human consumption */
	if (self->as_json) {
		fu_util_get_updates_print_json(self, devices);
		return TRUE;
	}
...
fu_util_get_remotes(FuUtil *self, gchar **values, GError **error)
{
	...
	if (self->as_json) {
		g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
		fwupd_codec_array_to_json(remotes, "Remotes", json_object, FWUPD_CODEC_FLAG_TRUSTED);
		fu_util_print_json_object(self->console, json_object);
	}
...
static void
fu_util_get_checksums_print_json(FuUtil *self, gchar **csums)
{
	g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();
	g_autoptr(FwupdJsonArray) json_array = fwupd_json_array_new();

	for (guint i = 0; csums[i] != NULL; i++)
		fwupd_json_array_add_string(json_array, csums[i]);
...
fu_util_get_approved_firmware(FuUtil *self, gchar **values, GError **error)
{
	...
	if (checksums == NULL)
		return FALSE;
	if (self->as_json) {
		fu_util_get_checksums_print_json(self, checksums);
		return TRUE;
	}
...
static void
fu_util_security_print_json(FuUtil *self, GPtrArray *attrs, GPtrArray *events, GPtrArray *devices)
{
	g_autoptr(GPtrArray) devices_issues = NULL;
	g_autoptr(FwupdJsonObject) json_object = fwupd_json_object_new();

	/* attrs */
	fwupd_codec_array_to_json(attrs, "SecurityAttributes", json_object, FWUPD_CODEC_FLAG_TRUSTED);
...
fu_util_security(FuUtil *self, gchar **values, GError **error)
{
	...
	/* not for human consumption */
	if (self->as_json) {
		fu_util_security_print_json(self, attrs, events, devices);
		return TRUE;
	}
...
fu_util_get_blocked_firmware(FuUtil *self, gchar **values, GError **error)
{
	...
	if (csums == NULL)
		return FALSE;
	if (self->as_json) {
		fu_util_get_checksums_print_json(self, csums);
		return TRUE;
	}
...
fu_util_hwids_as_json(FuUtil *self, GStrv hwids_keys, GStrv hwids_values)
{
	...
	for (guint i = 0; hwids_keys[i] != NULL; i++)
		fwupd_json_object_add_string(json_object, hwids_keys[i], hwids_values[i]);
	fu_util_print_json_object(self->console, json_object);
}
...
fu_util_version(FuUtil *self, GError **error)
{
	...
	if (self->as_json) {
		fu_util_project_versions_print_json(self->console, metadata);
		return TRUE;
	}
