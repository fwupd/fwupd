			     ...
			     GError **error)
{
	...
	if (g_key_file_has_key(kf, group, "AuthType", NULL)) {
		g_autofree gchar *tmp = g_key_file_get_string(kf, group, "AuthType", NULL);
		fwupd_remote_set_auth_type(self, fwupd_remote_auth_type_from_string(tmp));
	}
			   ...
			   GError **error)
{
	...
	if (fwupd_remote_get_auth_type(self) != FWUPD_REMOTE_AUTH_TYPE_NONE)
		g_key_file_set_string(kf, group, "AuthType", fwupd_remote_auth_type_to_string(fwupd_remote_get_auth_type(self)));
