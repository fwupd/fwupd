...
static void
fu_engine_plugin_device_gtype(FuContext *ctx, GType gtype)
{
	GType proxy_gtype;
	gboolean ret;
...
static void
fu_engine_plugin_firmware_gtype(FuContext *ctx, GType gtype)
{
	gboolean ret;
	g_autoptr(FuFirmware) firmware = NULL;
...
fu_engine_plugin_gtypes_func(gconstpointer user_data)
{
	...
	for (guint i = 0; i < plugins->len; i++) {
		...
		for (guint j = 0; device_gtypes != NULL && j < device_gtypes->len; j++) {
			...
			fu_engine_plugin_device_gtype(ctx, gtype);
		}
	}
	...
	for (guint j = 0; j < firmware_gtypes->len; j++) {
		...
		fu_engine_plugin_firmware_gtype(ctx, gtype);
	}
}
...
fu_engine_downgrade_func(gconstpointer user_data)
{
	...
	/* xxx */
	(void)g_setenv("FWUPD_LOCALSTATEDIR", "/tmp/fwupd-self-test3/var", TRUE);
	(void)g_setenv("FWUPD_RUNDIR", "/tmp/fwupd-self-test3/run", TRUE);

	/* write a broken file */
	ret = fu_path_mkdir_parent("/tmp/fwupd-self-test3/broken.xml.gz", &error);
	g_assert_no_error(error);
	g_assert_true(ret);
	ret = g_file_set_contents("/tmp/fwupd-self-test3/broken.xml.gz",
				  "this is not a valid",
				  -1,
				  &error);
	g_assert_no_error(error);
	...
	ret = g_file_set_contents(
	    "/tmp/fwupd-self-test3/stable.xml",
	    "<components>"
	    "  <component type=\"firmware\">"
	    "    <id>test</id>"
	...
	ret = g_file_set_contents(
	    "/tmp/fwupd-self-test3/testing.xml",
	    "<components>"
	    "  <component type=\"firmware\">"
	    "    <id>test</id>"
	    "    <name>Test Device</name>"
...
fu_engine_md_verfmt_func(gconstpointer user_data)
{
	...
	gboolean ret;
	g_autofree gchar *remote_fn = NULL;
	g_autoptr(FuDevice) device = fu_device_new(ctx);
	g_autoptr(FuEngine) engine = fu_engine_new(ctx);
	g_autoptr(FuEngineRequest) request = fu_engine_request_new(NULL);
	g_autoptr(FuProgress) progress = fu_progress_new(G_STRLOC);
	...
	/* deleted on error */
	tmpdir = fu_temporary_directory_new("engine-md-verfmt", &error);
	g_assert_no_error(error);
	g_assert_nonnull(tmpdir);

	(void)g_setenv("FWUPD_LOCALSTATEDIR", fu_temporary_directory_get_path(tmpdir), TRUE);
	(void)g_setenv("FWUPD_RUNDIR", "/tmp/fwupd-self-test2/run", TRUE);
	/* ensure empty tree */
	/* fu_self_test_mkroot(); */
	...
	/* write the main file */
	remote_fn = fu_temporary_directory_build(tmpdir, "stable.xml", NULL);
	ret = g_file_set_contents(
	    remote_fn,
	    "<components>"
	    "  <component type=\"firmware\">"
	...
	fwupd_remote_set_filename_cache(remote, remote_fn);
	...
	remote_tmp = fu_engine_get_remote_by_id(engine, "stable", &error);
	g_assert_no_error(error);
	g_assert_nonnull(remote_tmp);
...
fu_engine_history_verfmt_func(gconstpointer user_data)
{
	...
	/* deleted on error */
	tmpdir = fu_temporary_directory_new("engine-history-verfmt", &error);
	g_assert_no_error(error);
	g_assert_nonnull(tmpdir);
	(void)g_setenv("FWUPD_LOCALSTATEDIR", fu_temporary_directory_get_path(tmpdir), TRUE);
...
fu_engine_install_needs_reboot(gconstpointer user_data)
{
	...
	g_autoptr(XbNode) component = NULL;
	g_autoptr(XbSilo) silo_empty = xb_silo_new();
	g_autofree gchar *rundir = NULL;
	g_autofree gchar *reboot_file = NULL;

	(void)g_setenv("FWUPD_RUNDIR", "/tmp/fwupd-self-test/run", TRUE);

	/* create rundir */
	rundir = fu_path_from_kind(FU_PATH_KIND_RUNDIR);
...
static void
fu_engine_module_func(gconstpointer user_data)
{
	FuContext *ctx = (FuContext *)user_data;
	GError *error = NULL;
	gboolean ret;
...
static void
fu_engine_composite_multistep_func(gconstpointer user_data)
{
	FuContext *ctx = (FuContext *)user_data;
	gboolean ret;
	g_autoptr(FuDevice) device = NULL;
...
main(int argc, char **argv)
{
	...
	fu_context_add_flag(ctx, FU_CONTEXT_FLAG_FDE_SNAPD);
	fu_context_add_flag(ctx, FU_CONTEXT_FLAG_INHIBIT_VOLUME_MOUNT);
	...
	g_test_add_data_func("/fwupd/engine/module", ctx, fu_engine_module_func);
	...
	g_test_add_data_func("/fwupd/engine/device-unlock", ctx, fu_engine_device_unlock_func);
	...
	g_test_add_data_func("/fwupd/engine/multiple-releases", ctx, fu_engine_multiple_rels_func);
	g_test_add_data_func("/fwupd/engine/install/loop-restart",
			     ctx,
			     fu_engine_install_loop_restart_func);
	g_test_add_data_func("/fwupd/engine/install/request", ctx, fu_engine_install_request);
	g_test_add_data_func("/fwupd/engine/history-success", ctx, fu_engine_history_func);
	g_test_add_data_func("/fwupd/engine/history-verfmt", ctx, fu_engine_history_verfmt_func);
	g_test_add_data_func("/fwupd/engine/history-modify", ctx, fu_engine_history_modify_func);
	g_test_add_data_func("/fwupd/engine/history-error", ctx, fu_engine_history_error_func);
	g_test_add_data_func("/fwupd/engine/report-metadata", ctx, fu_engine_report_metadata_func);
	g_test_add_data_func("/fwupd/engine/require-hwid", ctx, fu_engine_require_hwid_func);
	g_test_add_data_func("/fwupd/engine/requires-reboot", ctx, fu_engine_install_needs_reboot);
	g_test_add_data_func("/fwupd/engine/history-inherit", ctx, fu_engine_history_inherit);
	g_test_add_data_func("/fwupd/engine/history-convert-version",
			     ctx,
			     fu_engine_history_convert_version_func);
	g_test_add_data_func("/fwupd/engine/partial-hash", ctx, fu_engine_partial_hash_func);
	g_test_add_data_func("/fwupd/engine/downgrade", ctx, fu_engine_downgrade_func);
	g_test_add_data_func("/fwupd/engine/md-verfmt", ctx, fu_engine_md_verfmt_func);
	...
	g_test_add_data_func("/fwupd/engine/release-dedupe", ctx, fu_engine_release_dedupe_func);
	g_test_add_data_func("/fwupd/engine/generate-md", ctx, fu_engine_generate_md_func);
	g_test_add_data_func("/fwupd/engine/plugin-gtypes", ctx, fu_engine_plugin_gtypes_func);
	g_test_add_data_func("/fwupd/engine/mutable",
			     ctx,
			     fu_engine_test_plugin_mutable_enumeration);
	g_test_add_data_func("/fwupd/engine/composite", ctx, fu_plugin_composite_func);
	g_test_add_data_func("/fwupd/engine/composite-multistep",
			     ctx,
			     fu_engine_composite_multistep_func);
	g_test_add_data_func("/fwupd/engine/write-bios-attrs",
			     ctx,
			     fu_engine_modify_bios_settings_func);
	return g_test_run();
}
