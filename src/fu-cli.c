...
static gboolean
fu_cli_client_check_daemon_version(FwupdClient *client, gpointer userdata, GError **error)
{
	FuCli *self = FU_CLI(userdata);
	const gchar *daemon = fwupd_client_get_daemon_version(client);
...
static gboolean
fu_cli_client_show_unsupported_warning(FwupdClient *client, gpointer user_data, GError **error)
{
			    ...
			    GError **error)
{
	...
	if (repeat < 1 || repeat > 1000) {
		g_set_error(error,
			    ...
			    "repeat value %" G_GINT64_FORMAT " is out of range",
			    repeat);
		return FALSE;
	}
...
fu_cli_report_history_full(FuCli *self, gboolean only_automatic_reports, GError **error)
{
	...
	fu_console_print_literal(priv->console, _("Successfully uploaded report"));
	return TRUE;
}
...
fu_cli_download_metadata_enable_lvfs(FuCli *self, GError **error)
{
	...
	remote =
	    fwupd_client_get_remote_by_id(priv->client, "lvfs", priv->cancellable, &error_local);
	if (remote == NULL) {
		g_debug("no LVFS remote: %s", error_local->message);
		return TRUE;
	}
...
fu_cli_watch_sigint_stop(FuCli *self)
{
	...
	g_clear_pointer(&priv->source_sigint, g_source_unref);
}
			   ...
			   GError **error)
{
	...
	if (helper->allow_branch_switch)
		fu_cli_add_arg_flag(self, FU_CLI_ARG_FLAG_ALLOW_BRANCH_SWITCH);
	if (helper->allow_older)
		fu_cli_add_arg_flag(self, FU_CLI_ARG_FLAG_ALLOW_OLDER);
	if (helper->allow_reinstall)
		fu_cli_add_arg_flag(self, FU_CLI_ARG_FLAG_ALLOW_REINSTALL);
...
static gboolean
fu_cli_client_show_tainted_warnings(FwupdClient *client, gpointer user_data, GError **error)
{
	FuCli *self = FU_CLI(user_data);
	FuCliPrivate *priv = GET_PRIVATE(self);
...
static gboolean
fu_cli_client_show_plugin_warnings(FwupdClient *client, gpointer user_data, GError **error)
{
	FuCli *self = FU_CLI(user_data);
	FuCliPrivate *priv = GET_PRIVATE(self);
	...
	/* get plugins from daemon */
	plugins = fwupd_client_get_plugins(client, priv->cancellable, error);
	if (plugins == NULL)
		return FALSE;
...
fu_cli_bios_settings_tui(FuCli *self, gchar **values, GError **error)
{
	...
	for (guint i = 0; i < attrs->len; i++)
		fu_cli_bios_setting_update_description(g_ptr_array_index(attrs, i));
	settings = fu_bios_settings_tui_run(attrs, error);
	if (settings == NULL)
		return FALSE;
...
fu_cli_main(FuCli *self, gint argc, gchar **argv)
{
	...
	/* functions to run post-connect */
	fwupd_client_add_connect_func(priv->client,
				      fu_cli_client_show_unsupported_warning,
				      self,
				      NULL);
	fwupd_client_add_connect_func(priv->client,
				      fu_cli_client_show_tainted_warnings,
				      self,
				      NULL);
	fwupd_client_add_connect_func(priv->client, fu_cli_client_show_plugin_warnings, self, NULL);
	fwupd_client_add_connect_func(priv->client, fu_cli_client_check_daemon_version, self, NULL);
	...
	if (argc < 2) {
		...
		/* TRANSLATORS: the user just typed in fwupdmgr with no command */
		fu_console_print(priv->console, "%s: %s", _("No command specified"), str->str);
		return EXIT_FAILURE;
	}
...
fu_cli_finalize(GObject *obj)
{
	...
	if (priv->source_sigint != NULL) {
		g_source_destroy(priv->source_sigint);
		g_source_unref(priv->source_sigint);
	}
