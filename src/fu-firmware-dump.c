...
extern void
HF_ITER(guint8 **buf, gsize *len);

static gboolean
fu_firmware_dump_parse(FuUtil *self, FuFirmware *firmware, GBytes *fw, GError **error)
{
	gboolean ret;
	gdouble elapsed_ms;
	...
	ret = fu_firmware_parse(firmware,
				fw,
				FWUPD_INSTALL_FLAG_NO_SEARCH | FWUPD_INSTALL_FLAG_IGNORE_VID_PID |
	...
	if (self->timeout > 0 && elapsed_ms > self->timeout) {
		g_set_error(error,
			    G_IO_ERROR,
...
main(int argc, char **argv)
{
	...
	const GOptionEntry options[] = {{"verbose",
					...
					{"timeout",
					 ...
					 &self->timeout,
					 /* TRANSLATORS: command line option */
					 _("Timeout in milliseconds for each parse"),
					 NULL},
					{NULL}};
