...
fu_binder_remote_func(void)
{
	...
	fwupd_remote_set_checksum_sig(remote, "sha256-abcdef");
	...
	remote2 = fu_binder_remote_from_aidl(r, &error);
	...
	/* the signature checksum round-trips */
	g_assert_cmpstr(fwupd_remote_get_checksum(remote2), ==, "sha256-abcdef");

	/* metadataUriSig and filenameCacheSig are derived from their parent URIs */
	g_assert_cmpstr(fwupd_remote_get_metadata_uri_sig(remote2),
			==,
			"https://fwupd.org/metadata.xml.gz.jcat");
	g_assert_cmpstr(fwupd_remote_get_filename_cache_sig(remote2), ==, "/tmp/cache.xml.gz.jcat");

	/* the daemon exports but cannot be set on the device */
	g_assert_cmpstr(r.checksumSig.value().c_str(), ==, "sha256-abcdef");
}
...
fu_binder_release_func(void)
{
	...
	release2 = fu_binder_release_from_aidl(r, &error);
	g_assert_no_error(error);
	g_assert_nonnull(release2);
...
fu_binder_device_func(void)
{
	...
	device2 = fu_binder_device_from_aidl(r, &error);
	g_assert_no_error(error);
...
fu_binder_request_func(void)
{
	...
	fwupd_request_set_device_id(request, "0000000000000000000000000000000000000000");
	fwupd_request_set_message(request, "message");
	fwupd_request_set_image(request, "image.png");
	fwupd_request_set_created(request, 0x1234);
	fwupd_request_add_flag(request, FWUPD_REQUEST_FLAG_ALLOW_GENERIC_MESSAGE);

	r = fu_binder_request_to_aidl(request);
	request2 = fu_binder_request_from_aidl(r, &error);
	g_assert_no_error(error);
	g_assert_nonnull(request2);
	...
	g_assert_cmpstr(fwupd_request_get_device_id(request2),
			==,
			"0000000000000000000000000000000000000000");
	g_assert_cmpstr(fwupd_request_get_message(request2), ==, "message");
	g_assert_cmpstr(fwupd_request_get_image(request2), ==, "image.png");
	g_assert_cmpint(fwupd_request_get_created(request2), ==, 0x1234);
