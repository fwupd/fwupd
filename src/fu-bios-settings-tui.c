...
static void
fu_bios_settings_tui_free(FuBiosSettingsTui *self)
{
...
G_DEFINE_AUTOPTR_CLEANUP_FUNC(FuBiosSettingsTui, fu_bios_settings_tui_free)
...
fu_bios_settings_tui_rebuild(FuBiosSettingsTui *self)
{
	...
	for (guint i = 0; i < self->settings->len; i++) {
		...
		const gchar *parent = fwupd_bios_setting_get_parent(setting);
		...
		if (self->search != NULL && self->search[0] != '\0') {
			if (!fu_bios_settings_tui_matches_search(name, self->search) &&
			    (parent == NULL ||
			     !fu_bios_settings_tui_matches_search(parent, self->search)))
				continue;
		...
		item = g_new0(FuBiosSettingsTuiItem, 1);
		if (self->parent == NULL && parent != NULL && parent[0] != '\0') {
			item->label = g_strdup(parent);
			...
			if (g_strcmp0(parent, self->parent) != 0) {
				fu_bios_settings_tui_item_free(item);
				continue;
			}
			item->label = g_strdup(name);
...
fu_bios_settings_tui_enter_single_group(FuBiosSettingsTui *self)
{
	while (self->items->len == 1) {
		...
		if (item->setting != NULL)
			return;
		g_set_str(&self->parent, item->parent);
		self->selected = 0;
		self->top = 0;
...
fu_bios_settings_tui_draw(FuBiosSettingsTui *self)
{
	...
	if (self->search != NULL) {
		...
		g_autofree gchar *title =
		    self->parent != NULL ? g_strdup_printf("/ %s", self->parent) : g_strdup("/");
		fu_bios_settings_tui_addnstr(1, 2, width - 4, title);
	}
...
fu_bios_settings_tui_edit_integer(FwupdBiosSetting *setting)
{
	...
	if (current != NULL)
		(void)fu_strtoull(current, &value, lower, upper, FU_INTEGER_BASE_AUTO, NULL);
...
fu_bios_settings_tui_search(FuBiosSettingsTui *self)
{
	...
	noecho();
	curs_set(0);
	g_set_str(&self->search, input[0] != '\0' ? input : NULL);
	self->selected = 0;
	self->top = 0;
	fu_bios_settings_tui_rebuild(self);
}
...
{
	g_autoptr(FuBiosSettingsTui) self = g_new0(FuBiosSettingsTui, 1);
	gboolean apply = FALSE;
	SCREEN *screen;

	self->settings = settings;
	self->items =
	    g_ptr_array_new_with_free_func((GDestroyNotify)fu_bios_settings_tui_item_free);
	...
	return g_steal_pointer(&self->pending);
}
