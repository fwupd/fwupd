...
fu_engine_emulator_load_json_blob(FuEngineEmulator *self, GBytes *json_blob, GError **error)
{
	...
	for (guint i = 0; i < backends->len; i++) {
		...
		if (!fwupd_codec_from_json(FWUPD_CODEC(backend), json_object, error))
			return FALSE;
	}
...
static void
fu_engine_emulator_to_json(FuEngineEmulator *self, GPtrArray *devices, FwupdJsonObject *json_object)
{
	...
	fwupd_json_object_add_string(json_object, "FwupdVersion", PACKAGE_VERSION);
			      ...
			      GError **error)
{
	...
	if (devices == NULL)
		return FALSE;
	fu_engine_emulator_to_json(self, devices, json_object);

	fn = fu_engine_emulator_phase_to_filename(composite_cnt, phase, write_cnt);
	g_debug("saving %s", fn);
	blob_old = g_hash_table_lookup(self->phase_blobs, fn);
	blob_new = fwupd_json_object_to_bytes(json_object, FWUPD_JSON_EXPORT_FLAG_INDENT);
