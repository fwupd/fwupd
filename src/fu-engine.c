...
fu_engine_verify_update (FuEngine *self, const gchar *device_id, GError **error)
{
	...
	for (guint i = 0; i < checksums->len; i++) {
		...
		csum = xb_builder_node_insert (provides, "checksum",
					       "type", _g_checksum_type_to_string (kind),
					       "target", "content",
					       NULL);
	...
	fn = g_strdup_printf ("%s/verify/%s.xmlb", localstatedir, device_id);
...
static gboolean
as_require_version_compare (XbNode *req, const gchar *version, GError **error)
{
	gboolean ret = FALSE;
	gint rc = 0;
		   ...
		   GError **error)
{
	...
	if (fu_device_has_flag (device, FWUPD_DEVICE_FLAG_NEEDS_BOOTLOADER)) {
		...
		caption = xb_node_query_text (component, "screenshots/screenshot/caption", NULL);
		if (caption != NULL) {
			g_set_error (error,
				     FWUPD_ERROR,
				     FWUPD_ERROR_INTERNAL,
...
static gboolean
fu_engine_add_releases_for_device_guid (FuEngine *self,
					FuDevice *device,
					const gchar *guid,
					GPtrArray *releases,
					GError **error)
{
	...
	/* find all the components that pass all the requirements */
	for (guint i = 0; i < components->len; i++) {
...
fu_engine_get_releases_for_device (FuEngine *self, FuDevice *device, GError **error)
{
	...
	for (guint i = 0; i < device_guids->len; i++) {
		...
		g_autoptr(GError) error_local = NULL;
		if (!fu_engine_add_releases_for_device_guid (self,
							     device,
							     guid,
							     releases,
							     &error_local)) {
