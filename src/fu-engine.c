...
static gboolean
fu_engine_security_attrs_from_json(FuEngine *self, FwupdJsonObject *json_object, GError **error)
{
	/* not supplied */
	if (!fwupd_json_object_has_node(json_object, "SecurityAttributes"))
		return TRUE;
	if (!fwupd_codec_from_json(FWUPD_CODEC(self->host_security_attrs), json_object, error))
		return FALSE;
...
static gboolean
fu_engine_devices_from_json(FuEngine *self, FwupdJsonObject *json_object, GError **error)
{
	g_autoptr(FwupdJsonArray) json_array = NULL;

	/* not supplied */
	json_array = fwupd_json_object_get_array(json_object, "Devices", NULL);
	if (json_array == NULL)
		return TRUE;
	for (guint i = 0; i < fwupd_json_array_get_size(json_array); i++) {
		...
		if (json_object_tmp == NULL)
			return FALSE;
		if (!fwupd_codec_from_json(FWUPD_CODEC(device), json_object_tmp, error))
			return FALSE;
...
fu_engine_load_host_emulation(FuEngine *self, const gchar *fn, GError **error)
{
	...
	if (json_object == NULL)
		return FALSE;
	if (!fu_engine_devices_from_json(self, json_object, error))
		return FALSE;
	if (!fu_engine_security_attrs_from_json(self, json_object, error))
		return FALSE;
	if (!fwupd_codec_from_json(FWUPD_CODEC(bios_settings), json_object, error))
		return FALSE;
