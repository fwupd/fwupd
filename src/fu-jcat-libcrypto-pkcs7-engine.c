...
fu_jcat_libcrypto_pkcs7_engine_add_public_key_raw(FuJcatEngine *engine,
						  ...
						  GError **error)
{
	FuJcatLibcryptoPkcs7Engine *self = FWUPD_JCAT_LIBCRYPTO_PKCS7_ENGINE(engine);
...
{
	FuJcatLibcryptoPkcs7Engine *self = FWUPD_JCAT_LIBCRYPTO_PKCS7_ENGINE(object);
...
fu_jcat_libcrypto_pkcs7_engine_new(FuJcatContext *context)
{
	...
	return FU_JCAT_ENGINE(g_object_new(FU_TYPE_JCAT_PKCS7_ENGINE,
					   "context",
					   context,
					   "kind",
