...
static FuEngine *
fu_engine_test_new (void)
{
	FuEngine *engine = fu_engine_new (FU_APP_FLAGS_NONE);
	fu_quirks_add_possible_key (fu_engine_get_quirks (engine), "Test");
	fu_quirks_add_possible_group (fu_engine_get_quirks (engine), "ACME Inc.");
	return engine;
}
...
fu_engine_generate_md_func (gconstpointer user_data)
{
	...
	g_autoptr(FuEngine) engine = fu_engine_test_new ();
