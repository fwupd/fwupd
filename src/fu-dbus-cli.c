...
fu_dbus_cli_get_bios_setting(FuCli *self, gchar **values, GError **error)
{
	...
	if (attrs == NULL)
		return FALSE;
	if (self->as_json) {
		fu_cli_bios_setting_console_print(self->console, values, attrs);
		return TRUE;
	}
...
fu_dbus_cli_monitor(FuCli *self, gchar **values, GError **error)
{
	...
	g_signal_connect(FWUPD_CLIENT(self->client),
			 "changed",
			 G_CALLBACK(fu_dbus_cli_changed_cb),
			 self);
	g_signal_connect(FWUPD_CLIENT(self->client),
			 "device-added",
			 G_CALLBACK(fu_dbus_cli_device_added_cb),
			 self);
	g_signal_connect(FWUPD_CLIENT(self->client),
			 "device-removed",
			 G_CALLBACK(fu_dbus_cli_device_removed_cb),
			 self);
	g_signal_connect(FWUPD_CLIENT(self->client),
			 "device-changed",
			 G_CALLBACK(fu_dbus_cli_device_changed_cb),
			 self);
