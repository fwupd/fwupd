...
"""
    ...
    if venv_python.exists():
        ...
        for package, version in PIP_PACKAGES.items():
            pip.install_package(package, version)
...
def setup_hooks(pre_push_hooks: bool) -> int:
    ...
    if not shutil.which("pre-commit"):
        ...
        pip.install_package("pre-commit")

    logger.info("Configuring pre-commit hooks")
    RunCmd(["pre-commit", "install"], capture=True, cwd=root)

    if pre_push_hooks:
        logger.info("Installing pre-push test hooks")
        RunCmd(["pre-commit", "install", "-t", "pre-push"], capture=True, cwd=root)
