...
int main(int argc, char *argv[]) {
  ...
  int rc = libusb_hotplug_register_callback(
      ...
      DockVid, DockPid, LIBUSB_HOTPLUG_MATCH_ANY, hotplug_callback,
      NULL, // 用户数据传递
      &callback_handle);
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "/c") == 0) {
      ...
      printf("%d.%d.%02d\n", Info[1].CurrentFwVersion[1],
             Info[1].CurrentFwVersion[2], Info[1].CurrentFwVersion[3]);
      printf("DP: ");
      printf("%d.%02d.%03d\n", Info[2].CurrentFwVersion[1],
             Info[2].CurrentFwVersion[2], Info[2].CurrentFwVersion[3]);
      printf("PD: ");
      for (int i = 0; i < 3; i++)
      ...
      // printf("%s\n", rs ? "true" : "false");
      if (rs == true) {
        printf("Phase-1 update is already done, by rc %d", FWU_PHASE1_LOCKED);
      ...
      if (r == 0) {
        ...
        /*Notice : rc == 0 includes 2 condition of FW update result as below .
                   1. FW update process is actaully be executed because Dock's
           current composite is < target composite version.
                   2. FW update process doesn't wich actually erase & flash FW
           image since the Dock's current composite version is >= target
           composite version. *Assume fwupd pluggin will cover for the condition
           with message output*/
