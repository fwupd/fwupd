...
uint8_t *GetBytes(struct UsageInformation targetUsageInformationTable);
...
int ReadCompositeImageFile(char **data);

uint8_t *GetCompositeData(int addr, int size, char *data) {
...
bool CheckFwVerify(int flashId,
                   struct UsageInformation targetUsageInformationTable,
                   char *compositeImageData) {
  int maxSize =
      BytesToInt(targetUsageInformationTable.FlashIdList[flashId].MaxSize, 4);
  int targetFwSize = BytesToInt(
      targetUsageInformationTable.FlashIdList[flashId].TargetFwFileSize, 4);
  if (targetFwSize + SignatureSize > maxSize)
    return false;

  int startAddr =
      BytesToInt(
          targetUsageInformationTable.FlashIdList[flashId].PhysicalAddress, 4) +
      SignatureSize;
  uint8_t *flashIdImageData =
      GetCompositeData(startAddr, targetFwSize, compositeImageData);
  int a = BytesToInt(
      targetUsageInformationTable.FlashIdList[flashId].TargetFwFileCrc32, 4);
...
struct FlashIdAttribute GetFlashIdAttribute(uint8_t *data) {
...
void TriggerPhase2(bool noUnplug) {
  uint8_t DfuCtrl[2] = {0};
  ...
  if (noUnplug) {
    ...
    DfuCtrl[1] = nonUnplug;
    int r =
        Function1(Dock, Set_Dock_Firmware_Upgrade_Ctrl, 0, DfuCtrl, 2, output1);
  } else {
    DfuCtrl[0] = Locked;
    DfuCtrl[1] = unplug;
    int r =
...
int WriteUsageInformationTable(uint8_t *usageInformationData) {
  int errorHandle = 0;
  ...
  for (int i = 0; i < Interface1Length; i++)
    output1[i] = 0;
  int usageInformationAttributeData = Function1(
      External_Flash, Get_Flash_Attribute, UsageTableFlashID, 0, 0, output1);
  ...
  for (int readBytes = 0; readBytes < UsageTableSize;
    ...
    uint8_t setUsageInformationMemoryErase[2 + AddressByteLength +
                                           EraseSizeByteLength];
    ...
    for (int i = 0; i < Interface2Length; i++)
      output2[i] = 0;
    errorHandle =
        Function2(External_Flash, Set_Flash_Memory_Access, UsageTableFlashID,
                  setUsageInformationMemoryErase,
                  2 + AddressByteLength + EraseSizeByteLength, output2);
    if (errorHandle != 0)
      return errorHandle;
  }
  ...
  for (int readBytes = 0; readBytes < UsageTableSize;
    ...
    for (int i = 0; i < usageInformationAttribute.ProgramSize + payloadLength;
         i++)
      write[i] = 0;
    ...
    for (int i = 0; i < Interface2Length; i++)
      output2[i] = 0;
    errorHandle = Function2(
        External_Flash, Set_Flash_Memory_Access, UsageTableFlashID, write,
        (usageInformationAttribute.ProgramSize + payloadLength), output2);
    if (errorHandle != 0)
      return errorHandle;
  }
  ...
  for (int i = 0; i < Interface1Length; i++)
    output1[i] = 0;
  int pageSelfVerifyData =
      Function1(External_Flash, Get_Flash_Memory_Self_Verify, UsageTableFlashID,
                writeSelfVerify, 1, output1);
...
int WriteFlashIdData(int flashId, struct FlashIdAttribute flashIdAttribute,
  ...
  if (!CheckFwVerify(flashId, changeTagetUsageInformationTable,
                     compositeImageData))
  ...
  int targetMaxSize = BytesToInt(
      changeTagetUsageInformationTable.FlashIdList[flashId].MaxSize, 4);
  int targetFwSize = BytesToInt(
  ...
  for (int eraseBytes = 0; eraseBytes < targetMaxSize;
    ...
    for (int i = 0; i < AddressByteLength; i++)
      setFlashMemoryErase[2 + EraseSizeByteLength + i] =
          IntToBytes(eraseAddr)[i];

    for (int i = 0; i < Interface2Length; i++)
      output2[i] = 0;
    errorHandle = Function2(
        External_Flash, Set_Flash_Memory_Access, flashId, setFlashMemoryErase,
        2 + EraseSizeByteLength + AddressByteLength, output2);
    if (errorHandle != 0)
      return errorHandle;
  }

  // Set Flash Memory Access (program)
  int flashIdStartAddr = BytesToInt(
      changeTagetUsageInformationTable.FlashIdList[flashId].PhysicalAddress, 4);
  int flashIdDataSize = BytesToInt(
      changeTagetUsageInformationTable.FlashIdList[flashId].MaxSize, 4);
  ...
  for (int readBytes = 0; readBytes < targetFwSize + SignatureSize;
    ...
    for (int i = 0; i < Interface2Length; i++)
      output2[i] = 0;
    errorHandle =
        Function2(External_Flash, Set_Flash_Memory_Access, flashId, write,
                  flashIdAttribute.ProgramSize + payloadLength, output2);
    if (errorHandle != 0)
      return errorHandle;
  }
  ...
  for (int i = 0; i < Interface1Length; i++)
    output1[i] = 0;
  int flashUpdateCheckSignatureData =
      Function1(External_Flash, Get_Flash_Memory_Self_Verify, flashId,
                writeFlashUpdateCheckSignature, 1, output1);
  ...
  for (int i = 0; i < Interface1Length; i++)
    output1[i] = 0;
  int flashUpdateCheckVerifyData =
      Function1(External_Flash, Get_Flash_Memory_Self_Verify, flashId,
                writeFlashUpdateCheckVerify, 1, output1);
  ...
  if (!arraysEqual(changeTagetUsageInformationTable.FlashIdList[flashId]
                       .TargetFwFileCrc32,
                   flashUpdateCheckVerifyCrc, 4))
    return UPDATE_DOCK_FAILED;
...
int FWUpdate(bool forceUpdate, bool noUnplug) {
  ...
  for (int i = 0; i < Interface2Length; i++)
    output2[i] = 0;
  r = Function2(External_Flash, Set_Flash_Memory_Access, 0,
                setFlashMemoryRequest, 2, output2);
  if (r != 0) {
  ...
  if (stat(Path_Of_Usage_Information_Table, &fileInfo) == 0) {
    if (fileInfo.st_size != UsageTableSize) {
      ...
      for (int i = 1; i <= targetUsageInformationTable.Totalnumber; i++) {
        ...
        for (int j = 0; j < 4; j++) {
          targetUsageInformationTable.FlashIdList[i].PhysicalAddress[j] = ds[j];
          targetUsageInformationTable.FlashIdList[i].MaxSize[j] = ds[4 + j];
          ...
          targetUsageInformationTable.FlashIdList[i].TargetFwFileCrc32[j] =
              ds[20 + j];
          targetUsageInformationTable.FlashIdList[i].ComponentID = ds[24];
      ...
      if (BytesToInt(targetUsageInformationTable.Crc32, 4) !=
          Compute(buffer, sizeof(buffer) / sizeof(buffer[0]), 0,
                  UsageTableSize - 4))
  ...
  for (int i = 0; i < Interface1Length; i++)
    output1[i] = 0;
  int getFlashIdListData =
      Function1(External_Flash, Get_Flash_ID_List, 0, 0, 0, output1);
  ...
  do {
    ...
    for (int i = 0; i < Interface2Length; i++)
      output2[i] = 0;
    int tempBytes =
        Function2(External_Flash, Get_Flash_Memory_Access, UsageTableFlashID,
                  setmcuUsageInformationTable, payload, output2);
  ...
  if (!forceUpdate) {
    ...
    uint8_t *targetFWPackageVer =
        targetUsageInformationTable.CompositeFwVersion;
    int dockFWPackageVerInt = (dockFWPackageVer[0] << 16) +
                              (dockFWPackageVer[1] << 8) + dockFWPackageVer[2];
    int targetFWPackageVerInt = (targetFWPackageVer[1] << 16) +
                                (targetFWPackageVer[2] << 8) +
                                targetFWPackageVer[3];
  ...
  // Compare The Component FW Version
    ...
    if (bcdVerUpdateReq) {
      ...
      mcuUsageInformationTable.MinorVersion =
          (mcuUsageInformationData[1] & 0x0f);
      ...
      for (int i = 0; i < 2; i++) {
        mcuUsageInformationTable.DockPid[i] = mcuUsageInformationData[8 + i];
      }
      ...
      for (int i = 1; i <= mcuUsageInformationTable.Totalnumber; i++) {
        if (!arraysEqual(
          ...
          changeTagetUsageInformationTable.FlashIdList[i] = mcuFlashIdList[i];
          changeTagetUsageInformationTable.FlashIdList[i].Flag = false;
        }
      }
      free(mcuFlashIdList);
    }
  ...
  if (NeedWriteTable) {
    ...
    for (int i = 0; i < Interface1Length; i++)
      output1[i] = 0;
    r = Function1(Dock, Set_Dock_Firmware_Upgrade_Ctrl, 0, mcuUpdateCtrl, 2,
                  output1);
  ...
  for (int i = 1; i <= targetUsageInformationTable.Totalnumber; i++) {
    ...
    // Write Flash ID Data
    r = WriteFlashIdData(flashId, flashIdAttribute,
                         changeTagetUsageInformationTable, compositeImageData);
    if (r != 0)
      return r;
  ...
  for (int i = 0; i < Interface2Length; i++)
    output2[i] = 0;
  r = Function2(External_Flash, Set_Flash_Memory_Access, 0,
                SetFlashMemoryAccessRelease, 2, output2);
...
bool CheckDockReadyForEnterPhase2Update() {
  bool rs = false;
